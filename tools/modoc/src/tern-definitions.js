...
import * as typeexpressionstypeparser_parsejs from "./type-expressions/type-parser";
import * as typeexpressionsternserializer_serializejs from "./type-expressions/tern-serializer";
...
var convertParameters = function (parameters) {
	...
	ext_underscore__.extend(additionalDefinitions, pair.definitions);
...
var compileOptions = (function () {
	var generateId = util_utiljsjs('_options_');
	...
	function translate(node, definitions) {
		function translate(node) {
			if (Object.keys(node.children).length === 0) {
				...
				ext_underscore__(node.children).forEach(function (node, name) {
		...
		return ext_underscore__(node.children).mapObject(function (node, name) {
...
function makeConverter(classNames) {
	...
	return function (closureType) {
		var parsed = typeexpressionstypeparser_parsejs.parse(closureType);
		var ternType = typeexpressionsternserializer_serializejs.serialize(parsed);
		...
		ext_underscore__.forEach(ternType.definitions, function (definition, key) {
			additionalDefinitions[key] = ext_underscore__.mapObject(definition, function (member) {
...
function buildClasses(classes) {
	...
	ternDefinitions.goo = ext_underscore__.mapObject(classes, compileClass);
	...
	ext_fs_fs.writeFileSync(args.outPath + util_utiljsjs + 'tern-defs.json', result);
}
...
(function () {
	var files = trunk_trunkjsjs(args.sourcePath, IGNORE_FILES);

	var classes = trunk_trunkjsjs(files);
