...
import { PATH_SEPARATOR as util_PATH_SEPARATOR } from "./util";
...
function buildClasses(classes) {
	var classTemplate = ext_fs_fs.readFileSync(
		args.templatesPath + util_PATH_SEPARATOR + 'class.handlebars', { encoding: 'utf8' });
	...
	ext_fs_fs.writeFileSync(args.outPath + util_PATH_SEPARATOR + 'everything.html', result);
}

function buildIndex(index) {
	var navTemplate = ext_fs_fs.readFileSync(
		args.templatesPath + util_PATH_SEPARATOR + 'nav.handlebars', { encoding: 'utf8' });

	var result = ext_handlebars_handlebars.compile(navTemplate)({ index: index });

	ext_fs_fs.writeFileSync(args.outPath + util_PATH_SEPARATOR + 'index.html', result);
}
...
function buildChangelog(file) {
	...
	var changelogTemplate = ext_fs_fs.readFileSync(args.templatesPath + util_PATH_SEPARATOR + 'changelog.handlebars', { encoding: 'utf8' });

	var result = ext_handlebars_handlebars.compile(changelogTemplate)({ content: formatted });

	ext_fs_fs.writeFileSync(args.outPath + util_PATH_SEPARATOR + 'changelog.html', result);
}
...
function buildDeprecated(classes) {
	var deprecatedTemplate = ext_fs_fs.readFileSync(
		args.templatesPath + util_PATH_SEPARATOR + 'deprecated.handlebars', { encoding: 'utf8' });

	var data = compileDeprecated(classes);

	var result = ext_handlebars_handlebars.compile(deprecatedTemplate)(data);

	ext_fs_fs.writeFileSync(args.outPath + util_PATH_SEPARATOR + 'deprecated.html', result);
}
