...
import * as typeexpressionsjsdocserializer_serializejs from "./type-expressions/jsdoc-serializer";
...
var translateType = function (closureType) {
	if (closureType.trim().length > 0) {
		try {
			...
			var jsdocType = typeexpressionsjsdocserializer_serializejs.serialize(parsed);
			return jsdocType;
		} catch (e) {
			console.warn(e);
			return closureType;
