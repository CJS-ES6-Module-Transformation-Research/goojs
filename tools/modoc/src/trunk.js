...
function compileDoc(files) {
	...
	files.forEach(function (file) {
		console.log('compiling doc for ' + _util.getFileName);
		...
		if (class_.constructor) {
			(0, _jsdocProcessor.jsdocprocessorjs)(class_, files);
	...
	var constructorFromComment = function constructorFromComment(comment) {
		(0, _jsdocProcessor.jsdocprocessorjs)(comment);
		return {
	...
	extraComments.map(_jsdocProcessor.jsdocprocessorjs).forEach(function (extraComment) {
