...
var extractor_extractjs = _interopRequireWildcard(_extractor);

var _jsdocProcessor = require("./jsdoc-processor");

var jsdocprocessor_jsdocprocessorjsjs = _interopRequireWildcard(_jsdocProcessor);
...
function compileDoc(files) {
	...
	files.forEach(function (file) {
		...
		var class_ = extractor_extractjs.extract(source, file);

		Array.prototype.push.apply(extraComments, class_.extraComments);

		if (class_.constructor) {
			jsdocprocessor_jsdocprocessorjsjs.all(class_, files);
	...
	var constructorFromComment = function constructorFromComment(comment) {
		jsdocprocessor_jsdocprocessorjsjs.link(comment);
		return {
	...
	extraComments.map(jsdocprocessor_jsdocprocessorjsjs.compileComment).forEach(function (extraComment) {
