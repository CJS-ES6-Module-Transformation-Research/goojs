...
function getIndex(classes) {
	var files = ext__.pluck(classes, 'file').filter(Boolean);
	var differentiator = getDifferentiatorIndex(files);

	var groups = ext__.groupBy(files, function (file) {
	...
	Object.keys(groups).forEach(function (name) {
		...
		group.forEach(function (file) {
			...
			var fileName = util_getFileName(file);
...
mod_getIndex = getIndex;
var mod_getIndex;
export { mod_getIndex as getIndex };