...
(function (window, undefined) {
    ...
    Hammer.event = {
         ...
         * @param handler
         */
        bindDom: function bindDom(element, types, handler) {
            types = types.split(' ');
        ...
        extend: function extend(dest, src, depth) {
            ...
            for (var key in src) {
                if (src.hasOwnProperty(key)) {
                    if (depth && _typeof(src[key]) == 'object') {
    ...
    Hammer.gestures.Tap = {
        ...
        handler: function tapGesture(ev, inst) {
            if (ev.eventType == Hammer.EVENT_END) {
                ...
                if (ev.touchTime > inst.options.tap_max_touchtime || ev.distance > inst.options.tap_max_distance) {
                    return;
                }

                // check if double tap
                if (prev && prev.name == 'tap' && ev.time - prev.lastEvent.time < inst.options.doubletap_interval && ev.distance < inst.options.doubletap_distance) {
    ...
    Hammer.gestures.Drag = {
        ...
        handler: function dragGesture(ev, inst) {
            ...
            if (ev.eventType == Hammer.EVENT_MOVE) {
                ...
                if (ev.distance < inst.options.drag_min_distance && Hammer.gesture.current.name != this.name) {
                    return;
                }
                ...
                if (inst.options.drag_block_vertical && (ev.direction == Hammer.DIRECTION_UP || ev.direction == Hammer.DIRECTION_DOWN) || inst.options.drag_block_horizontal && (ev.direction == Hammer.DIRECTION_LEFT || ev.direction == Hammer.DIRECTION_RIGHT)) {
                    ev.preventDefault();
                }
            }
        }
    ...
    Hammer.gestures.Swipe = {
        ...
        handler: function swipeGesture(ev, inst) {
            if (ev.eventType == Hammer.EVENT_END) {
                // when the distance we moved is too small we skip this gesture
                // or we can be already in dragging
                if (Hammer.gesture.current.name == 'drag' && ev.touchTime > inst.options.swipe_min_time && ev.touchTime < inst.options.swipe_max_time && ev.distance > inst.options.swipe_min_distance) {
