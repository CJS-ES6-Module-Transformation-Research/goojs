...
function addFire(translation) {
	...
	ParticleSystemUtils.createParticleSystemEntity(world, ParticleLib.getFire({
		scale: 1,
		startColor: [1, 1, 0],
		endColor: [1, 0, 0]
	}), material).set(translation).addToWorld();
}
...
function addSmoke(translation) {
	...
	ParticleSystemUtils.createParticleSystemEntity(world, ParticleLib.getSmoke({
		scale: 1,
		color: [0.1, 0.05, 0]
	}), material).set(translation).addToWorld();
}
