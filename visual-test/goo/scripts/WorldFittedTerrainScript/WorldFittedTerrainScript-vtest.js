...
function addSpheres(goo, worldFittedTerrainScript, dims) {
    ...
    for (var i = 0, k = 0; i < nSpheres; i++, k += ak) {
        ...
        material.uniforms.color = [Math.cos(k) * 0.5 + 0.5, Math.cos(k + Math.PI / 3 * 2) * 0.5 + 0.5, Math.cos(k + Math.PI / 3 * 4) * 0.5 + 0.5];
...
function addNormalPointers(goo, worldFittedTerrainScript, dims) {
    ...
    for (var i = 0, k = 0; i < nSpheres; i++, k += ak) {
        var material = new Material(ShaderLib.simpleColored, '');
        material.uniforms.color = [Math.cos(k / nSpheres) * 0.5 + 0.5, Math.cos(k + Math.PI / 3 * 2) * 0.5 + 0.5, Math.cos(k + Math.PI / 3 * 4) * 0.5 + 0.5];
        var sphereEntity = gooRunner.world.createEntity(meshData, material);
...
function buildTexturedGround(matrix, dimensions, id, gooWorld, txPath) {
    ...
    material.uniforms.materialAmbient = [0.310305785123966943, 0.310305785123966943, 0.386363636363636367, 1];
    material.uniforms.materialDiffuse = [0.25909090909090909, 0.24909090909090909, 0.29909090909090909, 1];
