...
V.describe(['Tests that cubemaps with mipmaps works with typed array images. Should display two spheres with checkerboard reflections (left using images and right using arrays).'].join('\n'));
...
var cubemapShader = {
	...
	vshader: ['attribute vec3 vertexPosition;', 'attribute vec3 vertexNormal;', 'uniform mat4 viewMatrix;', 'uniform mat4 projectionMatrix;', 'uniform mat4 worldMatrix;', 'uniform vec3 cameraPosition;', 'varying vec3 eyeVec;', 'varying vec3 normal;', 'void main(void) {', 'vec4 worldPos = worldMatrix * vec4(vertexPosition, 1.0);', 'gl_Position = projectionMatrix * viewMatrix * worldPos;', 'normal = vertexNormal;', 'eyeVec = worldPos.xyz - cameraPosition;', '}'].join('\n'),
	fshader: ['precision mediump float;', 'uniform samplerCube diffuseMap;', 'varying vec3 eyeVec;', 'varying vec3 normal;', 'void main(void) {', 'vec3 reflectionVector = reflect(normalize(eyeVec), normal);', 'reflectionVector.yz = -reflectionVector.yz;', 'gl_FragColor = textureCube(diffuseMap, reflectionVector);', '}'].join('\n')
};
...
var images = ['../../../../resources/check.png', '../../../../resources/check-alt.png', '../../../../resources/check.png', '../../../../resources/check-alt.png', '../../../../resources/check.png', '../../../../resources/check-alt.png'];
