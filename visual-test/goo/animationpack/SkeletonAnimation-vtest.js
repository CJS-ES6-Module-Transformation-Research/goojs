...
function createJointChannel(joint, joints, times, t, r, s, blendType, channels) {
	...
	var channel = new JointChannel(joint._index, joint._name, times, r, translation, s, blendType);
...
function createBottleMeshData() {
	...
	var verts = [
	//Outer shell
	0, 0, 0, radius, 0, 0, radius, height * 0.5, 0, radius * 0.5, height * 0.7, 0,
	// Bottle lip
	radius * 0.25, height, 0, radius * 0.2 - thickness, height, 0,
	//Inner shell
	radius * 0.5 - thickness, height * 0.7, 0, radius - thickness, height * 0.5, 0, radius - thickness, height * 0.1, 0, 0, height * 0.1, 0];
...
function addSkeletonAnimation(entity, meshData, diffuse, loopCount, timeScale) {
	...
	var scales = [1, 1, 1, 1, 1, 1, 1, 1, 1];

	var animChannels = [];

	var rootChannel = createJointChannel(rootJoint, joints, times, trans, rots, scales, AbstractAnimationChannel.BLENDTYPES.QUINTIC, animChannels);
	...
	q2.fromAngleNormalAxis(-MathUtils.HALF_PI * 0.5, new Vector3(1, 1, 0).normalize());
	Array.prototype.push.apply(rots, [q2.x, q2.y, q2.z, q2.w]);
	var trans = [0, 0, 0, 0, 0.25, 0, 0, 0.1, 0.1];
	var scales = [0.5, 0.5, 0.5, 1, 1, 1, 1, 1, 1];
	var midChannel = createJointChannel(midJoint, joints, times, trans, rots, scales, AbstractAnimationChannel.BLENDTYPES.QUADRATIC, animChannels);
	...
	Array.prototype.push.apply(rots, [q1.x, q1.y, q1.z, q1.w]);
	Array.prototype.push.apply(rots, [q2.x, q2.y, q2.z, q2.w]);
	var scales = [2, 2, 2, 1, 1, 1, 0.5, 0.5, 0.5];
	var trans = [0, -0.25, 0, 0, 0, 0, 0, 0.25, 0];
	var topChannel = createJointChannel(topJoint, joints, times, trans, rots, scales, AbstractAnimationChannel.BLENDTYPES.QUINTIC, animChannels);
