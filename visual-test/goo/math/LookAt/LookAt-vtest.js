...
function addCone() {
	var box = world.createEntity(new Box(), V.getColoredMaterial(), [0, 1, 0]).addToWorld();

	var cone = world.createEntity(new Cone(), V.getColoredMaterial()).setScale(0.5, 0.5, -8).addToWorld();

	return world.createEntity().attachChild(box).attachChild(cone).addToWorld();
}

function addSphere() {
	var sphere = world.createEntity(new Sphere(32, 32), V.getColoredMaterial()).addToWorld();
...
var getSphereTranslation = function getSphereTranslation() {
