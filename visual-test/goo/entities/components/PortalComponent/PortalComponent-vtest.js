...
V.describe(['4 portals with different properties', 'Controls:', '', '1: request redraw of the upper-left portal', '2: add-remove the portal component on the bottom-left portal'].join('\n'));
...
function addSpinningCamera(rotationOffset) {
	...
	cameraEntity.setComponent(new ScriptComponent({
		run: function run(entity) {
			entity.transformComponent.transform.translation.setDirect(Math.cos(world.time + rotationOffset) * radius, 0, Math.sin(world.time + rotationOffset) * radius);
...
var overridenMaterialPortalEntity = addPortal(userCamera, 3, 3, 2, 5, { preciseRecursion: true, autoUpdate: false, alwaysRender: true }, overrideMaterial);
...
document.addEventListener('keypress', function (e) {
	switch (e.which) {
		...
		case 50:
			key2();break;
		default:
			console.log('1 - request redraw of the non-auto updated portal\n' + '2 - add/remove a portal component on one of the portals');
	}
});
