...
function addPrimitives() {
	for (var i = 0; i < 20; i++) {
		var position = [V.rng.nextFloat() * 16 - 8, V.rng.nextFloat() * 16 + 8, V.rng.nextFloat() * 16 - 8];
...
function createGround() {
	var entity = world.createEntity(new Quad(1000, 1000, 100, 100), V.getColoredMaterial(0.7, 0.7, 0.7)).set([0, -10, 0]).setRotation(-Math.PI / 2, 0, 0);
	...
	entity.set(rigidBodyComponent).set(planeColliderComponent).addToWorld();
	rigidBodyComponent.initialize();
}

function createStaticBox(x, y, z, w, d, h) {
	var entity = world.createEntity(new Box(w, d, h), V.getColoredMaterial(), [x, y, z]).set(new RigidBodyComponent({ isKinematic: true })).set(new ColliderComponent({
		collider: new BoxCollider({
...
function createCompound(x, y, z) {
	...
	var subEntity1 = world.createEntity(new Box(h1.x * 2, h1.y * 2, h1.z * 2), V.getColoredMaterial(), new Vector3(0, 2, 0).scale(2), new ColliderComponent({ collider: new BoxCollider({ halfExtents: h1 }) }));
	var subEntity2 = world.createEntity(new Box(h2.x * 2, h2.y * 2, h2.z * 2), V.getColoredMaterial(), new Vector3(-1.5, 0, 0).scale(2), new ColliderComponent({ collider: new BoxCollider({ halfExtents: h2 }) }));
	var subEntity3 = world.createEntity(new Box(h3.x * 2, h3.y * 2, h3.z * 2), V.getColoredMaterial(), new Vector3(1, 0, 0).scale(2), new ColliderComponent({ collider: new BoxCollider({ halfExtents: h3 }) }));
	var subEntity4 = world.createEntity(new Box(h4.x * 2, h4.y * 2, h4.z * 2), V.getColoredMaterial(), new Vector3(1.5, -1, 0).scale(2), new ColliderComponent({ collider: new BoxCollider({ halfExtents: h4 }) }));
	var subEntity5 = world.createEntity(new Box(h5.x * 2, h5.y * 2, h5.z * 2), V.getColoredMaterial(), new Vector3(0, -2, 0).scale(2), new ColliderComponent({ collider: new BoxCollider({ halfExtents: h5 }) }));

	// Attach the children to the root
	compoundEntity.attachChild(subEntity1).attachChild(subEntity2).attachChild(subEntity3).attachChild(subEntity4).attachChild(subEntity5);
...
function createChain(x, y, z, numLinks, size) {
	...
	for (var i = 0; i < numLinks; i++) {
		...
		var e = world.createEntity(new Sphere(), V.getColoredMaterial(), [x, y - i * size, z]).set(rbComponent).set(new ColliderComponent({ collider: new SphereCollider() })).setScale(size, size, size).addToWorld();
...
function createHinge(x, y, z) {
	...
	for (var i = 0; i < 5; i++) {
		...
		var e = world.createEntity(new Box(a, a, b), V.getColoredMaterial(), [x, y - i * scale, z]).set(rbComponent).set(new ColliderComponent({ collider: new BoxCollider({ halfExtents: new Vector3(a * 0.5, a * 0.5, b * 0.5) }) })).addToWorld().setScale(scale, scale, scale);
...
function createKinematic() {
	...
	var entity = world.createEntity(new Box(halfExtents.x * 2, halfExtents.y * 2, halfExtents.z * 2), V.getColoredMaterial(), [0, 0, 5]).set(rbComponent).set(new ColliderComponent({
		collider: new BoxCollider({
