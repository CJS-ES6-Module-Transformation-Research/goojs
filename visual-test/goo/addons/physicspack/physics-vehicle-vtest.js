...
function createGround() {
	var entity = world.createEntity(new Quad(1000, 1000, 100, 100), V.getColoredMaterial(0.7, 0.7, 0.7)).set([0, -1, 0]).setRotation(-Math.PI / 2, 0, 0);
...
function createVehicle(x, y, z) {
	...
	var script = {
		update: function update(args, ctx) {
	...
	return world.createEntity(new Box(s, 0.5 * s, 2 * s), V.getColoredMaterial(), [x, y, z], script).set(rbComponent).set(new ColliderComponent({ collider: new BoxCollider({ halfExtents: new Vector3(0.5 * s, 0.25 * s, s) }) })).addToWorld();
}
