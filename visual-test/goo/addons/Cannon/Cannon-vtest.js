...
function createGround() {
	var groundEntity = world.createEntity(new Quad(1000, 1000, 100, 100), V.getColoredMaterial(0.7, 0.7, 0.7)).set([0, -10, 0]).setRotation(-Math.PI / 2, 0, 0);
	var rigidBodyComponent = new CannonRigidbodyComponent({
		mass: 0
	});
	var planeColliderComponent = new CannonPlaneColliderComponent();
	groundEntity.set(rigidBodyComponent).set(planeColliderComponent).addToWorld();
}

function createStaticBox(x, y, z, w, d, h) {
	return world.createEntity(new Box(w, d, h), V.getColoredMaterial(), [x, y, z]).set(new CannonRigidbodyComponent({ mass: 0 })).set(new CannonBoxColliderComponent({
...
function createChain(x, y, z, numLinks, linkDistance, radius) {
	...
	for (var i = 0; i < numLinks; i++) {
		...
		var e = world.createEntity(new Sphere(10, 10, radius), V.getColoredMaterial(), [x, y - i * radius * 2, z]).set(body).set(new CannonSphereColliderComponent({
			radius: radius
		})).addToWorld();
		if (lastBody) {
