...
function addPrimitives() {
	for (var i = 0; i < 20; i++) {
		var position = [V.rng.nextFloat() * 16 - 8, V.rng.nextFloat() * 16 + 8 + groundLevel, V.rng.nextFloat() * 16 - 8];
...
function createGround() {
	var groundEntity = world.createEntity(new Quad(1000, 1000, 100, 100), [0, groundLevel, 0], V.getColoredMaterial(0.7, 0.7, 0.7)).setRotation(-Math.PI / 2, 0, 0);
	var rigidBodyComponent = new CannonRigidbodyComponent({
		mass: 0
	});
		...
		if (descendant.hasTag('collider') && descendant.hasComponent('MeshDataComponent')) {
			...
			if (md instanceof Sphere) {
				...
				collider = new CannonBoxColliderComponent({
					halfExtents: new Vector3(md.xExtent * scale.x, md.yExtent * scale.y, md.zExtent * scale.z)
				});
