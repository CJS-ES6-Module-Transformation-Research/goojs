...
function addTerrain() {
	...
	var meshEntity = gooRunner.world.createEntity(meshData, material, [-(nCol - 1) / 2 * elementSize, 0, -(nLin - 1) / 2 * elementSize]).addToWorld();
	entity.attachChild(meshEntity);

	// Cannon terrain is facing in Z direction. Move it to center and rotate to compensate.
	colliderEntity = gooRunner.world.createEntity([(nCol - 1) / 2 * elementSize, 0, (nLin - 1) / 2 * elementSize]);
...
function init() {
	...
	addCar(boxEntity, [createSphere(3, 5, -3), createSphere(-3, 5, -3), createSphere(-3, 5, 3), createSphere(3, 5, 3)]);
	V.addLights();
