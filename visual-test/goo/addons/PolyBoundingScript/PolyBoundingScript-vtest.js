...
V.describe(['The polyBounding script is used do prevent the camera entity from going into the pillars', 'Control the camera entity using WASD and a mouse'].join('\n'));
...
function addBoxes(polyBoundingScript) {
	...
	for (var i = 0; i < nBoxes; i++) {
		for (var j = 0; j < nBoxes; j++) {
			...
			material.uniforms.color = [i / nBoxes, j / nBoxes, 0.2];
			...
			polyBoundingScript.addCollidable({
				poly: [x - boxDimp2 - marg, y - boxDimp2 - marg, x - boxDimp2 - marg, y + boxDimp2 + marg, x + boxDimp2 + marg, y + boxDimp2 + marg, x + boxDimp2 + marg, y - boxDimp2 - marg],
				bottom: -boxHeightp2,
				top: boxHeightp2
			});
		}
...
// Add camera
var cameraEntity = world.createEntity(new Camera(), [0, 0, 20]).lookAt(new Vector3(0, 0, 0)).addToWorld();
