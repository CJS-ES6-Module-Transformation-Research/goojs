...
function DollyZoomAction/*id, settings*/() {
	...
	this.from = new Vector3js();
	this.to = new Vector3js();
	this.completed = false;
}

DollyZoomAction.prototype = Object.create(Actionjs.prototype);
...
DollyZoomAction.prototype.enter = function (fsm) {
	...
	if (entity.cameraComponent && entity.cameraComponent.camera) {
		...
		this.fromDistance = new Vector3js(this.lookAt).distance(camera.translation);
		this.toDistance = this.fromDistance - this.forward;

		this.eyeTargetScale = Math.tan(camera.fov * (Math.PI / 180) / 2) * this.fromDistance;

		var initialTranslation = new Vector3js().copy(translation);
		var toVec = Vector3js.fromArray(this.lookAt)
...
DollyZoomAction.prototype.update = function (fsm) {
	...
	if (this.eyeTargetScale) {
		...
		var d = MathUtilsjs.lerp(fT, this.fromDistance, this.toDistance);
