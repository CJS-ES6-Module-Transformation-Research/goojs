...
import { DoubleQuad as DoubleQuadjs } from "../quadpack/DoubleQuad";
...
function QuadComponent(image, settings) {
	...
	ObjectUtilsjs.defaults(settings, defaults); //! AT: this will mutate settings which is BAD!!!
	 ...
	 * @private
	 */
	this.meshRendererComponent = new MeshRendererComponentjs();

	/** The material currently used by the component.
	 * @type {Material}
	 */
	this.material = new Materialjs(ShaderLibjs.uber, 'QuadComponent default material');

	/** The quad meshdata.
	 * @type {Quad}
	 * @private
	 */
	this.meshData = new DoubleQuadjs(settings.width, settings.height, settings.tileX, settings.tileY);

	/** Mesh data component that this component creates and adds to the entity.
	 * @type {MeshDataComponent}
	 * @private
	 */
	this.meshDataComponent = new MeshDataComponentjs(this.meshData);
	...
	if (image) {
		var texture = new Texturejs(image);
...
QuadComponent.prototype = Object.create(Componentjs.prototype);
