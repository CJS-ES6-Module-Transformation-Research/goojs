...
BinaryLerpSource.prototype = Object.create(Source_Sourcejs.prototype);
...
BinaryLerpSource.combineSourceData = function (sourceAData, sourceBData, blendWeight, store) {
	...
	for (var key in sourceAData) {
		...
		if (!isNaN(dataA)) {
			...
			continue;
		} else if (!(dataA instanceof TransformData_TransformDatajs)) {
...
BinaryLerpSource.blendFloatValues = function (rVal, key, blendWeight, dataA, dataB) {
	if (isNaN(dataB)) {
		...
		rVal[key] = MathUtils_MathUtilsjs.lerp(blendWeight, dataA[0], dataB[0]);
	}
};
