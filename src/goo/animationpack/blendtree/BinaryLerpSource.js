...
import { Source as Sourcejs } from "../../animationpack/blendtree/Source";
...
BinaryLerpSource.prototype = Object.create(Sourcejs.prototype);
...
BinaryLerpSource.combineSourceData = function (sourceAData, sourceBData, blendWeight, store) {
	...
	for (var key in sourceAData) {
		...
		if (!isNaN(dataA)) {
			...
			continue;
		} else if (!(dataA instanceof TransformDatajs)) {
...
BinaryLerpSource.blendFloatValues = function (rVal, key, blendWeight, dataA, dataB) {
	if (isNaN(dataB)) {
		...
		rVal[key] = MathUtilsjs.lerp(blendWeight, dataA[0], dataB[0]);
	}
};
