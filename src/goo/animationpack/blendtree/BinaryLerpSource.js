var mod_BinaryLerpSource = BinaryLerpSource;
...
BinaryLerpSource.prototype = Object.create(Source_Source.prototype);
...
BinaryLerpSource.combineSourceData = function (sourceAData, sourceBData, blendWeight, store) {
	...
	for (var key in sourceAData) {
		...
		if (!isNaN(dataA)) {
			...
			continue;
		} else if (!(dataA instanceof TransformData_TransformData)) {
...
BinaryLerpSource.blendFloatValues = function (rVal, key, blendWeight, dataA, dataB) {
	if (isNaN(dataB)) {
		...
		rVal[key] = MathUtils_MathUtils.lerp(blendWeight, dataA[0], dataB[0]);
	}
};
 ...
 * @extends Source
 */
export { mod_BinaryLerpSource as BinaryLerpSource };