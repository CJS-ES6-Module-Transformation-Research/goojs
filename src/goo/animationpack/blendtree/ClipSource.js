...
import { AnimationClipInstance as AnimationClipInstance_AnimationClipInstancejs } from "../../animationpack/clip/AnimationClipInstance";
...
function ClipSource(clip, filter, channelNames) {
	...
	this._clip = clip;
	this._clipInstance = new AnimationClipInstance_AnimationClipInstancejs();
	...
	if (instance._active) {
		...
		if (maxTime !== 0) {
			...
			if (clockTime > maxTime || clockTime < minTime) {
				clockTime = MathUtils_MathUtilsjs.clamp(clockTime, minTime, maxTime);
