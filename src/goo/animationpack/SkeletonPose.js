...
SkeletonPose.prototype.allocateTransforms = function () {
	...
	for (var i = 0; i < jointCount; i++) {
		this._localTransforms[i] = new Transform_Transformjs();
	}

	// init global transforms
	for (var i = 0; i < jointCount; i++) {
		this._globalTransforms[i] = new Transform_Transformjs();
	}

	// init palette
	for (var i = 0; i < jointCount; i++) {
		this._matrixPalette[i] = new Matrix4_Matrix4js();
	}
};
...
SkeletonPose.prototype.setToBindPose = function () {
	for (var i = 0; i < this._localTransforms.length; i++) {
		...
		if (parentIndex !== Joint_Jointjs.NO_PARENT) {
