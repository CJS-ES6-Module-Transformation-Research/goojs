...
SkeletonPose.prototype.allocateTransforms = function () {
	...
	for (var i = 0; i < jointCount; i++) {
		this._localTransforms[i] = new _Transform2.default();
	}

	// init global transforms
	for (var i = 0; i < jointCount; i++) {
		this._globalTransforms[i] = new _Transform2.default();
	}

	// init palette
	for (var i = 0; i < jointCount; i++) {
		this._matrixPalette[i] = new _Matrix2.default();
	}
};
...
SkeletonPose.prototype.setToBindPose = function () {
	for (var i = 0; i < this._localTransforms.length; i++) {
		...
		if (parentIndex !== _Joint2.default.NO_PARENT) {
