import { FadeTransitionState as FadeTransitionState_FadeTransitionStatejs } from "../../animationpack/state/FadeTransitionState";
import {     SyncFadeTransitionState as SyncFadeTransitionState_SyncFadeTransitionStatejs, } from "../../animationpack/state/SyncFadeTransitionState";
import { FrozenTransitionState as FrozenTransitionState_FrozenTransitionStatejs } from "../../animationpack/state/FrozenTransitionState";
...
AnimationLayer.prototype.setBlendWeight = function (weight) {
	if (this._layerBlender) {
		this._layerBlender._blendWeight = MathUtils_MathUtilsjs.clamp(weight, 0, 1);
	}
};
...
AnimationLayer.prototype.update = function (globalTime) {
	if (this._currentState) {
		this._currentState.update(typeof globalTime !== 'undefined' ? globalTime : World_Worldjs.time);
	}
};
...
AnimationLayer.prototype.transitionTo = function (state, globalTime, finishCallback) {
	globalTime = typeof globalTime !== 'undefined' ? globalTime : World_Worldjs.time;
	...
	if (cState instanceof SteadyState_SteadyStatejs && transition) {
...
AnimationLayer.prototype.setCurrentState = function (state, rewind, globalTime, finishCallback) {
	...
	if (state) {
		...
		state.onFinished = function () {
			...
			if (state instanceof SteadyState_SteadyStatejs && finishCallback instanceof Function) {
				finishCallback();
			}
			this.update();
		}.bind(this);
...
AnimationLayer.prototype.resetClips = function (globalTime) {
	if (this._currentState) {
		this._currentState.resetClips(typeof globalTime !== 'undefined' ? globalTime : World_Worldjs.time);
	}
};
...
AnimationLayer.prototype._getTransitionByType = function (type) {
	...
	switch (type) {
		case 'Fade':
			transition = new FadeTransitionState_FadeTransitionStatejs();
			break;
		case 'SyncFade':
			transition = new SyncFadeTransitionState_SyncFadeTransitionStatejs();
			break;
		case 'Frozen':
			transition = new FrozenTransitionState_FrozenTransitionStatejs();
			break;
		default:
