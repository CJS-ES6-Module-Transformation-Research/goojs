...
AnimationComponent.prototype = Object.create(Componentjs.prototype);
...
AnimationComponent.prototype.update = function (globalTime) {
	...
	globalTime = typeof globalTime !== 'undefined' ? globalTime : Worldjs.time;
...
AnimationComponent.prototype.pause = function () {
	if (!this.paused) {
		this.lastTimeOfPause = Worldjs.time;
		this.paused = true;
	}
};
...
AnimationComponent.prototype.resume = function () {
	if (this.paused || this.lastTimeOfPause === -1) {
		if (this.lastTimeOfPause === -1) {
			this.resetClips(Worldjs.time);
		} else {
			this.shiftClipTime(Worldjs.time - this.lastTimeOfPause);
		}
	}
	this.paused = false;
};
