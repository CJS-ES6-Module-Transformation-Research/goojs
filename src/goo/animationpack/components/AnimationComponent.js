...
import { AnimationLayer as AnimationLayer_AnimationLayerjs } from "../../animationpack/layer/AnimationLayer";
import { JointData as JointData_JointDatajs } from "../../animationpack/clip/JointData";
...
function AnimationComponent(pose) {
	...
	// Base layer
	var layer = new AnimationLayer_AnimationLayerjs(AnimationLayer_AnimationLayerjs.BASE_LAYER_NAME);
...
AnimationComponent.prototype.apply = function (transformComponent) {
	...
	for (var i = 0, l = keys.length; i < l; i++) {
		var key = keys[i];
		var value = data[key];
		if (value instanceof JointData_JointDatajs) {
			if (pose && value._jointIndex >= 0) {
				value.applyTo(pose._localTransforms[value._jointIndex]);
			}
		} else if (value instanceof TransformData_TransformDatajs) {
...
AnimationComponent.prototype.pause = function () {
	if (!this.paused) {
		this.lastTimeOfPause = World_Worldjs.time;
		this.paused = true;
	}
};
...
AnimationComponent.prototype.resume = function () {
	if (this.paused || this.lastTimeOfPause === -1) {
		if (this.lastTimeOfPause === -1) {
			this.resetClips(World_Worldjs.time);
		} else {
			this.shiftClipTime(World_Worldjs.time - this.lastTimeOfPause);
		}
	}
	this.paused = false;
};
