...
import { SteadyState as SteadyState_SteadyStatejs } from "../../animationpack/state/SteadyState";
import { ClipSource as ClipSource_ClipSourcejs } from "../../animationpack/blendtree/ClipSource";
import { ManagedTransformSource as ManagedTransformSource_ManagedTransformSourcejs } from "../../animationpack/blendtree/ManagedTransformSource";
import { BinaryLerpSource as BinaryLerpSource_BinaryLerpSourcejs } from "../../animationpack/blendtree/BinaryLerpSource";
import { FrozenClipSource as FrozenClipSource_FrozenClipSourcejs } from "../../animationpack/blendtree/FrozenClipSource";
...
AnimationStateHandler.prototype = Object.create(ConfigHandler_ConfigHandlerjs.prototype);
AnimationStateHandler.prototype.constructor = AnimationStateHandler;
ConfigHandler_ConfigHandlerjs._registerClass('animstate', AnimationStateHandler);
...
AnimationStateHandler.prototype._create = function (ref) {
	var steadyState = new SteadyState_SteadyStatejs();
...
AnimationStateHandler.prototype._update = function (ref, config, options) {
	var that = this;
	return ConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options).then(function (state) {
		if (!state) { return; }
		state._name = config.name;
		state.id = config.id;
		state._transitions = ObjectUtils_ObjectUtilsjs.deepClone(config.transitions);
	...
	switch (cfg.type) {
		case 'Clip':
			return this.loadObject(cfg.clipRef, options).then(function (clip) {
				if (clipSource && (clipSource instanceof ClipSource_ClipSourcejs)) {
					clipSource._clip = clip;
					clipSource.setFilter(cfg.filter, cfg.channels);
				} else {
					clipSource = new ClipSource_ClipSourcejs(clip, cfg.filter, cfg.channels);
				}
		...
		case 'Managed':
			if (!clipSource || !(clipSource instanceof ManagedTransformSource_ManagedTransformSourcejs)) {
				clipSource = new ManagedTransformSource_ManagedTransformSourcejs();
			}
			if (cfg.clipRef) {
				return this.loadObject(cfg.clipRef, options).then(function (clip) {
				...
				return PromiseUtils_PromiseUtilsjs.resolve(clipSource);
			}
		case 'Lerp':
			...
			return rsvp_rsvpjsjs.all(promises).then(function (clipSources) {
				clipSource = new BinaryLerpSource_BinaryLerpSourcejs(clipSources[0], clipSources[1]);
				if (cfg.blendWeight) {
		...
		case 'Frozen':
			return this._parseClipSource(cfg.clipSource).then(function (subClipSource) {
				if (!clipSource || !(clipSource instanceof FrozenClipSource_FrozenClipSourcejs)) {
					clipSource = new FrozenClipSource_FrozenClipSourcejs(subClipSource, cfg.frozenTime || 0.0);
				} else {
