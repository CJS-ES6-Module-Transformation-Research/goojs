...
(0, _ConfigHandler._registerClass)('animstate', AnimationStateHandler);
...
AnimationStateHandler.prototype._update = function (ref, config, options) {
	...
	return _ConfigHandler.ConfigHandler.prototype._update.call(this, ref, config, options).then(function (state) {
		...
		state.id = config.id;
		state._transitions = (0, _ObjectUtils.deepClone)(config.transitions);
	...
	switch (cfg.type) {
		...
		case 'Managed':
			...
			if (cfg.clipRef) {
				...
				return (0, _PromiseUtils.resolve)(clipSource);
			}
		case 'Lerp':
 ...
 * @private
 */
exports.AnimationStateHandler = exported_AnimationStateHandler;
