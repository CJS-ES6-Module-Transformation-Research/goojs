...
AnimationClipHandler.prototype = Object.create(ConfigHandler_ConfigHandlerjs.prototype);
AnimationClipHandler.prototype.constructor = AnimationClipHandler;
ConfigHandler_ConfigHandlerjs._registerClass('clip', AnimationClipHandler);
...
AnimationClipHandler.prototype._update = function (ref, config, options) {
	var that = this;
	return ConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options).then(function (clip) {
		if (!clip) { return clip; }
...
AnimationClipHandler.prototype._updateAnimationClip = function (clipConfig, bindata, clip) {
	...
	if (clipConfig.channels) {
		...
		for (var i = 0; i < keys.length; i++) {
			...
			// Time samples
			var times = ArrayUtils_ArrayUtilsjs.getTypedArray(bindata, channelConfig.times);
			...
			switch (type) {
				...
				case 'Transform':
					...
					rots = ArrayUtils_ArrayUtilsjs.getTypedArray(bindata, channelConfig.rotationSamples);
					trans = ArrayUtils_ArrayUtilsjs.getTypedArray(bindata, channelConfig.translationSamples);
					scales = ArrayUtils_ArrayUtilsjs.getTypedArray(bindata, channelConfig.scaleSamples);
					...
					if (type === 'Joint') {
						...
						channel = new TransformChannel_TransformChanneljs(
							channelConfig.name,
							times,
							rots,
							trans,
