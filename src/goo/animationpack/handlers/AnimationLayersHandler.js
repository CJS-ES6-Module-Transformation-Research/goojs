...
ConfigHandler_ConfigHandlerjs._registerClass('animation', AnimationLayersHandler);
...
AnimationLayersHandler.prototype._update = function (ref, config, options) {
	...
	return ConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options).then(function (object) {
		...
		var i = 0;
		ObjectUtils_ObjectUtilsjs.forEach(config.layers, function (layerCfg) {
...
AnimationLayersHandler.prototype._parseLayer = function (layerConfig, layer, options) {
	...
	if (!layer) {
		layer = new AnimationLayer_AnimationLayerjs(layerConfig.name);
	} else {
		layer._name = layerConfig.name;
	}

	layer.id = layerConfig.id;
	layer._transitions = ObjectUtils_ObjectUtilsjs.deepClone(layerConfig.transitions);
	...
	var promises = [];
	ObjectUtils_ObjectUtilsjs.forEach(layerConfig.states, function (stateCfg) {
