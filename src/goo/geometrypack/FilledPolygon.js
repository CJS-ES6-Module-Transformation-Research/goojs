...
function FilledPolygon(verts, indices) {
	...
	_MeshData2.default.call(this, attributeMap, this.verts.length / 3, this.indices.length);

	this.rebuild();
}

FilledPolygon.prototype = Object.create(_MeshData2.default.prototype);
...
FilledPolygon.prototype.rebuild = function () {
	...
	for (var i = 0; i < this.indices.length; i += 3) {
		var normal = _MathUtils2.default.getTriangleNormal(this.verts[this.indices[i + 0] * 3 + 0], this.verts[this.indices[i + 0] * 3 + 1], this.verts[this.indices[i + 0] * 3 + 2], this.verts[this.indices[i + 1] * 3 + 0], this.verts[this.indices[i + 1] * 3 + 1], this.verts[this.indices[i + 1] * 3 + 2], this.verts[this.indices[i + 2] * 3 + 0], this.verts[this.indices[i + 2] * 3 + 1], this.verts[this.indices[i + 2] * 3 + 2]);
