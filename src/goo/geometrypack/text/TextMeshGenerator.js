...
function getBoundingVolume(polygon) {
	var min = new Vector2js(polygon[0].x, polygon[0].y);
	var max = min.clone();
...
function mergeBoxes(a, b) {
	return {
		min: new Vector2js(Math.min(a.min.x, b.min.x), Math.min(a.min.y, b.min.y)),
		max: new Vector2js(Math.max(a.max.x, b.max.x), Math.max(a.max.y, b.max.y))
	};
}
	...
	function meshForGlyph(data, x, y, options) {
		...
		if (options.extrusion) {
			data.extrusions.forEach(function (polygon) {
				...
				var contourPolyLine = new PolyLinejs(contourVerts, true);
				var extrusionPolyLine = new PolyLinejs([0, 0, -options.extrusion / 2, 0, 0, options.extrusion / 2]);
