...
function SoundSystem() {
	this._isSupported = _AudioContext.anonymus.isSupported();
  ...
  */
	...
	var that = this;
	_SystemBus.anonymus.addListener('goo.setCurrentCamera', function (camConfig) {
...
SoundSystem.prototype._initializeAudioNodes = function () {
	...
	this._outNode.connect(_AudioContext.anonymus.getContext().destination);

	this._wetNode = _AudioContext.anonymus.getContext().createGain();
	this._wetNode.connect(this._outNode);
	this._wetNode.gain.value = 0.2;

	this._convolver = _AudioContext.anonymus.getContext().createConvolver();
	this._convolver.connect(this._wetNode);

	this._listener = _AudioContext.anonymus.getContext().listener;
