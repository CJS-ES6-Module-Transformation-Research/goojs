...
function PipRenderSystem(renderSystem) {
	...
	this.outPass = new _FullscreenPass.FullscreenPass(_ShaderLib.copy);
	var that = this;
	this.outPass.render = function (renderer, writeBuffer, readBuffer) {
		this.material.setTexture('DIFFUSE_MAP', readBuffer);
		renderer.render(this.renderable, _FullscreenUtils.camera, [], that.target, true);
	};
...
PipRenderSystem.prototype.updateQuad = function (quad, x, y, width, height) {
	quad.getAttributeBuffer(MeshDatajs_POSITION).set([x, y, 0, x, y + height, 0, x + width, y + height, 0, x + width, y, 0]);
...
PipRenderSystem.prototype.render = function (renderer) {
	...
	renderer.render(this.renderableQuad, _FullscreenUtils.camera, [], null, false);
};
