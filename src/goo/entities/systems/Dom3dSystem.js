...
var Dom3dSystem_Dom3dSystem = Dom3dSystem;
...
function Dom3dSystem(renderer) {
	...
	var frontMaterial = new _Material.Material(_ShaderLib.ShaderLib.simple);
	...
	var backMaterial = new _Material.Material(_ShaderLib.ShaderLib.uber);
...
Dom3dSystem.prototype.init = function () {
	...
	var handlePick = function handlePick(event) {
		...
		if (intersects && !doesIntersect) {
			_SystemBus.SystemBusjs.emit('goo.dom3d.enabled', true);
			that.renderer.domElement.style.pointerEvents = 'none';
			doesIntersect = true;
		} else if (!intersects && doesIntersect) {
			_SystemBus.SystemBusjs.emit('goo.dom3d.enabled', false);
...
Dom3dSystem.prototype.onPreRender = function () {
	...
	var fov = 0.5 / Math.tan(_MathUtils.MathUtils.DEG_TO_RAD * camera.fov * 0.5) * height;
 ...
 */
exports.Dom3dSystem = Dom3dSystem_Dom3dSystem;