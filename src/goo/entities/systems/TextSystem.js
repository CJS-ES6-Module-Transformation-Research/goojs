...
import { TextureGrid as TextureGrid_TextureGrid } from "../../shapes/TextureGrid";
	...
	for (var i = 0; i < entities.length; i++) {
		...
		if (textComponent.dirty) {
			if (entity.hasComponent('MeshDataComponent')) {
				entity.getComponent('MeshDataComponent').meshData = TextureGrid_TextureGrid.fromString(textComponent.text);
			}
			else {
				var meshData = TextureGrid_TextureGrid.fromString(textComponent.text);
