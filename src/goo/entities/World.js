...
import { Manager as Manager_Managerjs } from "./managers/Manager";
import { System as System_Systemjs } from "./systems/System";
import { Component as Component_Componentjs } from "./components/Component";
...
Object.defineProperties(World, {
	time: {
		get: ObjectUtils_ObjectUtilsjs.warnOnce('World.time is deprecated, use world.time instead.', function () {
	...
	tpf: {
		get: ObjectUtils_ObjectUtilsjs.warnOnce('World.tpf is deprecated, use world.tpf instead.', function () {
...
World.prototype.add = function () {
	for (var i = 0; i < arguments.length; i++) {
		...
		} else if (argument instanceof Manager_Managerjs) {
			this.setManager(argument);
		} else if (argument instanceof System_Systemjs) {
			this.setSystem(argument);
		} else if (argument instanceof Component_Componentjs) {
...
World.prototype.registerComponent = function (componentConstructor) {
	if (this._components.indexOf(componentConstructor) === -1) {
		...
		Component_Componentjs.applyEntitySelectionAPI(componentConstructor.entitySelectionAPI, componentConstructor.type);
	}
	return this;
};
