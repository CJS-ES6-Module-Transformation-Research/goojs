...
function cloneEntity(world, entity, settings) {
	...
	newEntity._tags = ObjectUtils_ObjectUtilsjs.cloneSet(entity._tags);
	newEntity._attributes = ObjectUtils_ObjectUtilsjs.cloneMap(entity._attributes);
		...
		if (component.type === 'TransformComponent') {
			...
			for (var j = 0; j < component.scripts.length; j++) {
				...
				if (key && Scripts.getScript(key)) { // Engine script
					...
					if (script.parameters) { newScript.parameters = ObjectUtils_ObjectUtilsjs.deepClone(script.parameters); }
 ...
 */
EntityUtils.updateWorldTransform = ObjectUtils_ObjectUtilsjs.warnOnce('EntityUtils.updateWorldTransform is deprecated. Please use entity.transformComponent.sync instead', function (transformComponent) {
...
EntityUtils.getTotalBoundingBox = function (entity) {
	var mergedWorldBound = new BoundingBox_BoundingBoxjs();
	var first = true;
	...
	if (first) {
		...
		mergedWorldBound = new BoundingBox_BoundingBoxjs(translation.clone(), 0.001, 0.001, 0.001);
	}

	return mergedWorldBound;
};

var exported_EntityUtils = EntityUtils;

/**
 * Utilities for entity creation etc
 * Only used to define the class. Should never be instantiated.
 */
export { exported_EntityUtils as EntityUtils };
