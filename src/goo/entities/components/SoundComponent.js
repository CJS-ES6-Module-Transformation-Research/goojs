...
function SoundComponent() {
  ...
  */
	...
	this._isPanned = true;
	this._outDryNode = _AudioContext.anonymus.getContext().createGain();
	this._outWetNode = _AudioContext.anonymus.getContext().createGain();
	this.connectTo();
	this._pannerNode = _AudioContext.anonymus.getContext().createPanner();
	this._pannerNode.connect(this._outDryNode);
	this._inNode = _AudioContext.anonymus.getContext().createGain();
	this._inNode.connect(this._pannerNode);

	// The 2D sounds are always in camera space
	// Do we need another outDryNode for 2D?
	this._inNode2d = _AudioContext.anonymus.getContext().createGain();
