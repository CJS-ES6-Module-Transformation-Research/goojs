...
function SoundComponent() {
	...
	this._oldPosition = new Vector3_Vector3js();
	this._position = new Vector3_Vector3js();
	this._orientation = new Vector3_Vector3js();
...
SoundComponent.prototype.updateConfig = function (config) {
	if (config.volume !== undefined) {
		this._outDryNode.gain.value = MathUtils_MathUtilsjs.clamp(config.volume, 0, 1);
	}
	if (config.reverb !== undefined) {
		this._outWetNode.gain.value = MathUtils_MathUtilsjs.clamp(config.reverb, 0, 1);
	}
};
