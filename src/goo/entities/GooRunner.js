import { World as Worldjs } from "./World";
...
function GooRunner(parameters) {
	...
	GameUtilsjs.addVisibilityChangeListener(function (paused) {
		if (paused) {
			this._stopGameLoop();
		} else {
...
GooRunner.prototype._buildLogo = function (settings) {
	...
	var color = settings && settings.color ? settings.color : Logojs.white;

	var svg = Logojs.getLogo({
		width: '70px',
		height: '50px',
		color: color
	});
...
GooRunner.prototype.clear = function () {
	...
	GameUtilsjs.clearVisibilityChangeListeners();
