...
import { TransformSystem as TransformSystem_TransformSystemjs } from "./systems/TransformSystem";
import { RenderSystem as RenderSystem_RenderSystemjs } from "./systems/RenderSystem";
import { BoundingUpdateSystem as BoundingUpdateSystem_BoundingUpdateSystemjs } from "./systems/BoundingUpdateSystem";
import { ScriptSystem as ScriptSystem_ScriptSystemjs } from "./systems/ScriptSystem";
import { LightingSystem as LightingSystem_LightingSystemjs } from "./systems/LightingSystem";
import { CameraSystem as CameraSystem_CameraSystemjs } from "./systems/CameraSystem";
import { ParticlesSystem as ParticlesSystem_ParticlesSystemjs } from "./systems/ParticlesSystem";
...
import { SoundSystem as SoundSystem_SoundSystemjs } from "./systems/SoundSystem";
import { TransformComponent as TransformComponent_TransformComponentjs } from "./components/TransformComponent";
import { MeshDataComponent as MeshDataComponent_MeshDataComponentjs } from "./components/MeshDataComponent";
import { MeshRendererComponent as MeshRendererComponent_MeshRendererComponentjs } from "./components/MeshRendererComponent";
import { CameraComponent as CameraComponent_CameraComponentjs } from "./components/CameraComponent";
import { LightComponent as LightComponent_LightComponentjs } from "./components/LightComponent";
import { ScriptComponent as ScriptComponent_ScriptComponentjs } from "./components/ScriptComponent";
import { GameUtils as GameUtils_GameUtilsjs } from "../util/GameUtils";
import { Logo as Logo_Logojs } from "../util/Logo";
import { Material as Material_Materialjs } from "../renderer/Material";
function GooRunner(parameters) {
	parameters = parameters || {};

	GameUtils_GameUtilsjs.initAllShims();
	 ...
	 * @type {Renderer}
	 */
	this.renderer = new Renderer_Rendererjs(parameters);
	...
	GameUtils_GameUtilsjs.addVisibilityChangeListener(function (paused) {
		if (paused) {
			this._stopGameLoop();
		} else {
...
GooRunner.prototype._setBaseSystems = function () {
	this.world.setSystem(new ScriptSystem_ScriptSystemjs(this.world));
	this.world.setSystem(new TransformSystem_TransformSystemjs());
	this.world.setSystem(new CameraSystem_CameraSystemjs());
	this.world.setSystem(new ParticlesSystem_ParticlesSystemjs());
	this.world.setSystem(new BoundingUpdateSystem_BoundingUpdateSystemjs());
	this.world.setSystem(new LightingSystem_LightingSystemjs());

	if (AudioContext_AudioContextjsjs.isSupported()) {
		this.world.setSystem(new SoundSystem_SoundSystemjs());
	}

	this.renderSystem = new RenderSystem_RenderSystemjs();
...
GooRunner.prototype._registerBaseComponents = function () {
	this.world.registerComponent(TransformComponent_TransformComponentjs);
	this.world.registerComponent(MeshDataComponent_MeshDataComponentjs);
	this.world.registerComponent(MeshRendererComponent_MeshRendererComponentjs);
	this.world.registerComponent(CameraComponent_CameraComponentjs);
	this.world.registerComponent(LightComponent_LightComponentjs);
	this.world.registerComponent(ScriptComponent_ScriptComponentjs);
};
...
GooRunner.prototype._updateFrame = function (time) {
	...
	if (this.doRender) {
		this.renderer.checkResize(Renderer_Rendererjs.mainCamera);
		...
		if (this._picking.doPick && Renderer_Rendererjs.mainCamera) {
			...
			this.renderer.pick(this._picking.x, this._picking.y, this._picking.pickingStore, Renderer_Rendererjs.mainCamera);
			if (this.useTryCatch) {
...
GooRunner.prototype._buildLogo = function (settings) {
	...
	var color = settings && settings.color ? settings.color : Logo_Logojs.white;

	var svg = Logo_Logojs.getLogo({
		width: '70px',
		height: '50px',
		color: color
	});
	...
	document.addEventListener('keydown', function (event) {
		...
		switch (event.which) {
			case 32: // Space
				...
				break;
			case 13: // Enter
				GameUtils_GameUtilsjs.togglePointerLock();
				break;
			case 49: // 1
...
GooRunner.prototype._enableEvent = function (type) {
	...
	var func = function (e) {
		...
		this.pick(x, y, function (index, depth) {
			...
			var intersection = Renderer_Rendererjs.mainCamera.getWorldPosition(x * dpx, y * dpx, this.renderer.viewportWidth, this.renderer.viewportHeight, depth);
			this._dispatchEvent({
				entity: entity,
				depth: depth,
				x: x,
...
GooRunner.prototype.pickSync = function (x, y, skipUpdateBuffer) {
	...
	var pickingStore = {};
	this.renderer.pick(x, y, pickingStore, Renderer_Rendererjs.mainCamera);
	return pickingStore;
};
...
GooRunner.prototype.clear = function () {
	...
	Material_Materialjs.store = [];
	Material_Materialjs.hash = [];

	// this should never have existed in the first place
	Renderer_Rendererjs.mainCamera = null;

	// clears out whatever visibility-change listeners were attached to document
	GameUtils_GameUtilsjs.clearVisibilityChangeListeners();
