...
exports.default = DebugRenderSystem;
...
function DebugRenderSystem() {
	_System2.default.call(this, 'DebugRenderSystem', ['TransformComponent']);
	...
	this.selectionRenderable = _DebugDrawHelper2.default.getRenderablesFor({ type: 'MeshRendererComponent' });
	this.selectionActive = false;
	this.oldSelectionActive = false;
}

DebugRenderSystem.prototype = Object.create(_System2.default.prototype);
DebugRenderSystem.prototype.constructor = DebugRenderSystem;

DebugRenderSystem.prototype.setup = function () {
	_SystemBus2.default.addListener('goo.setCurrentCamera', this.cameraListener);
	_SystemBus2.default.addListener('goo.setLights', this.lightsListener);
};
	...
	for (var i = 0; i < entities.length; i++) {
		...
		for (var j = 0, max = this._interestComponents.length; j < max; j++) {
			...
			if (!entity._hidden && entity.hasComponent(componentName)) {
				...
				if (tree[componentName] && (tree[componentName].length === 2 && options.full || tree[componentName].length === 1 && !options.full)) {
					...
					renderables = _DebugDrawHelper2.default.getRenderablesFor(component, options);
				...
				_DebugDrawHelper2.default.update(renderables, component, this.camera, this.renderer);
				for (var k = 0; k < renderables.length; k++) {
		...
		if (this.doRender.SkeletonPose && entity.meshDataComponent && entity.meshDataComponent.currentPose) {
			...
			if (tree.skeleton) {
				...
				renderables = _DebugDrawHelper2.default.getRenderablesFor(pose);
				for (var k = 0; k < renderables.length; k++) {
...
DebugRenderSystem.prototype.cleanup = function () {
	_SystemBus2.default.removeListener('goo.setCurrentCamera', this.cameraListener);
	_SystemBus2.default.removeListener('goo.setLights', this.lightsListener);
};
