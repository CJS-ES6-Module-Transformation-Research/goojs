...
exports.getRenderablesFor = undefined;
...
DebugDrawHelper.update = function (renderables, component, camera, renderer) {
	...
	if (component.camera) {
		...
		if (component.camera.changedProperties) {
			if (renderables.length > 1 && (camera.far / camera.near !== renderables[1].farNear || camera.fov !== renderables[1].fov || camera.size !== renderables[1].size || camera.aspect !== renderables[1].aspect || camera.projectionMode !== renderables[1].projectionMode)) {
				renderables[1].meshData = (0, _CameraDebug.buildFrustum)(camera);
	...
	if (mainCamera) {
		...
		if (mainCamera.projectionMode === Camerajs_Parallel) {
...
var DebugDrawHelper_getRenderablesFor = function DebugDrawHelper_getRenderablesFor(component, options) {
	...
	if (component.type === "LightComponent") {
		...
		material = new _Material.Material(_ShaderLib.simpleColored, "DebugDrawLightMaterial");
	} else if (component.type === "CameraComponent") {
		meshes = cameraDebug.getMesh(component.camera, options);
		material = new _Material.Material(_ShaderLib.simpleLit, "DebugDrawCameraMaterial");
		...
		material = new _Material.Material(_ShaderLib.simpleColored, "DebugMeshRendererComponentMaterial");
	} else if (component instanceof _SkeletonPose.SkeletonPose) {
		meshes = skeletonDebug.getMesh(component, options);
		var materials = [new _Material.Material(_ShaderLib.uber, "SkeletonDebugMaterial"), new _Material.Material(_ShaderLib.uber, "SkeletonDebugMaterial")];
		var renderables = [];
