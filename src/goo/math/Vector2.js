...
Vector2.prototype.equals = function (rhs) {
	return Math.abs(this.x - rhs.x) <= _MathUtils.EPSILON && Math.abs(this.y - rhs.y) <= _MathUtils.EPSILON;
};
...
Vector2.prototype.equalsDirect = function (x, y) {
	return Math.abs(this.x - x) <= _MathUtils.EPSILON && Math.abs(this.y - y) <= _MathUtils.EPSILON;
};
