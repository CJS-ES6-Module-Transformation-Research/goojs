...
Matrix3._tempX = new Vector3_Vector3();
Matrix3._tempY = new Vector3_Vector3();
Matrix3._tempZ = new Vector3_Vector3();

Matrix3.prototype = Object.create(Matrix_Matrix.prototype);
Matrix3.prototype.constructor = Matrix3;

Matrix_Matrix.setupAliases(Matrix3.prototype, [['e00'], ['e10'], ['e20'], ['e01'], ['e11'], ['e21'], ['e02'], ['e12'], ['e22']]);
...
Matrix3.invert = function (source, target) {
	...
	if (Math.abs(det) < MathUtils_MathUtils.EPSILON) {
		return target;
	}
...
Matrix3.prototype.isOrthogonal = function () {
	...
	if (Math.abs(dot) > MathUtils_MathUtils.EPSILON) {
		return false;
	}
...
Matrix3.prototype.isNormal = function () {
	...
	if (Math.abs(l - 1.0) > MathUtils_MathUtils.EPSILON) {
		return false;
	}
...
Matrix3.prototype.toAngles = function (store) {
	var result = store;
	if (!result) {
		result = new Vector3_Vector3();
	}

	var d = this.data;
	if (d[3] > 1 - MathUtils_MathUtils.EPSILON) { // singularity at north pole
		result.y = Math.atan2(d[2], d[8]);
		result.z = Math.PI / 2;
		result.x = 0;
	} else if (d[3] < -1 + MathUtils_MathUtils.EPSILON) { // singularity at south pole
		result.y = Math.atan2(d[2], d[8]);
		result.z = -Math.PI / 2;
		result.x = 0;
	} else {
...
Matrix3.prototype.lookAt = function (direction, up) {
	...
	if (x.equals(Vector3_Vector3.ZERO)) {
		if (z.x !== 0.0) {
...
Matrix3.prototype.equals = function (rhs) {
	...
	return (Math.abs(thisData[0] - rhsData[0]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[1] - rhsData[1]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[2] - rhsData[2]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[3] - rhsData[3]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[4] - rhsData[4]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[5] - rhsData[5]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[6] - rhsData[6]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[7] - rhsData[7]) <= MathUtils_MathUtils.EPSILON) &&
		(Math.abs(thisData[8] - rhsData[8]) <= MathUtils_MathUtils.EPSILON);
};
 ...
 * @deprecated
 */
Matrix3.add = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.sub = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.mul = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.div = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.prototype.div = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.combine = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.prototype.combine = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.transpose = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.prototype.applyPost = ObjectUtils_ObjectUtils.warnOnce(
 ...
 * @deprecated
 */
Matrix3.prototype.applyPre = ObjectUtils_ObjectUtils.warnOnce(
...
// @ifdef DEBUG
Matrix_Matrix.addPostChecks(Matrix3.prototype, [
