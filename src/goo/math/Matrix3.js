...
Matrix3.invert = function (source, target) {
	...
	if (Math.abs(det) < _MathUtils.EPSILON) {
		return target;
	}
...
Matrix3.prototype.toAngles = function (store) {
	...
	var d = this.data;
	if (d[3] > 1 - _MathUtils.EPSILON) {
		// singularity at north pole
		result.y = Math.atan2(d[2], d[8]);
		result.z = Math.PI / 2;
		result.x = 0;
	} else if (d[3] < -1 + _MathUtils.EPSILON) {
...
Matrix3.prototype.equals = function (rhs) {
	...
	return Math.abs(thisData[0] - rhsData[0]) <= _MathUtils.EPSILON && Math.abs(thisData[1] - rhsData[1]) <= _MathUtils.EPSILON && Math.abs(thisData[2] - rhsData[2]) <= _MathUtils.EPSILON && Math.abs(thisData[3] - rhsData[3]) <= _MathUtils.EPSILON && Math.abs(thisData[4] - rhsData[4]) <= _MathUtils.EPSILON && Math.abs(thisData[5] - rhsData[5]) <= _MathUtils.EPSILON && Math.abs(thisData[6] - rhsData[6]) <= _MathUtils.EPSILON && Math.abs(thisData[7] - rhsData[7]) <= _MathUtils.EPSILON && Math.abs(thisData[8] - rhsData[8]) <= _MathUtils.EPSILON;
};
