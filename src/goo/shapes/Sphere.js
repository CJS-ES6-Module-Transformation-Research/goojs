...
function Sphere(zSamples, radialSamples, radius, textureMode) {
	...
	rendererMeshData_MeshDatajs.call(this, attributeMap, verts, tris);

	this.rebuild();
}

Sphere.prototype = Object.create(rendererMeshData_MeshDatajs.prototype);
...
Sphere.prototype.rebuild = function () {
	...
	for (var iR = 0; iR < this.radialSamples; iR++) {
		var fAngle = mathMathUtils_MathUtilsjs.TWO_PI * fInvRS * iR;
	...
	var i = 0;
	var tempVa = new mathVector3_Vector3js();
	var tempVb = new mathVector3_Vector3js();
	var tempVc = new mathVector3_Vector3js();
	for (var iZ = zBegin; iZ < zEnd; iZ++) {
		var fAFraction = mathMathUtils_MathUtilsjs.HALF_PI * (-1.0 + fZFactor * iZ); // in (-pi / 2, pi / 2)
		...
		for (var iR = 0; iR < this.radialSamples; iR++) {
			...
			if (this.textureMode === Sphere.TextureModes.Linear) {
				...
				texs[i * 2 + 0] = fRadialFraction + uOffset;
				texs[i * 2 + 1] = (mathMathUtils_MathUtilsjs.HALF_PI + Math.asin(fZFraction)) / Math.PI;
			} else if (this.textureMode === Sphere.TextureModes.Polar) {
				var r = (mathMathUtils_MathUtilsjs.HALF_PI - Math.abs(fAFraction)) / Math.PI;
				var u = r * afCos[iR] + 0.5;
				var v = r * afSin[iR] + 0.5;
				texs[i * 2 + 0] = u;
				texs[i * 2 + 1] = v;
			} else if (this.textureMode === Sphere.TextureModes.Chromeball) {
				var r = Math.sin((mathMathUtils_MathUtilsjs.HALF_PI + fAFraction) / 2);
				r /= 2;
				var u = r * afCos[iR] + 0.5;
				var v = r * afSin[iR] + 0.5;
				texs[i * 2 + 0] = u;
	...
	if (this.textureMode === Sphere.TextureModes.Chromeball) {
		var epsilonAngle = mathMathUtils_MathUtilsjs.HALF_PI - 1e-3;
		...
		for (var iR = 0; iR < this.radialSamples; iR++) {
			...
			var r = Math.sin((mathMathUtils_MathUtilsjs.HALF_PI + epsilonAngle) / 2);
			r /= 2;
			var u = r * afCos[iR] + 0.5;
			var v = r * afSin[iR] + 0.5;
			texs[i * 2 + 0] = u;
...
Sphere.prototype.clone = function () {
	var options = utilObjectUtils_ObjectUtilsjs.shallowSelectiveClone(this, ['zSamples', 'radialSamples', 'radius', 'textureMode']);
 ...
 */
export { Sphere_Sphere as Sphere };