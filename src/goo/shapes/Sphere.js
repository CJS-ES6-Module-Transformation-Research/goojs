...
Sphere.prototype.rebuild = function () {
	...
	for (var iR = 0; iR < this.radialSamples; iR++) {
		var fAngle = MathUtils_MathUtils.TWO_PI * fInvRS * iR;
	...
	for (var iZ = zBegin; iZ < zEnd; iZ++) {
		var fAFraction = MathUtils_MathUtils.HALF_PI * (-1.0 + fZFactor * iZ); // in (-pi / 2, pi / 2)
		...
		for (var iR = 0; iR < this.radialSamples; iR++) {
			...
			if (this.textureMode === Sphere.TextureModes.Linear) {
				...
				texs[i * 2 + 0] = fRadialFraction + uOffset;
				texs[i * 2 + 1] = (MathUtils_MathUtils.HALF_PI + Math.asin(fZFraction)) / Math.PI;
			} else if (this.textureMode === Sphere.TextureModes.Polar) {
				var r = (MathUtils_MathUtils.HALF_PI - Math.abs(fAFraction)) / Math.PI;
				var u = r * afCos[iR] + 0.5;
				var v = r * afSin[iR] + 0.5;
				texs[i * 2 + 0] = u;
				texs[i * 2 + 1] = v;
			} else if (this.textureMode === Sphere.TextureModes.Chromeball) {
				var r = Math.sin((MathUtils_MathUtils.HALF_PI + fAFraction) / 2);
				r /= 2;
				var u = r * afCos[iR] + 0.5;
				var v = r * afSin[iR] + 0.5;
				texs[i * 2 + 0] = u;
	...
	if (this.textureMode === Sphere.TextureModes.Chromeball) {
		var epsilonAngle = MathUtils_MathUtils.HALF_PI - 1e-3;
		...
		for (var iR = 0; iR < this.radialSamples; iR++) {
			...
			var r = Math.sin((MathUtils_MathUtils.HALF_PI + epsilonAngle) / 2);
			r /= 2;
			var u = r * afCos[iR] + 0.5;
			var v = r * afSin[iR] + 0.5;
			texs[i * 2 + 0] = u;
