...
function ParticleSystemComponent(options) {
	...
	this.material = new Materialjs({
		defines: ObjectUtilsjs.clone(defines),
		processors: [
	...
	ObjectUtilsjs.extend(this.material.uniforms, {
		textureTileInfo: [1, 1, 1, 0],
		invWorldRotation: [1, 0, 0, 0, 1, 0, 0, 0, 1],
		worldRotation: [1, 0, 0, 0, 1, 0, 0, 0, 1],
		gravity: [0, 0, 0],
...
ParticleSystemComponent.prototype = Object.create(Componentjs.prototype);
...
ParticleSystemComponent.prototype._updateVertexData = function () {
	...
	var offset = meshData.getAttributeBuffer(MeshDatajs.TEXCOORD0);
	var pos = meshData.getAttributeBuffer(MeshDatajs.POSITION);
	var indices = meshData.getIndexBuffer();

	var mesh = this.mesh;
	var meshIndices = mesh.getIndexBuffer();
	var meshPos = mesh.getAttributeBuffer(MeshDatajs.POSITION);
	var meshUV = mesh.getAttributeBuffer(MeshDatajs.TEXCOORD0);
	var meshVertexCount = mesh.vertexCount;
	...
	meshData.setAttributeDataUpdated(MeshDatajs.TEXCOORD0);
	meshData.setAttributeDataUpdated(MeshDatajs.POSITION);
...
ParticleSystemComponent.prototype.attached = function (entity) {
	...
	var attributeMap = MeshDatajs.defaultMap([
		MeshDatajs.POSITION,
		MeshDatajs.TEXCOORD0
	]);
	attributeMap.TIME_INFO = MeshDatajs.createAttribute(4, 'Float');
	attributeMap.START_POS = MeshDatajs.createAttribute(4, 'Float');
	attributeMap.START_DIR = MeshDatajs.createAttribute(4, 'Float');
