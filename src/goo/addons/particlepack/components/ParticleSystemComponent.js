...
function ParticleSystemComponent(options) {
	...
	this.material = new _Material.Material({
		defines: _ObjectUtils.ObjectUtils.clone(defines),
	...
	_ObjectUtils.ObjectUtils.extend(this.material.uniforms, {
		textureTileInfo: [1, 1, 1, 0],
		invWorldRotation: [1, 0, 0, 0, 1, 0, 0, 0, 1],
		worldRotation: [1, 0, 0, 0, 1, 0, 0, 0, 1],
		gravity: [0, 0, 0],
...
ParticleSystemComponent.prototype._updateVertexData = function () {
	...
	var offset = meshData.getAttributeBuffer(_MeshData.MeshData.TEXCOORD0);
	var pos = meshData.getAttributeBuffer(_MeshData.MeshData.POSITION);
	var indices = meshData.getIndexBuffer();

	var mesh = this.mesh;
	var meshIndices = mesh.getIndexBuffer();
	var meshPos = mesh.getAttributeBuffer(_MeshData.MeshData.POSITION);
	var meshUV = mesh.getAttributeBuffer(_MeshData.MeshData.TEXCOORD0);
	var meshVertexCount = mesh.vertexCount;
	...
	meshData.setAttributeDataUpdated(_MeshData.MeshData.TEXCOORD0);
	meshData.setAttributeDataUpdated(_MeshData.MeshData.POSITION);
...
ParticleSystemComponent.prototype.attached = function (entity) {
	...
	attributeMap.TIME_INFO = _MeshData.MeshData.createAttribute(4, 'Float');
	attributeMap.START_POS = _MeshData.MeshData.createAttribute(4, 'Float');
	attributeMap.START_DIR = _MeshData.MeshData.createAttribute(4, 'Float');
 ...
 */
exports.ParticleSystemComponent = ParticleSystemComponent_ParticleSystemComponent;