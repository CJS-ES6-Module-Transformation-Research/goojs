...
function ParticleSystemComponent(options) {
	...
	this.material = new Material_Materialjs({
		defines: ObjectUtils_ObjectUtilsjs.clone(defines),
		processors: [
			ShaderBuilder_ShaderBuilderjs.uber.fog
		],
		attributes: {
	...
	ObjectUtils_ObjectUtilsjs.extend(this.material.uniforms, {
		textureTileInfo: [1, 1, 1, 0],
		invWorldRotation: [1, 0, 0, 0, 1, 0, 0, 0, 1],
		worldRotation: [1, 0, 0, 0, 1, 0, 0, 0, 1],
		gravity: [0, 0, 0],
	...
	this._localGravity = new Vector3_Vector3js();
	this._lastTime = this.time;
	this._worldToLocalRotation = new Matrix3_Matrix3js();
	this._localToWorldRotation = new Matrix3_Matrix3js();
	 ...
	 * @type {Vector3}
	 */
	this.gravity = options.gravity ? options.gravity.clone() : new Vector3_Vector3js();
	 ...
	 * @readonly
	 */
	this.boxExtents = options.boxExtents ? options.boxExtents.clone() : new Vector3_Vector3js(1, 1, 1);
	 ...
	 */
	this.startColorScale = options.startColorScale ? options.startColorScale.clone() : new Vector4_Vector4js(1,1,1,1);

	this.emissionRate = options.emissionRate ? options.emissionRate.clone() : new ConstantCurve_ConstantCurvejs({ value: 10 });
	...
	this.startSpeed = options.startSpeed ? options.startSpeed.clone() : new ConstantCurve_ConstantCurvejs({ value: 5 });
	...
	this.startLifetime = options.startLifetime ? options.startLifetime.clone() : new ConstantCurve_ConstantCurvejs({ value: 5 });
	...
	this.mesh = options.mesh ? options.mesh : new Quad_Quadjs();
...
ParticleSystemComponent.prototype._updateVertexData = function () {
	...
	var offset = meshData.getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD0);
	var pos = meshData.getAttributeBuffer(MeshData_MeshDatajs.POSITION);
	var indices = meshData.getIndexBuffer();

	var mesh = this.mesh;
	var meshIndices = mesh.getIndexBuffer();
	var meshPos = mesh.getAttributeBuffer(MeshData_MeshDatajs.POSITION);
	var meshUV = mesh.getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD0);
	var meshVertexCount = mesh.vertexCount;
	...
	meshData.setAttributeDataUpdated(MeshData_MeshDatajs.TEXCOORD0);
	meshData.setAttributeDataUpdated(MeshData_MeshDatajs.POSITION);
...
var tmpWorldPos = new Vector3_Vector3js();
...
ParticleSystemComponent.prototype._sortParticles = function () {
	...
	while (l--) {
		...
		particle.sortValue = -particle.getWorldPosition(tmpWorldPos).dot(Renderer_Rendererjs.mainCamera._direction);
	}
...
var tmpPos = new Vector3_Vector3js();
var tmpDir = new Vector3_Vector3js();
...
ParticleSystemComponent.prototype.attached = function (entity) {
	...
	var attributeMap = MeshData_MeshDatajs.defaultMap([
		MeshData_MeshDatajs.POSITION,
		MeshData_MeshDatajs.TEXCOORD0
	]);
	attributeMap.TIME_INFO = MeshData_MeshDatajs.createAttribute(4, 'Float');
	attributeMap.START_POS = MeshData_MeshDatajs.createAttribute(4, 'Float');
	attributeMap.START_DIR = MeshData_MeshDatajs.createAttribute(4, 'Float');

	var maxParticles = this.maxParticles;
	var meshData = new MeshData_MeshDatajs(attributeMap, maxParticles * this.mesh.vertexCount, maxParticles * this.mesh.indexCount);
	meshData.vertexData.setDataUsage('DynamicDraw');
	this.meshData = meshData;

	var meshRendererComponent = new MeshRendererComponent_MeshRendererComponentjs(this.material);
