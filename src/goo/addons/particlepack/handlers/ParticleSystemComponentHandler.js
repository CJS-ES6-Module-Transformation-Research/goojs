...
ComponentHandler_ComponentHandler._registerClass('particleSystem', ParticleSystemComponentHandler);
...
ParticleSystemComponentHandler.prototype.update = function (entity, config, options) {
	...
	return ComponentHandler_ComponentHandler.prototype.update.call(this, entity, config, options).then(function (component) {
		...
		component.coneAngle = config.coneAngle * MathUtils_MathUtils.DEG_TO_RAD;
		...
		component.startAngle = createCurve(config.startAngle, MathUtils_MathUtils.DEG_TO_RAD);
		component.rotationSpeedOverLifetime = createCurve(config.rotationSpeedOverLifetime, MathUtils_MathUtils.DEG_TO_RAD);
		...
		if (textureRef && config.texturePreset === 'Custom') {
			...
			cachedTextures.Flare = cachedTextures.Flare || ParticleSystemUtils_ParticleSystemUtils.createFlareTexture(32);
			component.texture = cachedTextures.Flare;
		} else if (config.texturePreset === 'Splash') {
			cachedTextures.Splash = cachedTextures.Splash || ParticleSystemUtils_ParticleSystemUtils.createSplashTexture(32);
			component.texture = cachedTextures.Splash;
		} else if (config.texturePreset === 'Plankton') {
			cachedTextures.Plankton = cachedTextures.Plankton || ParticleSystemUtils_ParticleSystemUtils.createPlanktonTexture(32);
			component.texture = cachedTextures.Plankton;
		} else if (config.texturePreset === 'Snowflake') {
			cachedTextures.Snowflake = cachedTextures.Snowflake || ParticleSystemUtils_ParticleSystemUtils.createSnowflakeTexture(32);
