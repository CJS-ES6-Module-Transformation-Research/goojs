...
import {     ParticleSystemComponent as ParticleSystemComponent_ParticleSystemComponentjs, } from "../../../addons/particlepack/components/ParticleSystemComponent";
import { LinearCurve as LinearCurve_LinearCurvejs } from "../../../addons/particlepack/curves/LinearCurve";
import { ConstantCurve as ConstantCurve_ConstantCurvejs } from "../../../addons/particlepack/curves/ConstantCurve";
import { PolyCurve as PolyCurve_PolyCurvejs } from "../../../addons/particlepack/curves/PolyCurve";
import { Vector3Curve as Vector3Curve_Vector3Curvejs } from "../../../addons/particlepack/curves/Vector3Curve";
import { Vector4Curve as Vector4Curve_Vector4Curvejs } from "../../../addons/particlepack/curves/Vector4Curve";
import { LerpCurve as LerpCurve_LerpCurvejs } from "../../../addons/particlepack/curves/LerpCurve";
...
ParticleSystemComponentHandler.prototype = Object.create(ComponentHandler_ComponentHandlerjs.prototype);
ParticleSystemComponentHandler.prototype.constructor = ParticleSystemComponentHandler;
ComponentHandler_ComponentHandlerjs._registerClass('particleSystem', ParticleSystemComponentHandler);
...
ParticleSystemComponentHandler.prototype._create = function () {
	return new ParticleSystemComponent_ParticleSystemComponentjs();
};
...
function createCurve(configs, multiplier) {
	...
	var curve = new PolyCurve_PolyCurvejs();

	for (var i = 0; i < configs.length; i++) {
		var config = configs[i];
		switch (config.type) {
		case 'linear':
			curve.addSegment(new LinearCurve_LinearCurvejs({
			...
			break;
		case 'constant':
			curve.addSegment(new ConstantCurve_ConstantCurvejs({
				timeOffset: config.offset,
				value: config.value * multiplier
			}));
			break;
		case 'lerp':
			curve.addSegment(new LerpCurve_LerpCurvejs({
				timeOffset: config.offset,
...
function createVec3Curve(vector) {
	return new Vector3Curve_Vector3Curvejs({
...
function createVec4Curve(vector) {
	return new Vector4Curve_Vector4Curvejs({
		x: createCurve(vector[0]),
		y: createCurve(vector[1]),
		z: createCurve(vector[2]),
...
ParticleSystemComponentHandler.prototype.update = function (entity, config, options) {
	...
	return ComponentHandler_ComponentHandlerjs.prototype.update.call(this, entity, config, options).then(function (component) {
		...
		component.setBoxExtents(new Vector3_Vector3js(config.boxExtents));
		component.coneRadius = config.coneRadius;
		component.coneAngle = config.coneAngle * MathUtils_MathUtilsjs.DEG_TO_RAD;
		...
		component.sortMode = {
			'none': ParticleSystemComponent_ParticleSystemComponentjs.SORT_NONE,
			'camera_distance': ParticleSystemComponent_ParticleSystemComponentjs.SORT_CAMERA_DISTANCE
		}[config.sortMode];
		component.billboard = config.billboard;
		component.sizeOverLifetime = createCurve(config.sizeOverLifetime);
		component.startAngle = createCurve(config.startAngle, MathUtils_MathUtilsjs.DEG_TO_RAD);
		component.rotationSpeedOverLifetime = createCurve(config.rotationSpeedOverLifetime, MathUtils_MathUtilsjs.DEG_TO_RAD);
		...
		if (textureRef && config.texturePreset === 'Custom') {
			...
			cachedTextures.Flare = cachedTextures.Flare || ParticleSystemUtils_ParticleSystemUtilsjs.createFlareTexture(32);
			component.texture = cachedTextures.Flare;
		} else if (config.texturePreset === 'Splash') {
			cachedTextures.Splash = cachedTextures.Splash || ParticleSystemUtils_ParticleSystemUtilsjs.createSplashTexture(32);
			component.texture = cachedTextures.Splash;
		} else if (config.texturePreset === 'Plankton') {
			cachedTextures.Plankton = cachedTextures.Plankton || ParticleSystemUtils_ParticleSystemUtilsjs.createPlanktonTexture(32);
			component.texture = cachedTextures.Plankton;
		} else if (config.texturePreset === 'Snowflake') {
			cachedTextures.Snowflake = cachedTextures.Snowflake || ParticleSystemUtils_ParticleSystemUtilsjs.createSnowflakeTexture(32);
