...
function ProjectedGrid(densityX, densityY) {
	...
	this.projectorCamera = new Camera_Camerajs(45, 1, 0.1, 2000);
	this.mainCamera = new Camera_Camerajs(45, 1, 0.1, 2000);

	this.freezeProjector = false;
	this.upperBound = 20.0;

	this.origin = new Vector4_Vector4js();
	this.direction = new Vector4_Vector4js();
	this.source = new Vector2_Vector2js();
	this.rangeMatrix = new Matrix4_Matrix4js();

	this.intersectBottomLeft = new Vector4_Vector4js();
	this.intersectTopLeft = new Vector4_Vector4js();
	this.intersectTopRight = new Vector4_Vector4js();
	this.intersectBottomRight = new Vector4_Vector4js();

	this.planeIntersection = new Vector3_Vector3js();
	...
	this.intersections = [];
	for (var i = 0; i < 24; i++) {
		this.intersections.push(new Vector3_Vector3js());
	}
	...
	MeshData_MeshDatajs.call(this, attributeMap, vertexCount, indexCount);

	this.rebuild();
}

ProjectedGrid.prototype = Object.create(MeshData_MeshDatajs.prototype);
...
ProjectedGrid.prototype.update = function (camera) {
	...
	var tmpStorage = new Vector3_Vector3js();
	for (var i = 0; i < 8; i++) {
	...
	} else if (length < MathUtils_MathUtilsjs.EPSILON) {
	...
	projectorCamera.lookAt(planeIntersection, Vector3_Vector3js.UNIT_Y);

	// transform points to projector space
	var modelViewProjectionMatrix = projectorCamera.getViewProjectionMatrix();
	var spaceTransformation = new Vector4_Vector4js();
...
ProjectedGrid.prototype.calculateIntersection = function (planeHeight, screenPosition, modelViewProjectionInverseMatrix) {
	...
	if (Math.abs(this.direction.y) > MathUtils_MathUtilsjs.EPSILON) {
