...
function ProjectedGrid(densityX, densityY) {
	...
	this.projectorCamera = new _Camera2.default(45, 1, 0.1, 2000);
	this.mainCamera = new _Camera2.default(45, 1, 0.1, 2000);

	this.freezeProjector = false;
	this.upperBound = 20.0;

	this.origin = new _Vector6.default();
	this.direction = new _Vector6.default();
	this.source = new _Vector2.default();
	this.rangeMatrix = new _Matrix2.default();

	this.intersectBottomLeft = new _Vector6.default();
	this.intersectTopLeft = new _Vector6.default();
	this.intersectTopRight = new _Vector6.default();
	this.intersectBottomRight = new _Vector6.default();

	this.planeIntersection = new _Vector4.default();
	...
	this.intersections = [];
	for (var i = 0; i < 24; i++) {
		this.intersections.push(new _Vector4.default());
	}
	...
	_MeshData2.default.call(this, attributeMap, vertexCount, indexCount);

	this.rebuild();
}

ProjectedGrid.prototype = Object.create(_MeshData2.default.prototype);
...
ProjectedGrid.prototype.update = function (camera) {
	...
	var tmpStorage = new _Vector4.default();
	for (var i = 0; i < 8; i++) {
	...
	if (projectorCamera.translation.y > 0.0 && projectorCamera._direction.y > 0.0 || projectorCamera.translation.y < 0.0 && projectorCamera._direction.y < 0.0) {
		...
		var tmpVec = new _Vector4.default();
	...
	} else if (length < _MathUtils2.default.EPSILON) {
	...
	projectorCamera.lookAt(planeIntersection, _Vector4.default.UNIT_Y);

	// transform points to projector space
	var modelViewProjectionMatrix = projectorCamera.getViewProjectionMatrix();
	var spaceTransformation = new _Vector6.default();
...
ProjectedGrid.prototype.calculateIntersection = function (planeHeight, screenPosition, modelViewProjectionInverseMatrix) {
	...
	if (Math.abs(this.direction.y) > _MathUtils2.default.EPSILON) {
