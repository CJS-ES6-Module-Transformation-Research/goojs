...
var packDepthY = {
	...
	fshader: ['uniform float waterHeight;', 'uniform float waterDensity;', _ShaderFragment.methods.packDepth, 'varying vec4 worldPosition;', 'void main(void)', '{', 'float linearDepth = clamp(pow((waterHeight - worldPosition.y) * waterDensity, 0.25), 0.0, 0.999);', 'gl_FragColor = packDepth(linearDepth);', '}'].join('\n')
};
