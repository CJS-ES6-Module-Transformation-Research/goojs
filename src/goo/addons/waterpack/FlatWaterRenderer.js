...
exports.FlatWaterRenderer = undefined;
...
function FlatWaterRenderer(settings) {
	...
	var waterMaterial = new _Material.Material(waterShaderDef, 'WaterMaterial');
	...
	this.offset = new _Vector.Vector3();
	this.clipPlane = new _Vector2.Vector4();

	this.waterEntity = null;

	this.depthMaterial = new _Material.Material(packDepthY, 'depth');
}
...
FlatWaterRenderer.prototype.updateSize = function (renderer) {
	...
	this.reflectionTarget = new _RenderTarget.RenderTarget(width, height);
	...
	if (this.useRefraction) {
		...
		this.refractionTarget = new _RenderTarget.RenderTarget(width, height);
...
var packDepthY = {
	...
	vshader: ['attribute vec3 vertexPosition;', 'uniform mat4 viewMatrix;', 'uniform mat4 projectionMatrix;', 'uniform mat4 worldMatrix;', 'varying vec4 worldPosition;', _ShaderBuilder.ShaderBuilder.animation.prevertex, 'void main(void) {', 'mat4 wMatrix = worldMatrix;', _ShaderBuilder.ShaderBuilder.animation.vertex, 'worldPosition = wMatrix * vec4(vertexPosition, 1.0);', 'gl_Position = projectionMatrix * viewMatrix * worldPosition;', '}'].join('\n'),
	fshader: ['uniform float waterHeight;', 'uniform float waterDensity;', _ShaderFragment.ShaderFragment.methods.packDepth, 'varying vec4 worldPosition;', 'void main(void)', '{', 'float linearDepth = clamp(pow((waterHeight - worldPosition.y) * waterDensity, 0.25), 0.0, 0.999);', 'gl_FragColor = packDepth(linearDepth);', '}'].join('\n')
};
 ...
 */
exports.FlatWaterRenderer = FlatWaterRenderer_FlatWaterRenderer;