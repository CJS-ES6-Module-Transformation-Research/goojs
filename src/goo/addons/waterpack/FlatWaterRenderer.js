...
function FlatWaterRenderer(settings) {
	...
	var waterMaterial = new Material_Materialjs(waterShaderDef, 'WaterMaterial');
	...
	this.offset = new Vector3_Vector3js();
	this.clipPlane = new Vector4_Vector4js();

	this.waterEntity = null;

	this.depthMaterial = new Material_Materialjs(packDepthY, 'depth');
}
...
FlatWaterRenderer.prototype.updateSize = function (renderer) {
	...
	this.reflectionTarget = new RenderTarget_RenderTargetjs(width, height);
	...
	if (this.useRefraction) {
		...
		this.refractionTarget = new RenderTarget_RenderTargetjs(width, height);
