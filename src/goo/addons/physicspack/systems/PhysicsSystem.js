import { AbstractPhysicsSystem as AbstractPhysicsSystem_AbstractPhysicsSystemjs } from "../../../addons/physicspack/systems/AbstractPhysicsSystem";
import { RaycastResult as RaycastResult_RaycastResultjs } from "../../../addons/physicspack/RaycastResult";
import { RigidBodyComponent as RigidBodyComponent_RigidBodyComponentjs } from "../../../addons/physicspack/components/RigidBodyComponent";
...
var tmpCannonResult;
var tmpTransform = new Transform_Transformjs();
...
function PhysicsSystem(settings) {
	...
	this.setGravity(settings.gravity || new Vector3_Vector3js(0, -10, 0));

	this.initialized = false;

	AbstractPhysicsSystem_AbstractPhysicsSystemjs.call(this, 'PhysicsSystem', ['RigidBodyComponent']);
}

PhysicsSystem.prototype = Object.create(AbstractPhysicsSystem_AbstractPhysicsSystemjs.prototype);
...
PhysicsSystem.prototype.raycastAny = function (start, direction, maxDistance, options, result) {
	if (options instanceof RaycastResult_RaycastResultjs) {
		result = options;
		options = {};
	}
	options = options || {};
	result = result || new RaycastResult_RaycastResultjs();
...
var tmpResult = new RaycastResult_RaycastResultjs();
...
PhysicsSystem.prototype.syncTransforms = function (entities) {
	...
	while (queue.length) {
		...
		if (rigidBodyComponent.interpolation === RigidBodyComponent_RigidBodyComponentjs.INTERPOLATE) {
		...
		if (parent) {
			...
			Transform_Transformjs.combine(tmpTransform, transform, tmpTransform);
