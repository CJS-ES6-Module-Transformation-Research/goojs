...
ComponentHandler_ComponentHandlerjs._registerClass('rigidBody', RigidBodyComponentHandler);
...
RigidBodyComponentHandler.prototype._create = function () {
	return new RigidBodyComponent_RigidBodyComponentjs();
};
...
RigidBodyComponentHandler.prototype.update = function (entity, config, options) {
	return ComponentHandler_ComponentHandlerjs.prototype.update.call(this, entity, config, options).then(function (component) {
		...
		component.setVelocity(new Vector3_Vector3js(config.velocity));
		component.setAngularVelocity(new Vector3_Vector3js(config.angularVelocity));
		component.linearDamping = config.linearDrag;
		component.angularDamping = config.angularDrag;

		component.constraints = (
			(config.freezePositionX ? RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION_X : 0) |
			(config.freezePositionY ? RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION_Y : 0) |
			(config.freezePositionZ ? RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION_Z : 0) |
			(config.freezeRotationX ? RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION_X : 0) |
			(config.freezeRotationY ? RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION_Y : 0) |
			(config.freezeRotationZ ? RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION_Z : 0)
		);
