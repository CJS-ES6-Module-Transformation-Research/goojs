var CannonRigidbodyComponent_CannonRigidbodyComponent = CannonRigidbodyComponent;
...
function CannonRigidbodyComponent(settings) {
	...
	utilObjectUtils_ObjectUtilsjs.defaults(settings, {
		mass: 1,
		velocity: new mathVector3_Vector3js()
	...
	this._initialVelocity = new mathVector3_Vector3js();
	this._initialVelocity.set(settings.velocity);
	this.body = null;
	this.centerOfMassOffset = new mathVector3_Vector3js();
}

CannonRigidbodyComponent.prototype = Object.create(entitiescomponentsComponent_Componentjs.prototype);
...
CannonRigidbodyComponent.prototype.setPosition = function (pos) {
	if (this.body) {
		...
		this._initialPosition = new mathVector3_Vector3js(pos);
	}
};
...
CannonRigidbodyComponent.prototype.addShapesToBody = function (entity) {
	...
	if (!collider) {
		...
		var invBodyTransform = new mathTransform_Transformjs();
		invBodyTransform.copy(bodyTransform);
		invBodyTransform.invert(invBodyTransform);
		var gooTrans = new mathTransform_Transformjs();
		...
		entity.traverse(function (childEntity) {
			...
			if (collider) {
				...
				var gooTrans2 = new mathTransform_Transformjs();
				mathTransform_Transformjs.combine(invBodyTransform, gooTrans, gooTrans2);
				gooTrans2.update();
 ...
 */
export { CannonRigidbodyComponent_CannonRigidbodyComponent as CannonRigidbodyComponent };
