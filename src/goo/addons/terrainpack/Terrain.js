...
function Terrain(goo, size, count) {
	...
	var brush = new Quad_Quad(2 / size, 2 / size);

	var mat = this.drawMaterial1 = new Material_Material(brushShader);
	mat.blendState.blending = 'AdditiveBlending';
	mat.cullState.cullFace = 'Front';

	var mat2 = this.drawMaterial2 = new Material_Material(brushShader2);
	mat2.cullState.cullFace = 'Front';

	var mat3 = this.drawMaterial3 = new Material_Material(brushShader3);
	mat3.uniforms.size = 1 / size;
	mat3.cullState.cullFace = 'Front';

	var mat4 = this.drawMaterial4 = new Material_Material(brushShader4);
	...
	this.copyPass = new FullscreenPass_FullscreenPass(ShaderLib_ShaderLib.screenCopy);
	this.copyPass.material.depthState.enabled = false;

	this.upsamplePass = new FullscreenPass_FullscreenPass(upsampleShader);
	this.upsamplePass.material.depthState.enabled = false;

	this.normalmapPass = new FullscreenPass_FullscreenPass(normalmapShader);
	...
	this.extractFloatPass = new FullscreenPass_FullscreenPass(extractShader);
	// this.detailmapPass = new FullscreenPass(detailShader);

	this.normalMap = new RenderTarget_RenderTarget(size, size);
	// this.detailMap = new RenderTarget(size, size);

	this.textures = [];
	this.texturesBounce = [];
	for (var i = 0; i < count; i++) {
		this.textures[i] = new RenderTarget_RenderTarget(size, size, {
		...
		this.texturesBounce[i] = new RenderTarget_RenderTarget(size, size, {
			magFilter: 'NearestNeighbor',
			minFilter: 'NearestNeighborNoMipMaps',
			wrapS: 'EdgeClamp',
			wrapT: 'EdgeClamp',
	...
	this.splat = new RenderTarget_RenderTarget(this.size * this.splatMult, this.size * this.splatMult, {
			wrapS: 'EdgeClamp',
			wrapT: 'EdgeClamp',
			generateMipmaps: false
	});
	this.splatCopy = new RenderTarget_RenderTarget(this.size * this.splatMult, this.size * this.splatMult, {
			wrapS: 'EdgeClamp',
			wrapT: 'EdgeClamp',
			generateMipmaps: false
	});
...
Terrain.prototype.init = function (terrainTextures) {
	...
	for (var i = 0; i < count; i++) {
		...
		var material = new Material_Material(terrainShaderDefFloat, 'clipmap' + i);
		...
		var terrainPickingMaterial = new Material_Material(terrainPickingShader, 'terrainPickingMaterial' + i);
	...
	// edit marker
	var light = new DirectionalLight_DirectionalLight();
	...
	this.floatTexture = terrainTextures.heightMap instanceof Texture_Texture ? terrainTextures.heightMap : new Texture_Texture(terrainTextures.heightMap, {
		magFilter: 'NearestNeighbor',
		minFilter: 'NearestNeighborNoMipMaps',
		wrapS: 'EdgeClamp',
		wrapT: 'EdgeClamp',
	...
	this.splatTexture = terrainTextures.splatMap instanceof Texture_Texture ? terrainTextures.splatMap : new Texture_Texture(terrainTextures.splatMap, {
		magFilter: 'NearestNeighbor',
		minFilter: 'NearestNeighborNoMipMaps',
		wrapS: 'EdgeClamp',
		wrapT: 'EdgeClamp',
...
Terrain.prototype.pick = function (camera, x, y, store) {
	...
	this.renderer.renderToPick(entities, Renderer_Renderer.mainCamera, true, false, false, x, y, null, true);
	var pickStore = {};
	this.renderer.pick(x, y, pickStore, Renderer_Renderer.mainCamera);
...
Terrain.prototype.draw = function (mode, type, size, x, y, z, power, brushTexture, rgba) {
	power = MathUtils_MathUtils.clamp(power, 0, 1);
	...
	if (mode === 'paint') {
		...
		if (brushTexture) {
			this.renderable.materials[0].setTexture(Shader_Shader.DIFFUSE_MAP, brushTexture);
		} else {
			this.renderable.materials[0].setTexture(Shader_Shader.DIFFUSE_MAP, this.defaultBrushTexture);
		}
		...
		this.renderer.render(this.renderable, FullscreenUtils_FullscreenUtils.camera, [], this.splat, false);
	} else if (mode === 'smooth') {
		...
		this.renderer.render(this.renderable, FullscreenUtils_FullscreenUtils.camera, [], this.textures[0], false);
...
Terrain.prototype.update = function (pos) {
	...
	for (var i = 0; i < this.clipmaps.length; i++) {
		...
		if (clipmap.parentClipmap) {
			...
			var xxx = MathUtils_MathUtils.moduloPositive(xx + 1, 2);
			var zzz = MathUtils_MathUtils.moduloPositive(zz + 1, 2);
			var xmove = xxx % 2 === 0 ? -n : n + 1;
			var zmove = zzz % 2 === 0 ? -n : n + 1;
			interior1.setTranslation(-n, 0, zmove);
			zzz = MathUtils_MathUtils.moduloPositive(zz, 2);
			zmove = zzz % 2 === 0 ? -n : -n + 1;
...
Terrain.prototype.createGrid = function (w, h) {
	...
	var meshData = new MeshData_MeshData(attributeMap, (w + 1) * (h + 1), (w * 2 + 4) * h);
	this._gridCache[key] = meshData;

	meshData.indexModes = ['TriangleStrip'];

	var vertices = meshData.getAttributeBuffer(MeshData_MeshData.POSITION);
...
var terrainShaderDefFloat = {
	...
	processors: [
		ShaderBuilder_ShaderBuilder.light.processor,
		function (shader) {
			if (ShaderBuilder_ShaderBuilder.USE_FOG) {
				shader.setDefine('FOG', true);
				shader.uniforms.fogSettings = ShaderBuilder_ShaderBuilder.FOG_SETTINGS;
				shader.uniforms.fogColor = ShaderBuilder_ShaderBuilder.FOG_COLOR;
			} else {
	...
	uniforms: {
		...
		fogSettings: function () {
			return ShaderBuilder_ShaderBuilder.FOG_SETTINGS;
		},
		fogColor: function () {
			return ShaderBuilder_ShaderBuilder.FOG_COLOR;
		},
	...
	builder: function (shader, shaderInfo) {
		ShaderBuilder_ShaderBuilder.light.builder(shader, shaderInfo);
	},
	vshader: function () {
		return [
			...
			ShaderBuilder_ShaderBuilder.light.prevertex,
			...
			ShaderBuilder_ShaderBuilder.light.vertex,
			'}'
		].join('\n');
	},
	fshader: function () {
		return [
			...
			ShaderBuilder_ShaderBuilder.light.prefragment,
			...
			'void main(void) {',
				...
				'#else',
				ShaderBuilder_ShaderBuilder.light.fragment,
				'#endif',
...
var upsampleShader = {
	...
	uniforms: {
		diffuseMap: 'MAIN_MAP',
		childMap: Shader_Shader.DIFFUSE_MAP,
		res: [1, 1, 1, 1]
	},
	vshader: [
		'attribute vec3 vertexPosition;',
...
var normalmapShader = {
	...
	uniforms: {
		...
		projectionMatrix: Shader_Shader.PROJECTION_MATRIX,
		worldMatrix: Shader_Shader.WORLD_MATRIX,
		heightMap: Shader_Shader.DIFFUSE_MAP,
