...
Terrain.prototype.pick = function (camera, x, y, store) {
	...
	this.renderer.renderToPick(entities, _Renderer.mainCamera, true, false, false, x, y, null, true);
	var pickStore = {};
	this.renderer.pick(x, y, pickStore, _Renderer.mainCamera);
...
Terrain.prototype.draw = function (mode, type, size, x, y, z, power, brushTexture, rgba) {
	power = (0, _MathUtils.clamp)(power, 0, 1);
...
Terrain.prototype.update = function (pos) {
	...
	for (var i = 0; i < this.clipmaps.length; i++) {
		...
		if (clipmap.parentClipmap) {
			...
			var xxx = (0, _MathUtils.moduloPositive)(xx + 1, 2);
			var zzz = (0, _MathUtils.moduloPositive)(zz + 1, 2);
			var xmove = xxx % 2 === 0 ? -n : n + 1;
			var zmove = zzz % 2 === 0 ? -n : n + 1;
			interior1.setTranslation(-n, 0, zmove);
			zzz = (0, _MathUtils.moduloPositive)(zz, 2);
			zmove = zzz % 2 === 0 ? -n : -n + 1;
