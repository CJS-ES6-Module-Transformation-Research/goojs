...
function Terrain(goo, size, count) {
	...
	this.copyPass = new rendererpassFullscreenPass_FullscreenPassjs(renderershadersShaderLib_ShaderLibjs.screenCopy);
...
Terrain.prototype.pick = function (camera, x, y, store) {
	...
	this.renderer.renderToPick(entities, rendererRenderer_Rendererjs.mainCamera, true, false, false, x, y, null, true);
	var pickStore = {};
	this.renderer.pick(x, y, pickStore, rendererRenderer_Rendererjs.mainCamera);
...
Terrain.prototype.draw = function (mode, type, size, x, y, z, power, brushTexture, rgba) {
	power = mathMathUtils_MathUtilsjs.clamp(power, 0, 1);
	...
	if (mode === 'paint') {
		...
		this.renderer.render(this.renderable, rendererpassFullscreenUtils_FullscreenUtilsjs.camera, [], this.splat, false);
	} else if (mode === 'smooth') {
		...
		this.renderer.render(this.renderable, rendererpassFullscreenUtils_FullscreenUtilsjs.camera, [], this.textures[0], false);
...
Terrain.prototype.update = function (pos) {
	...
	for (var i = 0; i < this.clipmaps.length; i++) {
		...
		if (clipmap.parentClipmap) {
			...
			var xxx = mathMathUtils_MathUtilsjs.moduloPositive(xx + 1, 2);
			var zzz = mathMathUtils_MathUtilsjs.moduloPositive(zz + 1, 2);
			var xmove = xxx % 2 === 0 ? -n : n + 1;
			var zmove = zzz % 2 === 0 ? -n : n + 1;
			interior1.setTranslation(-n, 0, zmove);
			zzz = mathMathUtils_MathUtilsjs.moduloPositive(zz, 2);
			zmove = zzz % 2 === 0 ? -n : -n + 1;
