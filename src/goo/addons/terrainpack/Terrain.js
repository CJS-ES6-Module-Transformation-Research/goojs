...
function Terrain(goo, size, count) {
	...
	this.copyPass = new _FullscreenPass.FullscreenPass(_ShaderLib.screenCopy);
...
Terrain.prototype.draw = function (mode, type, size, x, y, z, power, brushTexture, rgba) {
	...
	if (mode === 'paint') {
		...
		this.renderer.render(this.renderable, _FullscreenUtils.camera, [], this.splat, false);
	} else if (mode === 'smooth') {
		...
		this.renderer.render(this.renderable, _FullscreenUtils.camera, [], this.textures[0], false);
...
var terrainPickingShader = {
	...
	fshader: ['uniform float id;', 'varying float depth;', _ShaderFragment.methods.packDepth16, 'void main() {', 'vec2 packedId = vec2(floor(id/255.0), mod(id, 255.0)) * vec2(1.0/255.0);', 'vec2 packedDepth = packDepth16(depth);', 'gl_FragColor = vec4(packedId, packedDepth);',
