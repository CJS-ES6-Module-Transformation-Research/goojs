...
function Forrest() {
	this.calcVec = new Vector3_Vector3js();
	this.initDone = false;
}
...
Forrest.prototype.loadLODTrees = function (world, terrainQuery, forrestAtlasTexture, forrestAtlasNormals, forrestTypes, entityMap) {
	...
	var material = new Material_Materialjs(vegetationShader, 'vegetation');
	...
	for (var x = 0; x < this.gridSize; x++) {
		...
		for (var z = 0; z < this.gridSize; z++) {
			...
			var meshDataComponent = new MeshDataComponent_MeshDataComponentjs(dummyMesh);
...
Forrest.prototype.update = function (x, z) {
	...
	for (var x = 0; x < this.gridSize; x++) {
		for (var z = 0; z < this.gridSize; z++) {
			...
			var modX = MathUtils_MathUtilsjs.moduloPositive(patchX, this.gridSize);
			var modZ = MathUtils_MathUtilsjs.moduloPositive(patchZ, this.gridSize);
...
Forrest.prototype.determineVegTypeAtPos = function (pos) {
	...
	if (norm === null) {
		norm = Vector3_Vector3js.UNIT_Y;
	}
	var slope = norm.dot(Vector3_Vector3js.UNIT_Y);
	return this.terrainQuery.getForrestType(pos[0], pos[2], slope, MathUtils_MathUtilsjs.fastRandom());
};

Forrest.prototype.fetchTreeMesh = function (vegetationType) {
    return EntityUtils_EntityUtilsjs.clone(this.world, this.entityMap[vegetationType]);
};
...
Forrest.prototype.getPointInPatch = function (x, z, patchX, patchZ, patchSpacing) {
	var pos = [0, 0, 0];
	pos[0] = patchX + (x + MathUtils_MathUtilsjs.fastRandom() * 0.75) * patchSpacing;
	pos[2] = 0.5 + patchZ + (z + MathUtils_MathUtilsjs.fastRandom() * 0.75) * patchSpacing;
...
Forrest.prototype.addVegMeshToPatch = function (vegetationType, pos, meshBuilder, levelOfDetail, gridEntity) {
	...
	var size = (MathUtils_MathUtilsjs.fastRandom() * 0.5 + 0.75);
...
Forrest.prototype.createForrestPatch = function (patchX, patchZ, levelOfDetail, gridEntity) {
	...
	MathUtils_MathUtilsjs.randomSeed = patchX * 10000 + patchZ;
...
Forrest.prototype.createBase = function (type) {
	var attributeMap = MeshData_MeshDatajs.defaultMap([MeshData_MeshDatajs.POSITION, MeshData_MeshDatajs.TEXCOORD0]);
	attributeMap.BASE = MeshData_MeshDatajs.createAttribute(1, 'Float');
	attributeMap.OFFSET = MeshData_MeshDatajs.createAttribute(2, 'Float');
	var meshData = new MeshData_MeshDatajs(attributeMap, 4, 6);

	meshData.getAttributeBuffer(MeshData_MeshDatajs.POSITION).set([
		0, -type.h * 0.1, 0,
		0, -type.h * 0.1, 0,
		0, -type.h * 0.1, 0,
		0, -type.h * 0.1, 0
	]);
	meshData.getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD0).set([
		type.tx, type.ty,
