	...
	return rsvp_rsvpjsjs.all(promises).then(function (textures) {
		...
		var terrainQuery = this.terrainQuery = {
			getHeightAt: function (pos) {
				...
				col = MathUtilsjs.moduloPositive(col, terrainSize);
				row = MathUtilsjs.moduloPositive(row, terrainSize);
				col1 = MathUtilsjs.moduloPositive(col1, terrainSize);
				row1 = MathUtilsjs.moduloPositive(row1, terrainSize);
				...
				return MathUtilsjs.lerp(intOnZ, MathUtilsjs.lerp(intOnX, topLeft, topRight),
					MathUtilsjs.lerp(intOnX, bottomLeft, bottomRight));
			}.bind(this),
			...
			getVegetationType: function (xx, zz, slope) {
				...
				if (MathUtilsjs.smoothstep(0.82, 0.91, slope) < rand) {
					return null;
				}
			...
			getForrestType: function (xx, zz, slope, rand) {
				if (MathUtilsjs.smoothstep(0.8, 0.88, slope) < rand) {
					return null;
				}
			...
			getLightAt: function (pos) {
				...
				col = MathUtilsjs.moduloPositive(col, this.lightMapSize);
				row = MathUtilsjs.moduloPositive(row, this.lightMapSize);
				col1 = MathUtilsjs.moduloPositive(col1, this.lightMapSize);
				row1 = MathUtilsjs.moduloPositive(row1, this.lightMapSize);
				...
				return MathUtilsjs.lerp(intOnZ, MathUtilsjs.lerp(intOnX, topLeft, topRight),
						MathUtilsjs.lerp(intOnX, bottomLeft, bottomRight)) / 255.0;
			}.bind(this),
