...
function BloomPass(settings) {
	...
	this.copyMaterial = new Materialjs(ShaderLibjs.copyPure);
	this.copyMaterial.uniforms.opacity = strength;
	this.copyMaterial.blendState.blending = 'AdditiveBlending';

	this.convolutionShader = ObjectUtilsjs.deepClone(ShaderLibjs.convolution);
	...
	this.bcMaterial = new Materialjs(ShaderLibExtrajs.brightnesscontrast);
...
BloomPass.prototype = Object.create(Passjs.prototype);
...
BloomPass.prototype.render = function (renderer, writeBuffer, readBuffer) {
	...
	renderer.render(this.renderable, FullscreenUtilsjs.camera, [], this.renderTargetY, true);
	...
	if (this.target !== null) {
		...
		renderer.render(this.renderable, FullscreenUtilsjs.camera, [], readBuffer, this.clear);
	}
};
