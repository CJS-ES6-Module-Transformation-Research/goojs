...
function DofPass(renderList, outShader) {
	...
	var size = (0, _MathUtils.nearestPowerOfTwo)(Math.max(width, height));
...
var packDepth = {
	...
	'uniform float farPlane;', _ShaderFragment.methods.packDepth, 'varying vec4 vPosition;', 'void main(void)', '{', '  float linearDepth = min(-vPosition.z, farPlane) / farPlane;', '  gl_FragColor = packDepth(linearDepth);', '}' //
	].join('\n')
};
