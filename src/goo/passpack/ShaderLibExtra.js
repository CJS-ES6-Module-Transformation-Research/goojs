...
exports.ShaderLibExtra = undefined;
...
var ShaderLibExtra_ShaderLibExtra = ShaderLibExtra;
...
ShaderLibExtra.bokehShader = {
		...
		fshader: ['varying vec2 vUv;', 'uniform sampler2D tColor;', 'uniform sampler2D tDepth;', 'uniform float maxblur;', // max blur amount
...
ShaderLibExtra.film = {
		attributes: _ShaderLib.ShaderLib.copy.attributes,
		uniforms: {
				tDiffuse: _Shader.Shader.DIFFUSE_MAP,
		...
		vshader: _ShaderLib.ShaderLib.copy.vshader,
...
ShaderLibExtra.ssao = {
		...
		vshader: ['attribute vec3 vertexPosition;', 'attribute vec2 vertexUV0;', 'uniform mat4 viewMatrix;', 'uniform mat4 projectionMatrix;', 'uniform mat4 worldMatrix;', 'varying vec2 vUv;', 'void main() {', 'vUv = vertexUV0;', 'gl_Position = projectionMatrix * viewMatrix * worldMatrix * vec4( vertexPosition, 1.0 );', '}'].join('\n'),
		fshader: ['uniform float cameraNear;', 'uniform float cameraFar;', 'uniform float fogNear;', 'uniform float fogFar;', 'uniform bool fogEnabled;', // attenuate AO with linear fog
		...
		'float cameraFarPlusNear = cameraFar + cameraNear;', 'float cameraFarMinusNear = cameraFar - cameraNear;', 'float cameraCoef = 2.0 * cameraNear;',
		...
		'if ( onlyAO ) {', 'final = onlyAOColor * vec3( mix( vec3( ao ), vec3( 1.0 ), luminance * lumInfluence ) );', // ambient occlusion only
...
ShaderLibExtra.levels = {
		...
		vshader: ['attribute vec3 vertexPosition;', 'attribute vec2 vertexUV0;', 'uniform mat4 viewMatrix;', 'uniform mat4 projectionMatrix;', 'uniform mat4 worldMatrix;', 'varying vec2 vUv;', 'void main() {', 'vUv = vertexUV0;', 'gl_Position = projectionMatrix * viewMatrix * worldMatrix * vec4( vertexPosition, 1.0 );', '}'].join('\n'),
		fshader: [_ShaderFragment.ShaderFragment.blendmodes, 'uniform sampler2D tDiffuse;', 'uniform float gamma;', 'uniform float minInput;', 'uniform float maxInput;', 'uniform float minOutput;', 'uniform float maxOutput;', 'varying vec2 vUv;', 'void main() {', 'gl_FragColor = texture2D( tDiffuse, vUv );', 'gl_FragColor.rgb = LevelsControl(gl_FragColor.rgb, minInput, gamma, maxInput, minOutput, maxOutput);', '}'].join('\n')
};
...
ShaderLibExtra.packDepth = {
		...
		fshader: ['uniform float farPlane;', _ShaderFragment.ShaderFragment.methods.packDepth, 'varying vec4 vPosition;', 'void main(void)', '{', 'float linearDepth = min(length(vPosition), farPlane) / farPlane;', 'gl_FragColor = packDepth(linearDepth);', '}'].join('\n')
};
 ...
 */
exports.ShaderLibExtra = ShaderLibExtra_ShaderLibExtra;