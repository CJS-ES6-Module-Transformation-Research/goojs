...
function MouseLookControlScript() {
	...
	function mouseDownToRequestPointerLock() {
		if (!hasPointerLock) {
			(0, _GameUtils.requestPointerLock)();
		}
	}
	...
	function update(parameters, environment) {
		...
		var maxAscent = parameters.maxAscent * _MathUtils.DEG_TO_RAD;
		var minAscent = parameters.minAscent * _MathUtils.DEG_TO_RAD;
		pitch = (0, _MathUtils.clamp)(pitch - deltaY * parameters.speed / 200, minAscent, maxAscent);

		var maxAzimuth = parameters.maxAzimuth * _MathUtils.DEG_TO_RAD - _initialAzimuth;
		var minAzimuth = parameters.minAzimuth * _MathUtils.DEG_TO_RAD - _initialAzimuth;
		yaw -= deltaX * parameters.speed / 200;
		if (parameters.clampAzimuth) {
			yaw = (0, _MathUtils.radialClamp)(yaw, minAzimuth, maxAzimuth);
		}
