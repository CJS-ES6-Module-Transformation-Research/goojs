...
function MouseLookControlScript() {
	...
	function mouseDownToRequestPointerLock() {
		if (!hasPointerLock) {
			GameUtils_GameUtilsjs.requestPointerLock();
		}
	}
	...
	function update(parameters, environment) {
		...
		var maxAscent = parameters.maxAscent * MathUtils_MathUtilsjs.DEG_TO_RAD;
		var minAscent = parameters.minAscent * MathUtils_MathUtilsjs.DEG_TO_RAD;
		pitch = MathUtils_MathUtilsjs.clamp(pitch - deltaY * parameters.speed / 200, minAscent, maxAscent);

		var maxAzimuth = parameters.maxAzimuth * MathUtils_MathUtilsjs.DEG_TO_RAD - _initialAzimuth;
		var minAzimuth = parameters.minAzimuth * MathUtils_MathUtilsjs.DEG_TO_RAD - _initialAzimuth;
		yaw -= deltaX * parameters.speed / 200;
		if (parameters.clampAzimuth) {
			yaw = MathUtils_MathUtilsjs.radialClamp(yaw, minAzimuth, maxAzimuth);
		}
	...
	function cleanup(parameters, environment) {
		var domElement = environment.domElement;
		if (button === 3) {
			GameUtils_GameUtilsjs.exitPointerLock();
...
var exported_MouseLookControlScript = MouseLookControlScript;
export { exported_MouseLookControlScript as MouseLookControlScript };