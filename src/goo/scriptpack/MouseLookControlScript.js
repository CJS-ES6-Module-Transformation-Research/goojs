...
function MouseLookControlScript() {
	...
	function update(parameters, environment) {
		...
		var maxAscent = parameters.maxAscent * MathUtilsjs_DEG_TO_RAD;
		var minAscent = parameters.minAscent * MathUtilsjs_DEG_TO_RAD;
		pitch = MathUtilsjs_clamp(pitch - deltaY * parameters.speed / 200, minAscent, maxAscent);

		var maxAzimuth = parameters.maxAzimuth * MathUtilsjs_DEG_TO_RAD - _initialAzimuth;
		var minAzimuth = parameters.minAzimuth * MathUtilsjs_DEG_TO_RAD - _initialAzimuth;
		yaw -= deltaX * parameters.speed / 200;
		if (parameters.clampAzimuth) {
			yaw = MathUtilsjs_radialClamp(yaw, minAzimuth, maxAzimuth);
		}
