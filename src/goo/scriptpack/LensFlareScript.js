...
function LensFlareScript() {
	...
	function generateTextures(txSize) {
		...
		textures.splash = ParticleSystemUtilsjs.createSplashTexture(512, { trailStartRadius: 25, trailEndRadius: 0 });
		textures.ring = ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.ring, startRadius: txSize / 4, endRadius: txSize / 2 });
		textures.dot = ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.dot, startRadius: 0, endRadius: txSize / 2 });
		textures.bell = ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.bell, startRadius: 0, endRadius: txSize / 2 });
		textures['default'] = ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.none, startRadius: 0, endRadius: txSize / 2 });
	}
...
function FlareQuad(lightColor, tx, displace, size, intensity, systemScale, edgeDampen, edgeScaling, textures, world) {
	...
	var material = new Materialjs(ShaderLibjs.uber, 'flareShader');
