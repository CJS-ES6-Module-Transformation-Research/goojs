...
function LensFlareScript() {
	...
	function generateTextures(txSize) {
		...
		textures.splash = ParticleSystemUtils_ParticleSystemUtilsjs.createSplashTexture(512, { trailStartRadius: 25, trailEndRadius: 0 });
		textures.ring = ParticleSystemUtils_ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.ring, startRadius: txSize / 4, endRadius: txSize / 2 });
		textures.dot = ParticleSystemUtils_ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.dot, startRadius: 0, endRadius: txSize / 2 });
		textures.bell = ParticleSystemUtils_ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.bell, startRadius: 0, endRadius: txSize / 2 });
		textures['default'] = ParticleSystemUtils_ParticleSystemUtilsjs.createFlareTexture(txSize, { steps: textureShapes.none, startRadius: 0, endRadius: txSize / 2 });
	}
	...
	function setup(args, ctx) {
		...
		ctx.bounds = new BoundingSphere_BoundingSpherejs(ctx.entity.transformComponent.sync().worldTransform.translation, 0);
	}
...
function FlareGeometry(edgeRelevance) {
	...
	this.offset = 0;
	this.centerRatio = 0;
	this.positionVector = new Vector3_Vector3js();
	this.distanceVector = new Vector3_Vector3js();
	this.centerVector = new Vector3_Vector3js();
	this.displacementVector = new Vector3_Vector3js();
...
function FlareQuad(lightColor, tx, displace, size, intensity, systemScale, edgeDampen, edgeScaling, textures, world) {
	this.sizeVector = new Vector3_Vector3js(size, size, size);
	this.sizeVector.scale(systemScale);
	this.positionVector = new Vector3_Vector3js();
	this.flareVector = new Vector3_Vector3js();
	...
	var material = new Material_Materialjs(ShaderLib_ShaderLibjs.uber, 'flareShader');
	...
	var meshData = new Quad_Quadjs(1, 1);
