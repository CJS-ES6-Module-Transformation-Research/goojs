...
ScriptComponentHandler.prototype = Object.create(ComponentHandlerjs.prototype);
ScriptComponentHandler.prototype.constructor = ScriptComponentHandler;
ComponentHandlerjs._registerClass('script', ScriptComponentHandler);
...
ScriptComponentHandler.prototype.update = function (entity, config, options) {
	...
	return ComponentHandlerjs.prototype.update.call(this, entity, config, options)
	.then(function (component) {
		if (!component) { return; }

		return rsvp_rsvpjsjs.all(ObjectUtilsjs.map(config.scripts, function (instanceConfig) {
...
ScriptComponentHandler.prototype._updateScriptInstance = function (component, instanceConfig, options) {
	...
	.then(function (script) {
		...
		if (script.parameters) {
			ObjectUtilsjs.defaults(newParameters, script.parameters);
		}

		if (script.externals && script.externals.parameters) {
			ScriptUtilsjs.fillDefaultValues(newParameters, script.externals.parameters);
		}
		...
		.then(ObjectUtilsjs.constant(newScript));
	});
};
...
ScriptComponentHandler.prototype._createOrLoadEngineScript = function (component, instanceConfig) {
	...
	if (existingScript) {
		return PromiseUtilsjs.resolve(existingScript);
	}
...
ScriptComponentHandler.prototype._findScriptInstance = function (component, instanceId) {
	return ObjectUtilsjs.find(component.scripts, function (script) {
...
ScriptComponentHandler.prototype._createEngineScript = function (scriptName) {
	...
	return PromiseUtilsjs.resolve(script);
};
...
ScriptComponentHandler.prototype._setParameter = function (parameters, config, external, options) {
	...
	function getInvalidParam() {
		if (external.default === undefined) {
			return ObjectUtilsjs.deepClone(ScriptUtilsjs.DEFAULTS_BY_TYPE[type]);
		} else {
			return ObjectUtilsjs.deepClone(external.default);
		}
	}
	...
	if (!ScriptUtilsjs.TYPE_VALIDATORS[type](config)) {
		...
		setRefParam();
		return PromiseUtilsjs.resolve();
	} else if (ScriptUtilsjs.isRefType(type)) {
		return setRefParam();
	} else {
		return setParam(ObjectUtilsjs.clone(config));
	}
};
