...
function AxisAlignedCamControlScript() {
	function setup(params, env) {
		// Look axis
		env.axis = Vector3_Vector3js.UNIT_Z.clone();
		// Up axis will most often be Y but you never know...
		env.upAxis = Vector3_Vector3js.UNIT_Y.clone();
		setView(params, env, params.view);
		env.currentView = params.view;
		env.lookAtPoint	= new Vector3_Vector3js();
		env.distance	= params.distance;
		env.smoothness	= Math.pow(MathUtils_MathUtilsjs.clamp(params.smoothness, 0, 1), 0.3);
	...
	function setView(params, env, view) {
		...
		switch (view) {
			case 'XY':
				env.axis.set(Vector3_Vector3js.UNIT_Z);
				env.upAxis.set(Vector3_Vector3js.UNIT_Y);
				break;
			case 'ZY':
				env.axis.set(Vector3_Vector3js.UNIT_X);
				env.upAxis.set(Vector3_Vector3js.UNIT_Y);
				break;
		}
		env.axisAlignedDirty = true;
