...
import { ScriptUtils as ScriptUtils_ScriptUtilsjs } from "../scripts/ScriptUtils";
...
ScriptHandler.prototype = Object.create(ConfigHandler_ConfigHandlerjs.prototype);
ScriptHandler.prototype.constructor = ScriptHandler;
ConfigHandler_ConfigHandlerjs._registerClass('script', ScriptHandler);
...
ScriptHandler.prototype._updateFromCustom = function (script, config) {
	...
	if (script.externals) {
		ScriptUtils_ScriptUtilsjs.fillDefaultNames(script.externals.parameters);
	}
...
function removeReference(scriptElement, scriptId) {
	...
	ArrayUtils_ArrayUtilsjs.remove(scriptElement.scriptRefs, scriptId);
}
...
ScriptHandler.prototype._addDependency = function (script, url, scriptId) {
	...
	if (scriptElem) {
		...
		return this._dependencyPromises[url] || PromiseUtils_PromiseUtilsjs.resolve();
	}
...
ScriptHandler.prototype._update = function (ref, config, options) {
	...
	return ConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options)
	.then(function (script) {
		...
		if (isCustomScript(config) && config.dependencies) {
			...
			ObjectUtils_ObjectUtilsjs.forEach(config.dependencies, function (dependencyConfig) {
				...
				var neededScriptElement = ArrayUtils_ArrayUtilsjs.find(scriptsElementsToRemove, function (scriptElement) {
					return scriptElement.src === url;
				});
				if (neededScriptElement) {
					ArrayUtils_ArrayUtilsjs.remove(scriptsElementsToRemove, neededScriptElement);
				}
			...
			ObjectUtils_ObjectUtilsjs.forEach(scriptsElementsToRemove, function (scriptElement) {
		...
		ObjectUtils_ObjectUtilsjs.forEach(config.dependencies, function (dependency) {
		...
		return rsvp_rsvpjsjs.all(addDependencyPromises)
		.then(function () {
			...
			ObjectUtils_ObjectUtilsjs.extend(script.parameters, config.options);
...
ScriptHandler.validateParameter = function validateParameter(parameter) {
   for (var i = 0; i < ScriptUtils_ScriptUtilsjs.PROPERTY_TYPES.length; ++i) {
       var entry = ScriptUtils_ScriptUtilsjs.PROPERTY_TYPES[i];
       ...
       if (entry.mustBeDefined || isPropDefined) {
           var validator = ScriptUtils_ScriptUtilsjs.TYPE_VALIDATORS[entry.type];
...
ScriptHandler.validateParameters = function validateParameters(script, outScript) {
	...
	for (var i = 0; i < externals.parameters.length; i++) {
		...
		if (parameter['default'] === null || parameter['default'] === undefined) {
			parameter['default'] = ScriptUtils_ScriptUtilsjs.DEFAULTS_BY_TYPE[parameter.type];
		}
