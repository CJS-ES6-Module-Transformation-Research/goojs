...
import { LogicLayer as LogicLayerjs } from "./logic/LogicLayer";
import { LogicInterface as LogicInterfacejs } from "./logic/LogicInterface";
function LogicSystem() {
	Systemjs.call(this, 'LogicSystem', null);

	this.passive = true;
	this._entities = {};
}

LogicSystem.prototype = Object.create(Systemjs.prototype);
...
LogicSystem.prototype.makeOutputWriteFn = function (sourceEntity, outPortDesc) {
	...
	this.forEachLogicObject(function (o) {
		...
		if (o.type === 'LogicNodeEntityProxy' && o.entityRef === sourceEntity.name) {
			matches.push([o.logicInstance, LogicInterfacejs.makePortDataName(outPortDesc)]);
	...
	return function (v) {
		for (var i = 0; i < matches.length; i++) {
			LogicLayerjs.writeValue(matches[i][0], matches[i][1], v);
		}
	};
};
...
var exported_LogicSystem = LogicSystem;
