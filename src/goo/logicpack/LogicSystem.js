...
function LogicSystem() {
	System_Systemjs.call(this, 'LogicSystem', null);

	this.passive = true;
	this._entities = {};
}

LogicSystem.prototype = Object.create(System_Systemjs.prototype);
...
LogicSystem.prototype.makeOutputWriteFn = function (sourceEntity, outPortDesc) {
	...
	this.forEachLogicObject(function (o) {
		...
		if (o.type === 'LogicNodeEntityProxy' && o.entityRef === sourceEntity.name) {
			matches.push([o.logicInstance, LogicInterface_LogicInterfacejs.makePortDataName(outPortDesc)]);
	...
	return function (v) {
		for (var i = 0; i < matches.length; i++) {
			LogicLayer_LogicLayerjs.writeValue(matches[i][0], matches[i][1], v);
		}
	};
};
