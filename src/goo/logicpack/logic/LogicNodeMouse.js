...
function LogicNodeMouse() {
	...
	LogicNodeMouse_eventMouseMove = function(event) {
        ...
        var dx = mx - this.x;
        var dy = my - this.y;
        LogicLayerjs_writeValue(this.logicInstance, LogicNodeMouse_portX, mx);
        LogicLayerjs_writeValue(this.logicInstance, LogicNodeMouse_portY, my);
        LogicLayerjs_writeValue(this.logicInstance, LogicNodeMouse_portDX, dx);
        LogicLayerjs_writeValue(this.logicInstance, LogicNodeMouse_portDY, dy);
    }.bind(this);;

	LogicNodeMouse_eventMouseDown = function(event) {
        if (event.button === 0) {
            LogicLayerjs_fireEvent(this.logicInstance, LogicNodeMouse_outEventLmb);
        }
        if (event.button === 2) {
            LogicLayerjs_fireEvent(this.logicInstance, LogicNodeMouse_outEventRmb);
        }
    }.bind(this);;
}
