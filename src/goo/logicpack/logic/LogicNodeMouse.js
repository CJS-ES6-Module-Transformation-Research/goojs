...
function LogicNodeMouse() {
  ...
  LogicNodeMouse_eventMouseMove = function (event) {
    ...
    var dx = mx - this.x;
    var dy = my - this.y;
    _LogicLayer.LogicLayer.writeValue(this.logicInstance, LogicNodeMouse_portX, mx);
    _LogicLayer.LogicLayer.writeValue(this.logicInstance, LogicNodeMouse_portY, my);
    _LogicLayer.LogicLayer.writeValue(this.logicInstance, LogicNodeMouse_portDX, dx);
    _LogicLayer.LogicLayer.writeValue(this.logicInstance, LogicNodeMouse_portDY, dy);
  }.bind(this);;

  LogicNodeMouse_eventMouseDown = function (event) {
    if (event.button === 0) {
      _LogicLayer.LogicLayer.fireEvent(this.logicInstance, LogicNodeMouse_outEventLmb);
    }
    if (event.button === 2) {
      _LogicLayer.LogicLayer.fireEvent(this.logicInstance, LogicNodeMouse_outEventRmb);
    }
  }.bind(this);;
}
