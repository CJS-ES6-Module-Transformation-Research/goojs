...
LogicNodeApplyMatrix.prototype.onInputChanged = function (instDesc) {
	var vec = LogicLayerjs_readPort(instDesc, LogicNodeApplyMatrix.inportX);
	var mat = LogicLayerjs_readPort(instDesc, LogicNodeApplyMatrix.inportY);
	this.vec.copy(vec);
	mat.applyPost(this.vec);
	LogicLayerjs_writeValue(this.logicInstance, LogicNodeApplyMatrix.outportProduct, this.vec);
};
