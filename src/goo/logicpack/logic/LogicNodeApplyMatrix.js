...
LogicNodeApplyMatrix.prototype.onInputChanged = function (instDesc) {
	var vec = _LogicLayer.LogicLayer.readPort(instDesc, LogicNodeApplyMatrix_inportX);
	var mat = _LogicLayer.LogicLayer.readPort(instDesc, LogicNodeApplyMatrix_inportY);
	this.vec.copy(vec);
	mat.applyPost(this.vec);
	_LogicLayer.LogicLayer.writeValue(this.logicInstance, LogicNodeApplyMatrix_outportProduct, this.vec);
};
