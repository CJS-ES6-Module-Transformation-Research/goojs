var LogicNodeTime_LogicNodeTime = LogicNodeTime;
...
LogicNodeTime.prototype.processLogic = function (tpf) {
	if (this._running) {
		var old = this._time;
		this._time += tpf;
		LogicLayerjs_writeValue(this.logicInstance, LogicNodeTime.outPropTime, this._time);

		if (old < 1 && this._time >= 1) {
			LogicLayerjs_fireEvent(this.logicInstance, LogicNodeTime.outEventReached1);
		}
	}
};
...
LogicNodeTime.prototype.onEvent = function (instDesc, event) {
	if (event === LogicNodeTime.inEventStart) {
		...
		this._time = 0;
		LogicLayerjs_writeValue(this.logicInstance, LogicNodeTime.outPropTime, 0);
	}
};
 ...
 * @private
 */
export { LogicNodeTime_LogicNodeTime as LogicNodeTime };