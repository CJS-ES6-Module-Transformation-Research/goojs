...
ScriptUtils_isRefType = function(type) {
    return utilObjectUtils_ObjectUtilsjs.contains(ScriptUtils_REF_TYPES, type);
};;

ScriptUtils_TYPE_VALIDATORS = function() {
    ...
    var isRef = function(type) {
        function isDirectRef(data) {
            return utilObjectUtils_ObjectUtilsjs.isString(data) && utilObjectUtils_ObjectUtilsjs.getExtension(data) === type;
        }
    ...
    return {
        "array": utilObjectUtils_ObjectUtilsjs.isArray,
        "float": utilObjectUtils_ObjectUtilsjs.isNumber,
        "number": utilObjectUtils_ObjectUtilsjs.isNumber,
        "string": utilObjectUtils_ObjectUtilsjs.isString,
        "boolean": utilObjectUtils_ObjectUtilsjs.isBoolean,
        "int": utilObjectUtils_ObjectUtilsjs.isInteger,
        "vec2": isVec(2),
        "vec3": isVec(3),
        "vec4": isVec(4),
...
ScriptUtils_fillDefaultValues = function(parameters, specs) {
    ...
    specs.forEach(function(spec) {
        ...
        if (spec.default === null || spec.default === undefined) {
            spec.default = utilObjectUtils_ObjectUtilsjs.deepClone(ScriptUtils_DEFAULTS_BY_TYPE[spec.type]);
        }

        keys.push(spec.key);
        if (typeof parameters[spec.key] === "undefined") {
            parameters[spec.key] = utilObjectUtils_ObjectUtilsjs.clone(spec.default);
        }
    });
...
export { ScriptUtils_DEFAULTS_BY_TYPE as DEFAULTS_BY_TYPE, ScriptUtils_isRefType as isRefType, ScriptUtils_TYPE_VALIDATORS as TYPE_VALIDATORS, ScriptUtils_PROPERTY_TYPES as PROPERTY_TYPES, ScriptUtils_fillDefaultValues as fillDefaultValues, ScriptUtils_fillDefaultNames as fillDefaultNames, ScriptUtils_keyForCode as keyForCode, ScriptUtils };
