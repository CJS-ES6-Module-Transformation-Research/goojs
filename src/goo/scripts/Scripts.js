...
Scripts.addClass = ObjectUtils_ObjectUtilsjs.warnOnce(
...
Scripts.getClasses = ObjectUtils_ObjectUtilsjs.warnOnce(
...
Scripts.create = function (key, options) {
	...
	if (factoryFunction.externals) {
		...
		ScriptUtils_ScriptUtilsjs.fillDefaultValues(script.parameters, factoryFunction.externals.parameters);
	}

	if (options) {
		ObjectUtils_ObjectUtilsjs.extend(script.parameters, options);
	}
...
var Scripts_register = function(factoryFunction) {
    ...
    ScriptUtils_ScriptUtilsjs.fillDefaultNames(factoryFunction.externals.parameters);
