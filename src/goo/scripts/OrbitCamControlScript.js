...
function setup(args, ctx) {
	...
	if (args.lookAtDistance) {
		...
		var spherical = ctx.spherical = new _Vector.Vector3(args.spherical[0], args.spherical[1] * _MathUtils.DEG_TO_RAD, args.spherical[2] * _MathUtils.DEG_TO_RAD);
	} else if (args.spherical) {
		var spherical = ctx.spherical = new _Vector.Vector3(args.spherical.x, args.spherical.y * _MathUtils.DEG_TO_RAD, args.spherical.z * _MathUtils.DEG_TO_RAD);
	} else {
...
function updateButtonState(buttonIndex, down, args, ctx) {
	...
	if (dragButton === -1 || dragButton === buttonIndex || down === false) {
		...
		if (down) {
			...
			ctx.spherical.y = (0, _MathUtils.moduloPositive)(ctx.spherical.y, _MathUtils.TWO_PI);
...
function move(azimuthAccel, thetaAccel, args, ctx) {
	...
	if (args.clampAzimuth) {
		var minAzimuth = args.minAzimuth * _MathUtils.DEG_TO_RAD;
		var maxAzimuth = args.maxAzimuth * _MathUtils.DEG_TO_RAD;
		td.y = (0, _MathUtils.radialClamp)(td.y - azimuthAccel, minAzimuth, maxAzimuth);
	} else {
		td.y -= azimuthAccel;
	}
	var minAscent = args.minAscent * _MathUtils.DEG_TO_RAD;
	var maxAscent = args.maxAscent * _MathUtils.DEG_TO_RAD;
	td.z = (0, _MathUtils.clamp)(td.z + thetaAccel, minAscent, maxAscent);
...
function applyWheel(e, args, ctx) {
	...
	var td = ctx.targetSpherical;
	td.x = (0, _MathUtils.clamp)(td.x + args.zoomSpeed * delta, args.minZoomDistance, args.maxZoomDistance);
	ctx.dirty = true;
}
...
function updateVelocity(time, args, ctx) {
	if (ctx.velocity.lengthSquared() > EPSILON) {
		...
		var rate = (0, _MathUtils.lerp)(ctx.inertia, 0, 1 - time / ctx.inertia);
...
function update(args, ctx /*, goo*/) {
	...
	var delta = (0, _MathUtils.lerp)(ctx.smoothness, 1, ctx.world.tpf);
	...
	sd.y = (0, _MathUtils.lerp)(delta, sd.y, tsd.y);
	// Move ascent to target
	sd.z = (0, _MathUtils.lerp)(delta, sd.z, tsd.z);

	// Move distance to target
	var deltaX = sd.x;
	sd.x = (0, _MathUtils.lerp)(delta, sd.x, tsd.x);
	deltaX /= sd.x;
	updateFrustumSize(deltaX, ctx);

	(0, _MathUtils.sphericalToCartesian)(sd.x, sd.y, sd.z, cartesian);
	...
	if (spherical.distanceSquared(targetSpherical) < EPSILON && ctx.lookAtPoint.equals(ctx.goingToLookAt)) {
		sd.y = (0, _MathUtils.moduloPositive)(sd.y, _MathUtils.TWO_PI);
...
function argsUpdated(args, ctx) {
	...
	ctx.smoothness = Math.pow((0, _MathUtils.clamp)(args.smoothness, 0, 1), 0.3);
	ctx.inertia = Math.pow((0, _MathUtils.clamp)(args.drag, 0, 1), 0.3);
