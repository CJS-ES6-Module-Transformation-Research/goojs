...
function setup(args, ctx) {
	...
	if (args.lookAtDistance) {
		...
		var spherical = ctx.spherical = new mathVector3_Vector3js(
			args.spherical[0],
			args.spherical[1] * MathUtilsjs_DEG_TO_RAD,
			args.spherical[2] * MathUtilsjs_DEG_TO_RAD
		);
	} else if (args.spherical) {
		var spherical = ctx.spherical = new mathVector3_Vector3js(
			args.spherical.x,
			args.spherical.y * MathUtilsjs_DEG_TO_RAD,
			args.spherical.z * MathUtilsjs_DEG_TO_RAD
		);
	} else {
...
function updateButtonState(buttonIndex, down, args, ctx) {
	...
	if (dragButton === -1 || dragButton === buttonIndex || down === false) {
		...
		if (down) {
			...
			ctx.spherical.y = MathUtilsjs_moduloPositive(ctx.spherical.y, MathUtilsjs_TWO_PI);
...
function move(azimuthAccel, thetaAccel, args, ctx) {
	...
	if (args.clampAzimuth) {
		var minAzimuth = args.minAzimuth * MathUtilsjs_DEG_TO_RAD;
		var maxAzimuth = args.maxAzimuth * MathUtilsjs_DEG_TO_RAD;
		td.y = MathUtilsjs_radialClamp(td.y - azimuthAccel, minAzimuth, maxAzimuth);
	} else {
		td.y -= azimuthAccel;
	}
	var minAscent = args.minAscent * MathUtilsjs_DEG_TO_RAD;
	var maxAscent = args.maxAscent * MathUtilsjs_DEG_TO_RAD;
	td.z = MathUtilsjs_clamp(td.z + thetaAccel, minAscent, maxAscent);
...
function updateVelocity(time, args, ctx) {
	if (ctx.velocity.lengthSquared() > EPSILON) {
		...
		var rate = MathUtilsjs_lerp(ctx.inertia, 0, 1 - time / ctx.inertia);
...
function update(args, ctx/*, goo*/) {
	...
	var delta = MathUtilsjs_lerp(ctx.smoothness, 1, ctx.world.tpf);
	...
	sd.y = MathUtilsjs_lerp(delta, sd.y, tsd.y);
	// Move ascent to target
	sd.z = MathUtilsjs_lerp(delta, sd.z, tsd.z);

	// Move distance to target
	var deltaX = sd.x;
	sd.x = MathUtilsjs_lerp(delta, sd.x, tsd.x);
	deltaX /= sd.x;
	updateFrustumSize(deltaX, ctx);

	MathUtilsjs_sphericalToCartesian(sd.x, sd.y, sd.z, cartesian);
	...
	if (spherical.distanceSquared(targetSpherical) < EPSILON && ctx.lookAtPoint.equals(ctx.goingToLookAt)) {
		sd.y = MathUtilsjs_moduloPositive(sd.y, MathUtilsjs_TWO_PI);
...
function argsUpdated(args, ctx) {
	...
	ctx.smoothness = Math.pow(MathUtilsjs_clamp(args.smoothness, 0, 1), 0.3);
	ctx.inertia = Math.pow(MathUtilsjs_clamp(args.drag, 0, 1), 0.3);
