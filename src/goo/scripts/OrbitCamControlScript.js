...
var OrbitCamControlScript_externals;
...
function setup(args, ctx) {
	...
	ctx.sample = 0;
	ctx.velocity = new Vector2js();
	ctx.cartesian = new Vector3js();
	ctx.worldUpVector = Vector3js_UNIT_Y.clone();
	...
	if (args.lookAtDistance) {
		...
		spherical = ctx.spherical = new Vector3js(
			args.lookAtDistance,
			-angles.y + Math.PI / 2,
			-angles.x
		);
	} else if (args.spherical instanceof Array) {
		var spherical = ctx.spherical = new Vector3js(
			args.spherical[0],
		...
		var spherical = ctx.spherical = new Vector3js(15, 0, 0); // Just something so the script won't crash
	}
	...
	if (args.lookAtDistance) {
		...
		ctx.lookAtPoint = new Vector3js(0, 0, -args.lookAtDistance);
		ctx.lookAtPoint.applyPost(rotation);
		ctx.lookAtPoint.add(ctx.entity.getTranslation());
	} else if (args.lookAtPoint) {
		ctx.lookAtPoint = args.lookAtPoint instanceof Array ? Vector3js_fromArray(args.lookAtPoint) : args.lookAtPoint.clone();
	} else {
		ctx.lookAtPoint = new Vector3js();
	}
...
function updateFrustumSize(delta, ctx) {
	...
	if (camera.projectionMode === Camerajs_Parallel) {
		ctx.size = camera.top;
		ctx.size /= delta;
		var size = ctx.size;
...
OrbitCamControlScript_externals = {
...
export { OrbitCamControlScript_externals as externals, OrbitCamControlScript };