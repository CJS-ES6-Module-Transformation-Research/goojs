...
function setup(args, ctx) {
	...
	ctx.worldUpVector = Vector3js.UNIT_Y.clone();
	...
	if (args.lookAtDistance) {
		...
		var spherical = ctx.spherical = new Vector3js(
			args.spherical[0],
			args.spherical[1] * MathUtilsjs.DEG_TO_RAD,
			args.spherical[2] * MathUtilsjs.DEG_TO_RAD
		);
	} else if (args.spherical) {
		var spherical = ctx.spherical = new Vector3js(
			args.spherical.x,
			args.spherical.y * MathUtilsjs.DEG_TO_RAD,
			args.spherical.z * MathUtilsjs.DEG_TO_RAD
		);
	} else {
	...
	if (args.lookAtDistance) {
		...
		ctx.lookAtPoint = args.lookAtPoint instanceof Array ? Vector3js.fromArray(args.lookAtPoint) : args.lookAtPoint.clone();
	} else {
...
function updateButtonState(buttonIndex, down, args, ctx) {
	...
	if (dragButton === -1 || dragButton === buttonIndex || down === false) {
		...
		if (down) {
			...
			ctx.spherical.y = MathUtilsjs.moduloPositive(ctx.spherical.y, MathUtilsjs.TWO_PI);
...
function move(azimuthAccel, thetaAccel, args, ctx) {
	...
	if (args.clampAzimuth) {
		var minAzimuth = args.minAzimuth * MathUtilsjs.DEG_TO_RAD;
		var maxAzimuth = args.maxAzimuth * MathUtilsjs.DEG_TO_RAD;
		td.y = MathUtilsjs.radialClamp(td.y - azimuthAccel, minAzimuth, maxAzimuth);
	} else {
		td.y -= azimuthAccel;
	}
	var minAscent = args.minAscent * MathUtilsjs.DEG_TO_RAD;
	var maxAscent = args.maxAscent * MathUtilsjs.DEG_TO_RAD;
	td.z = MathUtilsjs.clamp(td.z + thetaAccel, minAscent, maxAscent);
...
function updateFrustumSize(delta, ctx) {
	...
	if (camera.projectionMode === Camerajs.Parallel) {
		ctx.size = camera.top;
		ctx.size /= delta;
		var size = ctx.size;
...
function updateVelocity(time, args, ctx) {
	if (ctx.velocity.lengthSquared() > EPSILON) {
		...
		var rate = MathUtilsjs.lerp(ctx.inertia, 0, 1 - time / ctx.inertia);
...
function update(args, ctx/*, goo*/) {
	...
	var delta = MathUtilsjs.lerp(ctx.smoothness, 1, ctx.world.tpf);
	...
	sd.y = MathUtilsjs.lerp(delta, sd.y, tsd.y);
	// Move ascent to target
	sd.z = MathUtilsjs.lerp(delta, sd.z, tsd.z);

	// Move distance to target
	var deltaX = sd.x;
	sd.x = MathUtilsjs.lerp(delta, sd.x, tsd.x);
	deltaX /= sd.x;
	updateFrustumSize(deltaX, ctx);

	MathUtilsjs.sphericalToCartesian(sd.x, sd.y, sd.z, cartesian);
	...
	if (spherical.distanceSquared(targetSpherical) < EPSILON && ctx.lookAtPoint.equals(ctx.goingToLookAt)) {
		sd.y = MathUtilsjs.moduloPositive(sd.y, MathUtilsjs.TWO_PI);
...
function argsUpdated(args, ctx) {
	...
	ctx.smoothness = Math.pow(MathUtilsjs.clamp(args.smoothness, 0, 1), 0.3);
	ctx.inertia = Math.pow(MathUtilsjs.clamp(args.drag, 0, 1), 0.3);
