...
var functionObject_endsWith = function(str, suffix) {
    return str.indexOf(suffix, str.length - suffix.length) !== -1;
};
...
var functionObject_startsWith = function(str, prefix) {
    return str.indexOf(prefix) === 0;
};
...
var functionObject_capitalize = function(str) {
    return str.charAt(0).toUpperCase() + str.substring(1);
};
...
var functionObject_uncapitalize = function(str) {
    return str.charAt(0).toLowerCase() + str.substring(1);
};
...
var functionObject_createUniqueId = function(type) {
    var date = Date.now();
    var uuid = "xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx".replace(/[xy]/g, function(c) {
        // | 0 is a hack to floor a number, so this is a random number between 0 and 15
        var randomNumber = (date + Math.random() * 16) % 16 | 0;
...
var functionObject_getUntil = function(string, stopString) {
    var stopIndex = string.indexOf(stopString);
    if (stopIndex === -1) {
        return string;
    } else {
...
var functionObject_getAfterLast = function(string, stopString) {
    var stopIndex = string.lastIndexOf(stopString);
    if (stopIndex === -1) {
        return string;
    } else {
...
var functionObject_getFrom = function(string, startString) {
    var startIndex = string.indexOf(startString);
    if (startIndex === -1) {
        return "";
    } else {
...
var functionObject_getIndexedName = function(base, takenNames, separator) {
    if (!separator) {
        separator = "_";
    }
...
var functionObject_getUniqueName = function(desiredName, takenNames, separator) {
    if (takenNames.indexOf(desiredName) === -1) {
        return desiredName;
    }
...
var functionObject_toAscii = function(input) {
    return input.replace(/([^\x00-\x7F])/g, "x");
};
...
var functionObject_hashCode = function(str) {
    var hash = 0;
...
var functionObject_getUniqueId = function() {
    idCounter++;
    var stringedArguments = Array.prototype.slice.call(arguments, 0).join("");
...
var functionObject_escapeHtmlEntities = function(text) {
    var div = document.createElement("div");
    div.appendChild(document.createTextNode(text));
...
var functionObject_parseURL = function(uri) {
    var split = uri.match(splitRegExp);
    return {
        "scheme": split[1],
        "user_info": split[2],
