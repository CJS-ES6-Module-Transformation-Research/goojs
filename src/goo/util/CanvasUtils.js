...
var functionObject_loadCanvasFromPath = function(canvasPath, callback) {
    var options = {};
    if (arguments.length === 3) {
        // Called with loadCanvasFromPath(path, options, callback)
        options = arguments[1];
...
var functionObject_renderSvgToCanvas = function(svgSource, options, callback) {
    var url = "data:image/svg+xml;base64," + btoa(svgSource);
...
var functionObject_getMatrixFromCanvas = function(canvas) {
    var context = canvas.getContext("2d");
...
var functionObject_svgDataToImage = function(data) {
    var DOMURL = window.URL || window.webkitURL || window;
...
export { functionObject_renderSvgToCanvas as renderSvgToCanvas };
