...
TangentGenerator_addTangentBuffer = function(meshData, uvUnit) {
    ...
    var vertexBuffer = meshData.getAttributeBuffer(MeshDatajs_POSITION);
    if (!vertexBuffer) {
        return;
    }

    var normalBuffer = meshData.getAttributeBuffer(MeshDatajs_NORMAL);
    if (!normalBuffer) {
        return;
    }
    ...
    if (!textureBuffer && uvUnit !== 0) {
        textureBuffer = meshData.getAttributeBuffer(MeshDatajs_TEXCOORD0);
    }
    if (!textureBuffer) {
        return;
    }
    ...
    meshData.attributeMap[MeshDatajs_TANGENT] = MeshDatajs_createAttribute(4, "Float");
    meshData.rebuildData(meshData.vertexCount, meshData.indexCount, true);
    var tangentBuffer = meshData.getAttributeBuffer(MeshDatajs_TANGENT);
