...
TangentGenerator.addTangentBuffer = function (meshData, uvUnit) {
	...
	var vertexBuffer = meshData.getAttributeBuffer(MeshDatajs.POSITION);
	if (!vertexBuffer) {
		return;
	}

	var normalBuffer = meshData.getAttributeBuffer(MeshDatajs.NORMAL);
	if (!normalBuffer) {
		return;
	}
	...
	if (!textureBuffer && uvUnit !== 0) {
		textureBuffer = meshData.getAttributeBuffer(MeshDatajs.TEXCOORD0);
	}
	if (!textureBuffer) {
		return;
	}
	...
	for (var i = 0; i < vertexCount; i++) {
		tan1[i] = new Vector3js();
		tan2[i] = new Vector3js();
	}

	function getVector2Array(buf) {
		var arr = [];
		for (var i = 0; i < buf.length; i += 2) {
			arr.push(new Vector2js(buf[i + 0], buf[i + 1]));
		}
		return arr;
	}
	...
	function getVector3Array(buf) {
		...
		for (var i = 0; i < buf.length; i += 3) {
			arr.push(new Vector3js(buf[i + 0], buf[i + 1], buf[i + 2]));
		}
		return arr;
	}
	...
	for (var a = 0; a < triangleCount; a++) {
		...
		var sdir = new Vector3js((t2 * x1 - t1 * x2) * r, (t2 * y1 - t1 * y2) * r, (t2 * z1 - t1 * z2) * r);
		var tdir = new Vector3js((s1 * x2 - s2 * x1) * r, (s1 * y2 - s2 * y1) * r, (s1 * z2 - s2 * z1) * r);
	...
	meshData.attributeMap[MeshDatajs.TANGENT] = MeshDatajs.createAttribute(4, 'Float');
	meshData.rebuildData(meshData.vertexCount, meshData.indexCount, true);
	var tangentBuffer = meshData.getAttributeBuffer(MeshDatajs.TANGENT);

	var calc1 = new Vector3js();
	var calc2 = new Vector3js();
