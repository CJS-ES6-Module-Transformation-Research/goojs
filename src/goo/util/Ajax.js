...
Ajax.prototype.prefill = function (bundle, clear) {
	if (clear) {
		...
		ObjectUtilsjs.extend(this._cache, bundle);
	}
};
...
Ajax.prototype.load = function (path, reload) {
	var that = this;
	var path2 = StringUtilsjs.parseURL(path).path;//! AT: dunno what to call this
	...
	if (!path) {
		PromiseUtilsjs.reject('Path was undefined'); //! AT: no return?
	...
	if (this._cache[path] && !reload) {
		...
		if (this._cache[path] instanceof rsvp_rsvpjsjs.Promise) {
			...
			return PromiseUtilsjs.resolve(this._cache[path]);
		}
	}
...
Ajax.prototype._loadVideo = function (url, mimeType) {
	...
	var promise = PromiseUtilsjs.createPromise(function (resolve, reject) {
		var timeout;
...
Ajax.types = {
	...
	binary: addKeys({
		dat: true,
		bin: true
	}, Object.keys(TextureHandlerjs.loaders)),
	audio: {
		mp3: true,
		wav: true,
		ogg: true
