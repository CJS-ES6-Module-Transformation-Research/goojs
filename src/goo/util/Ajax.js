import { TextureHandler as TextureHandler_TextureHandlerjs } from "../loaders/handlers/TextureHandler";
...
Ajax.prototype.prefill = function (bundle, clear) {
	if (clear) {
		...
		ObjectUtils_ObjectUtilsjs.extend(this._cache, bundle);
	}
};
...
Ajax.prototype.get = function (options) {
	...
	return PromiseUtils_PromiseUtilsjs.createPromise(function (resolve, reject) {
...
Ajax.prototype.load = function (path, reload) {
	var that = this;
	var path2 = StringUtils_StringUtilsjs.parseURL(path).path;//! AT: dunno what to call this
	...
	if (!path) {
		PromiseUtils_PromiseUtilsjs.reject('Path was undefined'); //! AT: no return?
	...
	if (this._cache[path] && !reload) {
		...
		if (this._cache[path] instanceof rsvp_rsvpjsjs.Promise) {
			...
			return PromiseUtils_PromiseUtilsjs.resolve(this._cache[path]);
		}
	}
...
Ajax.prototype._loadVideo = function (url, mimeType) {
	...
	var promise = PromiseUtils_PromiseUtilsjs.createPromise(function (resolve, reject) {
		var timeout;
...
Ajax.types = {
	...
	binary: addKeys({
		dat: true,
		bin: true
	}, Object.keys(TextureHandler_TextureHandlerjs.loaders)),
	audio: {
		mp3: true,
		wav: true,
		ogg: true
