...
function Gizmo(name) {
	...
	this._plane = new _Plane2.default();
	this._line = new _Vector2.default();
...
Gizmo.prototype.updateTransforms = function () {
	if (_Renderer2.default.mainCamera) {
		...
		var scale;
		if (camera.projectionMode === _Camera2.default.Perspective) {
			var dist = camera.translation.distance(this.transform.translation);
			scale = dist * GIZMO_SIZE;
			scale *= Math.tan(camera.fov * _MathUtils2.default.DEG_TO_RAD / 2) * 2;
		} else {
...
(function () {
	...
	var worldX = new _Vector2.default();
	var worldY = new _Vector2.default();
	var worldZ = new _Vector2.default();
	var screenCenter = new _Vector2.default();
	var screenX = new _Vector2.default();
	var screenY = new _Vector2.default();
	var screenZ = new _Vector2.default();
	...
	Gizmo.prototype._setPlane = function () {
		...
		if (this._activeHandle.type === 'Plane') {
			...
			normal.copy([_Vector2.default.UNIT_X, _Vector2.default.UNIT_Y, _Vector2.default.UNIT_Z][this._activeHandle.axis]);
			...
			worldX.copy(_Vector2.default.UNIT_X);
			worldX.applyPostPoint(this.transform.matrix);

			worldY.copy(_Vector2.default.UNIT_Y);
			worldY.applyPostPoint(this.transform.matrix);

			worldZ.copy(_Vector2.default.UNIT_Z);
			worldZ.applyPostPoint(this.transform.matrix);

			// Gizmo handle points in screen space
			_Renderer2.default.mainCamera.getScreenCoordinates(worldCenter, 1, 1, screenCenter);
			_Renderer2.default.mainCamera.getScreenCoordinates(worldX, 1, 1, screenX);
			screenX.sub(screenCenter);
			_Renderer2.default.mainCamera.getScreenCoordinates(worldY, 1, 1, screenY);
			screenY.sub(screenCenter);
			_Renderer2.default.mainCamera.getScreenCoordinates(worldZ, 1, 1, screenZ);
...
Gizmo.prototype._setLine = function () {
	...
	this._line.copy([_Vector2.default.UNIT_X, _Vector2.default.UNIT_Y, _Vector2.default.UNIT_Z][this._activeHandle.axis]);
...
Gizmo.buildMaterialForAxis = function (axis, opacity) {
	var material = new _Material2.default(SHADER_DEF, axis + 'Material');
