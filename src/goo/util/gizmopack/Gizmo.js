...
Gizmo.prototype.updateTransforms = function () {
	if (rendererRenderer_Rendererjs.mainCamera) {
		...
		if (camera.projectionMode === rendererCamera_Camerajs.Perspective) {
			...
			scale *= Math.tan(camera.fov * mathMathUtils_MathUtilsjs.DEG_TO_RAD / 2) * 2;
		} else {
...
(function () {
	...
	Gizmo.prototype._setPlane = function () {
		...
		if (this._activeHandle.type === 'Plane') {
			...
			rendererRenderer_Rendererjs.mainCamera.getScreenCoordinates(worldCenter, 1, 1, screenCenter);
			rendererRenderer_Rendererjs.mainCamera.getScreenCoordinates(worldX, 1, 1, screenX);
			screenX.sub(screenCenter);
			rendererRenderer_Rendererjs.mainCamera.getScreenCoordinates(worldY, 1, 1, screenY);
			screenY.sub(screenCenter);
			rendererRenderer_Rendererjs.mainCamera.getScreenCoordinates(worldZ, 1, 1, screenZ);
