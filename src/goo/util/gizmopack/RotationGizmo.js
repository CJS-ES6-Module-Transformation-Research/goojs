...
(function () {
	...
	RotationGizmo.prototype.activate = function (props) {
		...
		if (this._activeHandle.axis < 3) {
			...
			_Renderer.mainCamera.getPickRay(props.x, props.y, 1, 1, ray);
			...
			_Renderer.mainCamera.getScreenCoordinates(rotationDirection, 1, 1, this._direction);
...
(function () {
	...
	RotationGizmo.prototype._rotateOnScreen = function (delta) {
		...
		var camMat = _Renderer.mainCamera.getViewMatrix();
...
function buildTorus(torusMesh, dim) {
	...
	if (dim === 0) {
		transform.setRotationXYZ(0, _MathUtils.HALF_PI, 0);
	} else if (dim === 1) {
		transform.setRotationXYZ(_MathUtils.HALF_PI, 0, 0);
	}
