...
function ScaleGizmo(gizmoRenderSystem) {
	Gizmo_Gizmojs.call(this, 'ScaleGizmo', gizmoRenderSystem);

	this._transformScale = new Vector3_Vector3js(1, 1, 1);

	this.compileRenderables();
}

ScaleGizmo.prototype = Object.create(Gizmo_Gizmojs.prototype);
...
ScaleGizmo.prototype._scaleUniform = function (mouseState, oldMouseState) {
	...
	var cameraEntityDistance = Renderer_Rendererjs.mainCamera.translation.distance(this.transform.translation);
	scale += cameraEntityDistance / 200000 * MathUtils_MathUtilsjs.sign(scale - 1);
...
(function () {
	...
	var result = new Vector3_Vector3js();
...
ScaleGizmo.prototype.compileRenderables = function () {
	var boxMesh = new Box_Boxjs(1.4, 1.4, 1.4);
...
function buildBox(boxMesh) {
	return {
		meshData: boxMesh,
		materials: [Gizmo_Gizmojs.buildMaterialForAxis(3)],
		transform: new Transform_Transformjs(),
		id: Gizmo_Gizmojs.registerHandle({ type: 'Scale', axis: 3 })
	};
}
...
function buildArrow(arrowMesh, dim) {
	...
	return {
		...
		transform: transform,
		id: Gizmo_Gizmojs.registerHandle({ type: 'Scale', axis: dim })
	};
}
...
function buildArrowMesh() {
	...
	// Box
	var mesh1Data = new Box_Boxjs();

	// Line
	var mesh2Data = new MeshData_MeshDatajs(MeshData_MeshDatajs.defaultMap([MeshData_MeshDatajs.POSITION]), 2, 2);
	mesh2Data.getAttributeBuffer(MeshData_MeshDatajs.POSITION).set([0, 0, 0, 0, 0, 1]);
