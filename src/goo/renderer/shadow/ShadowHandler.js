...
function ShadowHandler() {
	this.depthMaterial = new Material_Materialjs(ShaderLib_ShaderLibjs.lightDepth, 'depthMaterial');
	this.depthMaterial.cullState.cullFace = 'Back';
	this.depthMaterial.fullOverride = true;
	this.fullscreenPass = new FullscreenPass_FullscreenPassjs();
	this.downsample = Material_Materialjs.createShader(ShaderLib_ShaderLibjs.downsample, 'downsample');

	var sigma = 2;
	this.blurfilter = Material_Materialjs.createShader(ShaderLib_ShaderLibjs.convolution, 'blurfilter');
	var kernelSize = 2 * Math.ceil(sigma * 3.0) + 1;
	this.blurfilter.defines = {
		KERNEL_SIZE_FLOAT: kernelSize.toFixed(1),
		KERNEL_SIZE_INT: kernelSize.toFixed(0)
	};
	this.blurfilter.uniforms.cKernel = ShaderLib_ShaderLibjs.convolution.buildKernel(sigma);

	this.oldClearColor = new Vector4_Vector4js(0, 0, 0, 0);
	this.shadowClearColor = new Vector4_Vector4js(1, 1, 1, 1);
...
ShadowHandler.prototype._createShadowData = function (shadowSettings, renderer) {
	...
	var linearFloat = !!Capabilities_Capabilitiesjs.TextureFloatLinear;
	...
	if (shadowSettings.shadowType === 'VSM') {
		var floatType = Capabilities_Capabilitiesjs.TextureHalfFloat ? 'HalfFloat' : 'Float';
		var type = {
			type: floatType
		};
		if (!linearFloat) {
		...
		shadowSettings.shadowData.shadowTargetDown = new RenderTarget_RenderTargetjs(shadowX / 2, shadowY / 2, type);
		if (shadowSettings.shadowData.shadowBlurred) {
			renderer._deallocateRenderTarget(shadowSettings.shadowData.shadowBlurred);
		}
		shadowSettings.shadowData.shadowBlurred = new RenderTarget_RenderTargetjs(shadowX / 2, shadowY / 2, type);

		shadowSettings.shadowData.shadowTarget = new RenderTarget_RenderTargetjs(shadowX, shadowY, {
			type: floatType,
	...
	for (var i = 0; i < lights.length; i++) {
		...
		if (light.shadowCaster || light.lightCookie) {
			...
			if (!shadowSettings.shadowData) {
				...
				shadowSettings.shadowData.lightCamera = new Camera_Camerajs(55, 1, 1, 1000);
			}
			...
			if (light.direction) {
				...
				lightCamera.lookAt(Vector3_Vector3js.ZERO, shadowSettings.upVector);
			}
			...
			if (!shadowSettings.shadowData.shadowTarget ||
				...
				if (light instanceof SpotLight_SpotLightjs) {
				...
				} else if (light instanceof PointLight_PointLightjs) {
					...
					lightCamera.projectionMode = Camera_Camerajs.Parallel;
				}
