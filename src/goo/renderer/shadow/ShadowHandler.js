...
function ShadowHandler() {
	this.depthMaterial = new _Material.Material(_ShaderLib.ShaderLib.lightDepth, 'depthMaterial');
	...
	this.downsample = _Material.Material.createShader(_ShaderLib.ShaderLib.downsample, 'downsample');

	var sigma = 2;
	this.blurfilter = _Material.Material.createShader(_ShaderLib.ShaderLib.convolution, 'blurfilter');
	var kernelSize = 2 * Math.ceil(sigma * 3.0) + 1;
	this.blurfilter.defines = {
		KERNEL_SIZE_FLOAT: kernelSize.toFixed(1),
		KERNEL_SIZE_INT: kernelSize.toFixed(0)
	};
	this.blurfilter.uniforms.cKernel = _ShaderLib.ShaderLib.convolution.buildKernel(sigma);
...
ShadowHandler.prototype._createShadowData = function (shadowSettings, renderer) {
	...
	var linearFloat = !!_Capabilities.Capabilities.TextureFloatLinear;
	...
	if (shadowSettings.shadowType === 'VSM') {
		var floatType = _Capabilities.Capabilities.TextureHalfFloat ? 'HalfFloat' : 'Float';
		var type = {
			type: floatType
		};
		if (!linearFloat) {
	...
	for (var i = 0; i < lights.length; i++) {
		...
		if (light.shadowCaster || light.lightCookie) {
			...
			if (light.direction) {
				...
				lightCamera.lookAt(_Vector.Vector3.ZERO, shadowSettings.upVector);
			}
			...
			if (!shadowSettings.shadowData.shadowTarget || record.angle !== light.angle || !record.resolution || record.resolution[0] !== shadowSettings.resolution[0] || record.resolution[1] !== shadowSettings.resolution[1] || record.near !== shadowSettings.near || record.far !== shadowSettings.far || record.size !== shadowSettings.size) {
				...
				if (light instanceof _SpotLight.SpotLight) {
					...
					lightCamera.projectionMode = _Camera.Camera.Parallel;
				}
 ...
 * Handles shadow techniques
 */
exports.ShadowHandler = mod_ShadowHandler;