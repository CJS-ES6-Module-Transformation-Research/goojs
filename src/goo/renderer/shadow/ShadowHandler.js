var mod_ShadowHandler = ShadowHandler;
...
function ShadowHandler() {
	this.depthMaterial = new Material_Material(ShaderLib_ShaderLib.lightDepth, 'depthMaterial');
	this.depthMaterial.cullState.cullFace = 'Back';
	this.depthMaterial.fullOverride = true;
	this.fullscreenPass = new FullscreenPass_FullscreenPass();
	this.downsample = Material_Material.createShader(ShaderLib_ShaderLib.downsample, 'downsample');

	var sigma = 2;
	this.blurfilter = Material_Material.createShader(ShaderLib_ShaderLib.convolution, 'blurfilter');
	var kernelSize = 2 * Math.ceil(sigma * 3.0) + 1;
	this.blurfilter.defines = {
		KERNEL_SIZE_FLOAT: kernelSize.toFixed(1),
		KERNEL_SIZE_INT: kernelSize.toFixed(0)
	};
	this.blurfilter.uniforms.cKernel = ShaderLib_ShaderLib.convolution.buildKernel(sigma);

	this.oldClearColor = new Vector4_Vector4(0, 0, 0, 0);
	this.shadowClearColor = new Vector4_Vector4(1, 1, 1, 1);
...
ShadowHandler.prototype._createShadowData = function (shadowSettings, renderer) {
	...
	var linearFloat = !!Capabilities_Capabilities.TextureFloatLinear;
	...
	if (shadowSettings.shadowType === 'VSM') {
		var floatType = Capabilities_Capabilities.TextureHalfFloat ? 'HalfFloat' : 'Float';
		var type = {
			type: floatType
		};
		if (!linearFloat) {
		...
		shadowSettings.shadowData.shadowTargetDown = new RenderTarget_RenderTarget(shadowX / 2, shadowY / 2, type);
		if (shadowSettings.shadowData.shadowBlurred) {
			renderer._deallocateRenderTarget(shadowSettings.shadowData.shadowBlurred);
		}
		shadowSettings.shadowData.shadowBlurred = new RenderTarget_RenderTarget(shadowX / 2, shadowY / 2, type);

		shadowSettings.shadowData.shadowTarget = new RenderTarget_RenderTarget(shadowX, shadowY, {
			type: floatType,
	...
	for (var i = 0; i < lights.length; i++) {
		...
		if (light.shadowCaster || light.lightCookie) {
			...
			if (!shadowSettings.shadowData) {
				...
				shadowSettings.shadowData.lightCamera = new Camera_Camera(55, 1, 1, 1000);
			}
			...
			if (light.direction) {
				...
				lightCamera.lookAt(Vector3_Vector3.ZERO, shadowSettings.upVector);
			}
			...
			if (!shadowSettings.shadowData.shadowTarget ||
				...
				if (light instanceof SpotLight_SpotLight) {
				...
				} else if (light instanceof PointLight_PointLight) {
					...
					lightCamera.projectionMode = Camera_Camera.Parallel;
				}
 ...
 * Handles shadow techniques
 */
export { mod_ShadowHandler as ShadowHandler };