...
(function () {
	var relativePoint = new Vector3_Vector3js();
...
BoundingSphere.prototype.computeFromPrimitives = function (data, section, indices, start, end) {
	...
	var vertList = [];
	var store = [];
	var vertsPerPrimitive = MeshData_MeshDatajs.getVertexCount(data.indexModes[section]);

	var count = 0;
	for (var i = start; i < end; i++) {
		store = data.getPrimitiveVertices(indices[i], section, store);
		for (var j = 0; j < vertsPerPrimitive; j++) {
			vertList[count++] = new Vector3_Vector3js().set(store[j]);
		}
	}
...
BoundingSphere.prototype.intersectsRayWhere = function (ray) {
	var diff = new Vector3_Vector3js().copy(ray.origin).sub(this.center);
	var a = diff.dot(diff) - this.radius * this.radius;
	var a1, discr, root;
	if (a <= 0.0) {
		// inside sphere
		...
		var points = [new Vector3_Vector3js().copy(ray.direction).scale(distances[0]).add(ray.origin)];
		return {
			distances: distances,
			points: points
		};
	...
	if (discr < 0.0) {
		...
		var points = [new Vector3_Vector3js().copy(ray.direction).scale(distances[0]).add(ray.origin),
			new Vector3_Vector3js().copy(ray.direction).scale(distances[1]).add(ray.origin)];
		return {
			distances: distances,
			points: points
		};
