var mod_BoundingSphere = BoundingSphere;
import { Vector3 as Vector3_Vector3 } from "../../math/Vector3";
import { MathUtils as MathUtils_MathUtils } from "../../math/MathUtils";
import { BoundingVolume as BoundingVolume_BoundingVolume } from "../../renderer/bounds/BoundingVolume";
...
BoundingSphere.prototype = Object.create(BoundingVolume_BoundingVolume.prototype);
...
(function () {
	var relativePoint = new Vector3_Vector3();
...
BoundingSphere.prototype.computeFromPrimitives = function (data, section, indices, start, end) {
	...
	var vertList = [];
	var store = [];
	var vertsPerPrimitive = MeshData_MeshData.getVertexCount(data.indexModes[section]);

	var count = 0;
	for (var i = start; i < end; i++) {
		store = data.getPrimitiveVertices(indices[i], section, store);
		for (var j = 0; j < vertsPerPrimitive; j++) {
			vertList[count++] = new Vector3_Vector3().set(store[j]);
		}
	}
...
BoundingSphere.prototype.whichSide = function (plane) {
	...
	if (distance < -this.radius) {
		return BoundingVolume_BoundingVolume.Inside;
	} else if (distance > this.radius) {
		return BoundingVolume_BoundingVolume.Outside;
	} else {
		return BoundingVolume_BoundingVolume.Intersects;
	}
};
...
BoundingSphere.prototype.intersectsRayWhere = function (ray) {
	var diff = new Vector3_Vector3().copy(ray.origin).sub(this.center);
	var a = diff.dot(diff) - this.radius * this.radius;
	var a1, discr, root;
	if (a <= 0.0) {
		// inside sphere
		...
		var points = [new Vector3_Vector3().copy(ray.direction).scale(distances[0]).add(ray.origin)];
		return {
			distances: distances,
			points: points
		};
	...
	if (discr < 0.0) {
		...
		var points = [new Vector3_Vector3().copy(ray.direction).scale(distances[0]).add(ray.origin),
			new Vector3_Vector3().copy(ray.direction).scale(distances[1]).add(ray.origin)];
		return {
			distances: distances,
			points: points
		};
 ...
 *        <code>averagePoints</code>. A call to <code>computeFramePoint</code> in turn calls <code>containAABB</code>.
 */
export { mod_BoundingSphere as BoundingSphere };