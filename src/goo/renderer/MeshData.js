...
MeshData.prototype.rebuildVertexData = function (vertexCount) {
	...
	if (this.vertexCount > 0) {
		...
		for (var i = 0; i < keys.length; i++) {
			...
			vertexByteSize += RendererUtils_RendererUtils.getByteSize(attribute.type) * attribute.count;
		}
...
MeshData.prototype.rebuildIndexData = function (indexCount) {
	...
	if (this.indexCount > 0) {
		var indices = BufferUtils_BufferUtils.createIndexBuffer(this.indexCount, this.vertexCount);
...
MeshData.prototype.generateAttributeData = function () {
	...
	for (var key in this.attributeMap) {
		...
		offset += length * RendererUtils_RendererUtils.getByteSize(attribute.type);
...
MeshData.prototype.makeInterleavedData = function () {
	...
	for (var key in this.attributeMap) {
		...
		stride += attribute.count * RendererUtils_RendererUtils.getByteSize(attribute.type);
	}
	...
	for (var key in this.attributeMap) {
		...
		var size = RendererUtils_RendererUtils.getByteSize(attribute.type);
