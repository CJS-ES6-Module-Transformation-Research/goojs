import { BufferData as BufferDatajs } from "../renderer/BufferData";
...
MeshData.prototype.rebuildVertexData = function (vertexCount) {
	...
	if (this.vertexCount > 0) {
		...
		for (var i = 0; i < keys.length; i++) {
			...
			vertexByteSize += RendererUtilsjs.getByteSize(attribute.type) * attribute.count;
		}
		this.vertexData = new BufferDatajs(new ArrayBuffer(vertexByteSize * this.vertexCount), 'ArrayBuffer');
...
MeshData.prototype.rebuildIndexData = function (indexCount) {
	...
	if (this.indexCount > 0) {
		var indices = BufferUtilsjs.createIndexBuffer(this.indexCount, this.vertexCount);
		this.indexData = new BufferDatajs(indices, 'ElementArrayBuffer');
	} else {
		this.indexData = null;
		this.indexLengths = null;
		this.indexModes = ['Triangles'];
...
MeshData.prototype.generateAttributeData = function () {
	...
	for (var key in this.attributeMap) {
		...
		offset += length * RendererUtilsjs.getByteSize(attribute.type);
...
MeshData.prototype.makeInterleavedData = function () {
	...
	for (var key in this.attributeMap) {
		...
		stride += attribute.count * RendererUtilsjs.getByteSize(attribute.type);
	}

	var newVertexData = new BufferDatajs(new ArrayBuffer(stride * this.vertexCount), this.vertexData.target);
	...
	for (var key in this.attributeMap) {
		...
		var size = RendererUtilsjs.getByteSize(attribute.type);
...
MeshData.prototype.buildWireframeData = function () {
	var attributeMap = ObjectUtilsjs.deepClone(this.attributeMap);
...
// Calculation helpers
var v1 = new Vector3js();
var v2 = new Vector3js();
var v3 = new Vector3js();
/**
...
MeshData.prototype.clone = function () {
	var attributeMapClone = ObjectUtilsjs.deepClone(this.attributeMap);
...
function buildMap(types) {
	...
	for (var i = 0; i < types.length; i++) {
		...
		if (defaults[type] !== undefined) {
			map[type] = ObjectUtilsjs.deepClone(defaults[type]);
		} else {
