...
Renderer.prototype.bindData = function (bufferData) {
	...
	if (glBuffer !== null) {
		...
		if (bufferData._dataNeedsRefresh) {
			context.bufferSubData((0, _RendererUtils.getGLBufferTarget)(context, bufferData.target), 0, bufferData.data);
		...
		this.setBoundBuffer(glBuffer, bufferData.target);
		context.bufferData((0, _RendererUtils.getGLBufferTarget)(context, bufferData.target), bufferData.data, (0, _RendererUtils.getGLBufferUsage)(context, bufferData._dataUsage));
	}
};
...
Renderer.prototype.preloadTexture = function (context, texture) {
	...
	context.bindTexture((0, _RendererUtils.getGLType)(context, texture.variant), texture.glTexture);
	...
	if (texture.variant === '2D') {
		if (!image) {
			context.texImage2D(context.TEXTURE_2D, 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), texture.width, texture.height, 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLDataType)(context, texture.type), null);
		} else {
			...
			if (image.isData === true) {
				if (image.isCompressed) {
					...
					context.texImage2D(context.TEXTURE_2D, 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), image.width, image.height, texture.hasBorder ? 1 : 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLDataType)(context, texture.type), image.data);
				}
			} else {
				context.texImage2D(context.TEXTURE_2D, 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLDataType)(context, texture.type), image);
			}
		...
		if (image && !image.isData && (texture.generateMipmaps || image.width > this.maxCubemapSize || image.height > this.maxCubemapSize)) {
			for (var i = 0; i < Texturejs_CUBE_FACES.length; i++) {
				if (image.data[i] && !image.data[i].buffer) {
					(0, _RendererUtils.scaleImage)(texture, image.data[i], image.width, image.height, this.maxCubemapSize, i);
				} else {
					// REVIEW: Hard coded background color that should be determined by Create?
					(0, _RendererUtils.getBlankImage)(texture, [0.3, 0.3, 0.3, 0], image.width, image.height, this.maxCubemapSize, i);
				}
			}
			texture.image.width = Math.min(this.maxCubemapSize, _MathUtils.MathUtils.nearestPowerOfTwo(texture.image.width));
			texture.image.height = Math.min(this.maxCubemapSize, _MathUtils.MathUtils.nearestPowerOfTwo(texture.image.height));
			image = texture.image;
		}
		...
		for (var faceIndex = 0; faceIndex < Texturejs_CUBE_FACES.length; faceIndex++) {
			...
			if (!image) {
				context.texImage2D((0, _RendererUtils.getGLCubeMapFace)(context, face), 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), texture.width, texture.height, 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLDataType)(context, texture.type), null);
			} else {
				if (image.isData === true) {
					if (image.isCompressed) {
						this.loadCompressedTexture(context, (0, _RendererUtils.getGLCubeMapFace)(context, face), texture, image.data[faceIndex]);
					} else {
						context.texImage2D((0, _RendererUtils.getGLCubeMapFace)(context, face), 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), image.width, image.height, texture.hasBorder ? 1 : 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLDataType)(context, texture.type), image.data[faceIndex]);
					}
				} else {
					context.texImage2D((0, _RendererUtils.getGLCubeMapFace)(context, face), 0, (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLInternalFormat)(context, texture.format), (0, _RendererUtils.getGLDataType)(context, texture.type), image.data[faceIndex]);
				}
			}
		}
		...
		textureList.forEach(function (texture) {
			...
			queue.push(function () {
				if (texture instanceof _RenderTarget.RenderTarget === false && (texture.image === undefined || texture.checkDataReady() === false)) {
					if (texture.variant === '2D') {
						texture = _TextureCreator.DEFAULT_TEXTURE_2D;
					} else if (texture.variant === 'CUBE') {
						texture = _TextureCreator.DEFAULT_TEXTURE_CUBE;
					}
				}
...
Renderer.prototype.preloadMaterials = function (renderList) {
	...
	return (0, _TaskScheduler.each)(queue);
};
...
Renderer.prototype.render = function (renderList, camera, lights, renderTarget, clear, overrideMaterials) {
	...
	if (renderTarget && renderTarget.generateMipmaps && _MathUtils.MathUtils.isPowerOfTwo(renderTarget.width) && _MathUtils.MathUtils.isPowerOfTwo(renderTarget.height)) {
...
Renderer.prototype.drawElementsVBO = function (indices, indexModes, indexLengths) {
	...
	var type = indices.type = indices.type || (0, _RendererUtils.getGLArrayType)(this.context, indices);
	var byteSize = (0, _RendererUtils.getGLByteSize)(indices);

	for (var i = 0; i < indexLengths.length; i++) {
		var count = indexLengths[i];
		var glIndexMode = (0, _RendererUtils.getGLIndexMode)(this.context, indexModes[indexModeCounter]);
...
Renderer.prototype.renderToPick = function (renderList, camera, clear, skipUpdateBuffer, doScissor, clientX, clientY, customPickingMaterial, skipOverride) {
	...
	if (this.hardwarePicking === null) {
		var pickingMaterial = _Material.Material.createEmptyMaterial(_ShaderLib.pickingShader, 'pickingMaterial');
...
Renderer.prototype.updateDepthTest = function (material) {
	...
	if (record.depthFunc !== depthState.depthFunc) {
		this.context.depthFunc((0, _RendererUtils.getGLDepthFunc)(this.context, depthState.depthFunc));
...
Renderer.prototype.updateTextures = function (material) {
	...
	for (var i = 0; i < textureSlots.length; i++) {
		...
		for (var j = 0; j < textureList.length; j++) {
			...
			var isTexturePowerOfTwo = _MathUtils.MathUtils.isPowerOfTwo(imageObject.width) && _MathUtils.MathUtils.isPowerOfTwo(imageObject.height);
...
Renderer.prototype.updateTextureParameters = function (texture, isImagePowerOfTwo) {
	...
	var glType = (0, _RendererUtils.getGLType)(context, texture.variant);
	if (texrecord.magFilter !== texture.magFilter) {
		context.texParameteri(glType, context.TEXTURE_MAG_FILTER, (0, _RendererUtils.getGLMagFilter)(context, texture.magFilter));
		texrecord.magFilter = texture.magFilter;
	}
	var minFilter = isImagePowerOfTwo ? texture.minFilter : (0, _RendererUtils.getFilterFallback)(texture.minFilter);
	if (texrecord.minFilter !== minFilter) {
		context.texParameteri(glType, context.TEXTURE_MIN_FILTER, (0, _RendererUtils.getGLMinFilter)(context, minFilter));
	...
	if (texrecord.wrapS !== wrapS) {
		var glwrapS = (0, _RendererUtils.getGLWrap)(context, wrapS);
	...
	if (texrecord.wrapT !== wrapT) {
		var glwrapT = (0, _RendererUtils.getGLWrap)(context, wrapT);
...
Renderer.prototype.unbindTexture = function (context, texture, unit, record) {
	context.activeTexture(context.TEXTURE0 + unit);
	context.bindTexture((0, _RendererUtils.getGLType)(context, texture.variant), null);
...
Renderer.prototype.checkRescale = function (texture, image, width, height, maxSize, index) {
	(0, _RendererUtils.scaleImage)(texture, image, width, height, maxSize, index);
};
...
Renderer.prototype.updateBlending = function (material) {
	...
	if (blending !== blendRecord.blending) {
		if (blending === 'NoBlending') {
			...
			context.enable(context.BLEND);
			context.blendEquationSeparate((0, _RendererUtils.getGLBlendParam)(context, material.blendState.blendEquationColor), (0, _RendererUtils.getGLBlendParam)(context, material.blendState.blendEquationAlpha));
			context.blendFuncSeparate((0, _RendererUtils.getGLBlendParam)(context, material.blendState.blendSrcColor), (0, _RendererUtils.getGLBlendParam)(context, material.blendState.blendDstColor), (0, _RendererUtils.getGLBlendParam)(context, material.blendState.blendSrcAlpha), (0, _RendererUtils.getGLBlendParam)(context, material.blendState.blendDstAlpha));
		} else {
			context.enable(context.BLEND);
			context.blendEquationSeparate(context.FUNC_ADD, context.FUNC_ADD);
			context.blendFuncSeparate(context.SRC_ALPHA, context.ONE_MINUS_SRC_ALPHA, context.ONE, context.ONE_MINUS_SRC_ALPHA);
	...
	if (blending === 'CustomBlending') {
		...
		if (blendEquation !== blendRecord.blendEquation) {
			context.blendEquation((0, _RendererUtils.getGLBlendParam)(context, blendEquation));
			blendRecord.blendEquation = blendEquation;
		}

		if (blendSrc !== blendRecord.blendSrc || blendDst !== blendRecord.blendDst) {
			context.blendFunc((0, _RendererUtils.getGLBlendParam)(context, blendSrc), (0, _RendererUtils.getGLBlendParam)(context, blendDst));
...
Renderer.prototype.setBoundBuffer = function (buffer, target) {
	...
	if (!targetBuffer.valid || targetBuffer.buffer !== buffer) {
		this.context.bindBuffer((0, _RendererUtils.getGLBufferTarget)(this.context, target), buffer);
...
Renderer.prototype.bindVertexAttribute = function (attribIndex, attribute) {
	...
	if (hashKey !== attribute.hashKey) {
		this.context.vertexAttribPointer(attribIndex, attribute.count, (0, _RendererUtils.getGLDataType)(this.context, attribute.type), attribute.normalized, attribute.stride, attribute.offset);
...
Renderer.prototype.setRenderTarget = function (renderTarget) {
	...
	if (renderTarget && !renderTarget._glFrameBuffer) {
		...
		var isTargetPowerOfTwo = _MathUtils.MathUtils.isPowerOfTwo(renderTarget.width) && _MathUtils.MathUtils.isPowerOfTwo(renderTarget.height);
		var glFormat = (0, _RendererUtils.getGLInternalFormat)(context, renderTarget.format);
		var glType = (0, _RendererUtils.getGLDataType)(context, renderTarget.type);
