...
exports.Renderer = exports.mainCamera = undefined;
...
function Renderer(parameters) {
  ...
  */
	...
	_SystemBus.SystemBusjs.addListener('goo.setClearColor', function (color) {
	...
	_SystemBus.SystemBusjs.addListener('goo.setCurrentCamera', function (newCam) {
		exports.mainCamera = mainCamera = newCam.camera;
...
Renderer.prototype._setupContextLost = function () {
	this.domElement.addEventListener('webglcontextlost', function (event) {
		...
		_SystemBus.SystemBusjs.emit('goo.contextLost');
	}, false);

	this.domElement.addEventListener('webglcontextrestored', function () {
		this._restoreContext();
		_SystemBus.SystemBusjs.emit('goo.contextRestored');
	}.bind(this), false);
};
...
exports.mainCamera = mainCamera = null;
...
Renderer.prototype.setViewport = function (x, y, width, height) {
	...
	_SystemBus.SystemBusjs.emit('goo.viewportResize', {
		x: this.viewportX,
		y: this.viewportY,
...
Renderer.prototype.preloadTexture = function (context, texture) {
	...
	if (texture.variant === '2D') {
		...
		if (image && !image.isData && (texture.generateMipmaps || image.width > this.maxCubemapSize || image.height > this.maxCubemapSize)) {
			...
			texture.image.width = Math.min(this.maxCubemapSize, (0, _MathUtils.nearestPowerOfTwo)(texture.image.width));
			texture.image.height = Math.min(this.maxCubemapSize, (0, _MathUtils.nearestPowerOfTwo)(texture.image.height));
			image = texture.image;
		}
...
Renderer.prototype.render = function (renderList, camera, lights, renderTarget, clear, overrideMaterials) {
	...
	if (!camera) {
		...
		exports.mainCamera = mainCamera = camera;
	}
	...
	if (renderTarget && renderTarget.generateMipmaps && (0, _MathUtils.isPowerOfTwo)(renderTarget.width) && (0, _MathUtils.isPowerOfTwo)(renderTarget.height)) {
...
Renderer.prototype.renderToPick = function (renderList, camera, clear, skipUpdateBuffer, doScissor, clientX, clientY, customPickingMaterial, skipOverride) {
	...
	if (this.hardwarePicking === null) {
		var pickingMaterial = _Material.Material.createEmptyMaterial(_ShaderLib.ShaderLib.pickingShader, 'pickingMaterial');
...
Renderer.prototype.updateTextures = function (material) {
	...
	for (var i = 0; i < textureSlots.length; i++) {
		...
		for (var j = 0; j < textureList.length; j++) {
			...
			var isTexturePowerOfTwo = (0, _MathUtils.isPowerOfTwo)(imageObject.width) && (0, _MathUtils.isPowerOfTwo)(imageObject.height);
...
Renderer.prototype.setRenderTarget = function (renderTarget) {
	...
	if (renderTarget && !renderTarget._glFrameBuffer) {
		...
		var isTargetPowerOfTwo = (0, _MathUtils.isPowerOfTwo)(renderTarget.width) && (0, _MathUtils.isPowerOfTwo)(renderTarget.height);
