...
exports.pickingShader = exports.lightDepth = exports.downsample = exports.normalmap = exports.particles = exports.showNormals = exports.convolution = exports.texturedLit = exports.textured = exports.simpleLit = exports.simpleColored = exports.simple = exports.copyPure = exports.copy = exports.screenCopy = exports.uber = undefined;
...
var functionObject_pickingShader;
var functionObject_lightDepth;
var functionObject_downsample;
var functionObject_point;
var functionObject_normalmap;
var functionObject_particles;
var functionObject_showNormals;
var functionObject_convolution;
var functionObject_texturedLit;
var functionObject_textured;
var functionObject_simpleLit;
var functionObject_simpleColored;
var functionObject_simple;
var functionObject_copyPure;
var functionObject_copy;
var functionObject_screenCopy;
...
exports.uber = functionObject_uber = {
    processors: [ShaderBuilder.uber.processor, ShaderBuilder.light.processor, ShaderBuilder.animation.processor],
...
exports.copy = functionObject_copy = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION,
        vertexUV0: _MeshData.MeshData.TEXCOORD0
    },
...
exports.copyPure = functionObject_copyPure = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION,
        vertexUV0: _MeshData.MeshData.TEXCOORD0
    },
...
exports.simple = functionObject_simple = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION
    },
...
exports.simpleColored = functionObject_simpleColored = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION
    },
...
exports.simpleLit = functionObject_simpleLit = {
    processors: [ShaderBuilder.light.processor],
    ...
    vshader: function vshader() {
        return ["attribute vec3 vertexPosition;", "attribute vec3 vertexNormal;", "uniform mat4 viewProjectionMatrix;", "uniform mat4 worldMatrix;", "uniform vec3 cameraPosition;", ShaderBuilder.light.prevertex, "varying vec3 normal;", "varying vec3 vWorldPos;", "varying vec3 viewPosition;", "void main(void) {", "vec4 worldPos = worldMatrix * vec4(vertexPosition, 1.0);", "vWorldPos = worldPos.xyz;", "gl_Position = viewProjectionMatrix * worldPos;", ShaderBuilder.light.vertex, "normal = (worldMatrix * vec4(vertexNormal, 0.0)).xyz;", "viewPosition = cameraPosition - worldPos.xyz;", "}"].join("\n");
    },

    fshader: function fshader() {
        return ["#ifdef SPECULAR_MAP", "uniform sampler2D specularMap;", "#ifdef TEXCOORD0", "varying vec2 texCoord0;", "#endif", "#endif", "uniform float opacity;", ShaderBuilder.light.prefragment, "#ifdef NORMAL", "varying vec3 normal;", "#endif", "varying vec3 vWorldPos;", "varying vec3 viewPosition;", "void main(void)", "{", "if (opacity == 0.0) {", "discard;",
        // 'return;',
        "}", "#ifdef NORMAL", "vec3 N = normalize(normal);", "#else", "vec3 N = vec3(0,0,1);", "#endif", "vec4 final_color = vec4(1.0);", ShaderBuilder.light.fragment, "final_color.a = opacity;", "gl_FragColor = final_color;", "}"].join("\n");
    }
};

exports.textured = functionObject_textured = {
    defines: {
        TEXCOORD0: true,
        DIFFUSE_MAP: true
    },
...
exports.texturedLit = functionObject_texturedLit = {
    processors: [ShaderBuilder.light.processor],
    ...
    vshader: function vshader() {
        return ["attribute vec3 vertexPosition;", "attribute vec3 vertexNormal;", "attribute vec2 vertexUV0;", "uniform mat4 viewProjectionMatrix;", "uniform mat4 worldMatrix;", "uniform vec3 cameraPosition;", ShaderBuilder.light.prevertex, "varying vec3 normal;", "varying vec3 vWorldPos;", "varying vec3 viewPosition;", "varying vec2 texCoord0;", "void main(void) {", "vec4 worldPos = worldMatrix * vec4(vertexPosition, 1.0);", "vWorldPos = worldPos.xyz;", "gl_Position = viewProjectionMatrix * worldPos;", ShaderBuilder.light.vertex, "normal = (worldMatrix * vec4(vertexNormal, 0.0)).xyz;", "texCoord0 = vertexUV0;", "viewPosition = cameraPosition - worldPos.xyz;", "}"].join("\n");
    },

    fshader: function fshader() {
        return ["uniform sampler2D diffuseMap;", ShaderBuilder.light.prefragment, "varying vec3 normal;", "varying vec3 vWorldPos;", "varying vec3 viewPosition;", "varying vec2 texCoord0;", "void main(void)", "{", "vec3 N = normalize(normal);", "vec4 final_color = texture2D(diffuseMap, texCoord0);", ShaderBuilder.light.fragment, "gl_FragColor = final_color;", "}"].join("\n");
    }
};

exports.convolution = functionObject_convolution = {
    defines: {
...
exports.showNormals = functionObject_showNormals = {
    defines: {
        NORMAL: true
    },
...
exports.particles = functionObject_particles = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION,
        vertexColor: _MeshData.MeshData.COLOR,
        vertexUV0: _MeshData.MeshData.TEXCOORD0
...
exports.normalmap = functionObject_normalmap = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION,
        vertexUV0: _MeshData.MeshData.TEXCOORD0
    },
...
exports.downsample = functionObject_downsample = {
    attributes: {
        vertexPosition: _MeshData.MeshData.POSITION,
        vertexUV0: _MeshData.MeshData.TEXCOORD0
    },
...
exports.lightDepth = functionObject_lightDepth = {
    ...
    vshader: ["attribute vec3 vertexPosition;", "uniform mat4 viewMatrix;", "uniform mat4 projectionMatrix;", "uniform mat4 worldMatrix;", "varying vec4 worldPosition;", ShaderBuilder.animation.prevertex, "void main(void) {", "mat4 wMatrix = worldMatrix;", ShaderBuilder.animation.vertex, "worldPosition = viewMatrix * (wMatrix * vec4(vertexPosition, 1.0));", "gl_Position = projectionMatrix * worldPosition;", "}"].join("\n"),

    fshader: ["#if SHADOW_TYPE == 2", "uniform float cameraScale;", "#endif", "varying vec4 worldPosition;", ShaderFragment.methods.packDepth, "void main(void)", "{", "#if SHADOW_TYPE == 0", "gl_FragColor = packDepth(gl_FragCoord.z);", "#elif SHADOW_TYPE == 1", "gl_FragColor = packDepth(gl_FragCoord.z);", "#elif SHADOW_TYPE == 2", "float linearDepth = length(worldPosition) * cameraScale;", "gl_FragColor = vec4(linearDepth, linearDepth * linearDepth, 0.0, 0.0);", "#endif", "}"].join("\n")
};

exports.pickingShader = functionObject_pickingShader = {
    defines: {
        WEIGHTS: true,
        JOINTIDS: true
    },
    ...
    ShaderBuilder.animation.processor, function (shader) {
        shader.setDefine("NORMAL", true);
    }],

    vshader: ["attribute vec3 vertexPosition;", "#ifdef NORMAL", "attribute vec3 vertexNormal;", "#endif", "uniform mat4 viewMatrix;", "uniform mat4 projectionMatrix;", "uniform mat4 worldMatrix;", "uniform float cameraFar;", "uniform float thickness;", "uniform mat3 normalMatrix;", ShaderBuilder.animation.prevertex, "varying float depth;", "void main() {", "#ifdef NORMAL", "mat3 nMatrix = normalMatrix;", "#endif", "mat4 wMatrix = worldMatrix;", ShaderBuilder.animation.vertex, "#ifdef NORMAL", "vec4 mvPosition = viewMatrix * (wMatrix * vec4( vertexPosition + vertexNormal * thickness, 1.0 ));", "#else", "vec4 mvPosition = viewMatrix * (wMatrix * vec4( vertexPosition, 1.0 ));", "#endif", "depth = -mvPosition.z / cameraFar;", "gl_Position = projectionMatrix * mvPosition;", "}"].join("\n"),

    fshader: ["uniform float id;", "varying float depth;", ShaderFragment.methods.packDepth16, "void main() {", "vec2 packedId = vec2(floor(id/255.0), mod(id, 255.0)) * vec2(1.0/255.0);", "vec2 packedDepth = packDepth16(depth);", "gl_FragColor = vec4(packedId, packedDepth);", "}"].join("\n")
};
