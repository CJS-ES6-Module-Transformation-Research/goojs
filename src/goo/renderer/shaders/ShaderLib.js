 ...
 */
ShaderLib.uber = {
	processors: [_ShaderBuilder2.default.uber.processor, _ShaderBuilder2.default.light.processor, _ShaderBuilder2.default.animation.processor],
	attributes: {
		vertexPosition: _MeshData2.default.POSITION,
		vertexNormal: _MeshData2.default.NORMAL,
		vertexTangent: _MeshData2.default.TANGENT,
		vertexColor: _MeshData2.default.COLOR,
		vertexUV0: _MeshData2.default.TEXCOORD0,
		vertexUV1: _MeshData2.default.TEXCOORD1,
	...
	uniforms: {
		...
		specularMap: _Shader2.default.SPECULAR_MAP,
		emissiveMap: _Shader2.default.EMISSIVE_MAP,
		aoMap: _Shader2.default.AO_MAP,
		lightMap: _Shader2.default.LIGHT_MAP,
...
ShaderLib.simpleLit = {
	processors: [_ShaderBuilder2.default.light.processor],
	defines: {
		NORMAL: true
	},
	attributes: {
	...
	vshader: function vshader() {
		return ['attribute vec3 vertexPosition;', 'attribute vec3 vertexNormal;', 'uniform mat4 viewProjectionMatrix;', 'uniform mat4 worldMatrix;', 'uniform vec3 cameraPosition;', _ShaderBuilder2.default.light.prevertex, 'varying vec3 normal;', 'varying vec3 vWorldPos;', 'varying vec3 viewPosition;', 'void main(void) {', 'vec4 worldPos = worldMatrix * vec4(vertexPosition, 1.0);', 'vWorldPos = worldPos.xyz;', 'gl_Position = viewProjectionMatrix * worldPos;', _ShaderBuilder2.default.light.vertex, 'normal = (worldMatrix * vec4(vertexNormal, 0.0)).xyz;', 'viewPosition = cameraPosition - worldPos.xyz;', '}'].join('\n');
	},
	fshader: function fshader() {
		return ['#ifdef SPECULAR_MAP', 'uniform sampler2D specularMap;', '#ifdef TEXCOORD0', 'varying vec2 texCoord0;', '#endif', '#endif', 'uniform float opacity;', _ShaderBuilder2.default.light.prefragment, '#ifdef NORMAL', 'varying vec3 normal;', '#endif', 'varying vec3 vWorldPos;', 'varying vec3 viewPosition;', 'void main(void)', '{', 'if (opacity == 0.0) {', 'discard;',
		// 'return;',
		'}', '#ifdef NORMAL', 'vec3 N = normalize(normal);', '#else', 'vec3 N = vec3(0,0,1);', '#endif', 'vec4 final_color = vec4(1.0);', _ShaderBuilder2.default.light.fragment, 'final_color.a = opacity;', 'gl_FragColor = final_color;', '}'].join('\n');
	}
};
...
ShaderLib.texturedLit = {
	...
	vshader: function vshader() {
		return ['attribute vec3 vertexPosition;', 'attribute vec3 vertexNormal;', 'attribute vec2 vertexUV0;', 'uniform mat4 viewProjectionMatrix;', 'uniform mat4 worldMatrix;', 'uniform vec3 cameraPosition;', _ShaderBuilder2.default.light.prevertex, 'varying vec3 normal;', 'varying vec3 vWorldPos;', 'varying vec3 viewPosition;', 'varying vec2 texCoord0;', 'void main(void) {', 'vec4 worldPos = worldMatrix * vec4(vertexPosition, 1.0);', 'vWorldPos = worldPos.xyz;', 'gl_Position = viewProjectionMatrix * worldPos;', _ShaderBuilder2.default.light.vertex, 'normal = (worldMatrix * vec4(vertexNormal, 0.0)).xyz;', 'texCoord0 = vertexUV0;', 'viewPosition = cameraPosition - worldPos.xyz;', '}'].join('\n');
	},
	fshader: function fshader() {
		return ['uniform sampler2D diffuseMap;', _ShaderBuilder2.default.light.prefragment, 'varying vec3 normal;', 'varying vec3 vWorldPos;', 'varying vec3 viewPosition;', 'varying vec2 texCoord0;', 'void main(void)', '{', 'vec3 N = normalize(normal);', 'vec4 final_color = texture2D(diffuseMap, texCoord0);', _ShaderBuilder2.default.light.fragment, 'gl_FragColor = final_color;', '}'].join('\n');
	}
};
...
ShaderLib.convolution = {
	...
	attributes: {
		position: _MeshData2.default.POSITION,
		uv: _MeshData2.default.TEXCOORD0
	},
	uniforms: {
		viewMatrix: _Shader2.default.VIEW_MATRIX,
		projectionMatrix: _Shader2.default.PROJECTION_MATRIX,
		worldMatrix: _Shader2.default.WORLD_MATRIX,
		tDiffuse: _Shader2.default.DIFFUSE_MAP,
...
ShaderLib.normalmap = {
	...
	uniforms: {
		viewProjectionMatrix: _Shader2.default.VIEW_PROJECTION_MATRIX,
		worldMatrix: _Shader2.default.WORLD_MATRIX,
		heightMap: _Shader2.default.DIFFUSE_MAP,
		resolution: [512, 512],
		height: 0.05
	},
...
ShaderLib.lightDepth = {
	...
	uniforms: {
		...
		projectionMatrix: _Shader2.default.PROJECTION_MATRIX,
		worldMatrix: _Shader2.default.WORLD_MATRIX,
		cameraScale: _Shader2.default.MAIN_DEPTH_SCALE
	},
	vshader: ['attribute vec3 vertexPosition;', 'uniform mat4 viewMatrix;', 'uniform mat4 projectionMatrix;', 'uniform mat4 worldMatrix;', 'varying vec4 worldPosition;', _ShaderBuilder2.default.animation.prevertex, 'void main(void) {', 'mat4 wMatrix = worldMatrix;', _ShaderBuilder2.default.animation.vertex, 'worldPosition = viewMatrix * (wMatrix * vec4(vertexPosition, 1.0));', 'gl_Position = projectionMatrix * worldPosition;', '}'].join('\n'),
	fshader: ['#if SHADOW_TYPE == 2', 'uniform float cameraScale;', '#endif', 'varying vec4 worldPosition;', _ShaderFragment2.default.methods.packDepth, 'void main(void)', '{', '#if SHADOW_TYPE == 0', 'gl_FragColor = packDepth(gl_FragCoord.z);', '#elif SHADOW_TYPE == 1', 'gl_FragColor = packDepth(gl_FragCoord.z);', '#elif SHADOW_TYPE == 2', 'float linearDepth = length(worldPosition) * cameraScale;', 'gl_FragColor = vec4(linearDepth, linearDepth * linearDepth, 0.0, 0.0);', '#endif', '}'].join('\n')
};
...
ShaderLib.pickingShader = {
	...
	uniforms: {
		...
		projectionMatrix: _Shader2.default.PROJECTION_MATRIX,
		worldMatrix: _Shader2.default.WORLD_MATRIX,
		cameraFar: _Shader2.default.FAR_PLANE,
		thickness: 0.0,
	...
	_ShaderBuilder2.default.animation.processor, function (shader) {
		shader.setDefine('NORMAL', true);
	}],
	vshader: ['attribute vec3 vertexPosition;', '#ifdef NORMAL', 'attribute vec3 vertexNormal;', '#endif', 'uniform mat4 viewMatrix;', 'uniform mat4 projectionMatrix;', 'uniform mat4 worldMatrix;', 'uniform float cameraFar;', 'uniform float thickness;', 'uniform mat3 normalMatrix;', _ShaderBuilder2.default.animation.prevertex, 'varying float depth;', 'void main() {', '#ifdef NORMAL', 'mat3 nMatrix = normalMatrix;', '#endif', 'mat4 wMatrix = worldMatrix;', _ShaderBuilder2.default.animation.vertex, '#ifdef NORMAL', 'vec4 mvPosition = viewMatrix * (wMatrix * vec4( vertexPosition + vertexNormal * thickness, 1.0 ));', '#else', 'vec4 mvPosition = viewMatrix * (wMatrix * vec4( vertexPosition, 1.0 ));', '#endif', 'depth = -mvPosition.z / cameraFar;', 'gl_Position = projectionMatrix * mvPosition;', '}'].join('\n'),
	fshader: ['uniform float id;', 'varying float depth;', _ShaderFragment2.default.methods.packDepth16, 'void main() {', 'vec2 packedId = vec2(floor(id/255.0), mod(id, 255.0)) * vec2(1.0/255.0);', 'vec2 packedDepth = packDepth16(depth);', 'gl_FragColor = vec4(packedId, packedDepth);', '}'].join('\n')
};
