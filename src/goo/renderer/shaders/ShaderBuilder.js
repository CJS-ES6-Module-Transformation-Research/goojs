...
import { TangentGenerator as TangentGenerator_TangentGeneratorjs } from "../../util/TangentGenerator";
import { ShaderFragment as ShaderFragment_ShaderFragmentjs } from "../../renderer/shaders/ShaderFragment";
function ShaderBuilder() {}

var defaultLight = new DirectionalLight_DirectionalLightjs();
...
ShaderBuilder.uber = {
	...
	normalTangents: function (shader, shaderInfo) {
		//TODO: Hacky?
		if (shader.hasDefine('NORMAL') && shader.hasDefine('NORMAL_MAP') && !shaderInfo.meshData.getAttributeBuffer(MeshData_MeshDatajs.TANGENT)) {
			TangentGenerator_TangentGeneratorjs.addTangentBuffer(shaderInfo.meshData);
		}
	},
...
ShaderBuilder.light = {
	...
	spotLight: function (light, uniforms, spotIndex) {
		...
		uniform[ind + 12] = Math.cos(light.angle * MathUtils_MathUtilsjs.DEG_TO_RAD / 2);
		uniform[ind + 13] = light.penumbra !== undefined ? Math.sin(light.penumbra * MathUtils_MathUtilsjs.DEG_TO_RAD / 4) : 0;
	...
	shadows: function (light, uniforms, i, shader, shaderInfo, shadowIndex) {
		var useLightCookie = light.lightCookie instanceof Texture_Texturejs;
	...
	builder: function (shader, shaderInfo) {
		...
		if (lights.length > 0) {
			...
			for (var i = 0; i < lights.length; i++) {
				...
				if (light.lightCookie instanceof Texture_Texturejs || light.shadowCaster) {
					shadowIndex++;
				}
			}
			if (pointIndex > 0) {
