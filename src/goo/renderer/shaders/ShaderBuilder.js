...
var functionObject_defaultLight = defaultLight;

ShaderBuilder.SKYBOX = null;
var functionObject_SKYBOX = null;
ShaderBuilder.SKYSPHERE = null;
var functionObject_SKYSPHERE = null;
ShaderBuilder.ENVIRONMENT_TYPE = 0;
var functionObject_ENVIRONMENT_TYPE = 0;
ShaderBuilder.GLOBAL_AMBIENT = [0, 0, 0];
var functionObject_GLOBAL_AMBIENT = [0, 0, 0];
ShaderBuilder.CLEAR_COLOR = [0.3, 0.3, 0.3, 1];
var functionObject_CLEAR_COLOR = [0.3, 0.3, 0.3, 1];
ShaderBuilder.USE_FOG = false;
var functionObject_USE_FOG = false;
ShaderBuilder.FOG_SETTINGS = [0, 10000];
var functionObject_FOG_SETTINGS = [0, 10000];
ShaderBuilder.FOG_COLOR = [1, 1, 1];

var functionObject_FOG_COLOR = [1, 1, 1];
...
ShaderBuilder.light = {
	...
	builder: function (shader, shaderInfo) {
        ...
        var functionObject_prevertex = prevertex.join("\n");
        ShaderBuilder.light.vertex = vertex.join('\n');
        var functionObject_vertex = vertex.join("\n");
        ShaderBuilder.light.prefragment = prefragment.join('\n');
        var functionObject_prefragment = prefragment.join("\n");
        ShaderBuilder.light.fragment = fragment.join('\n');
        var functionObject_fragment = fragment.join("\n");
    }
};
	...
	processor: function (shader, shaderInfo) {
		...
		if (pose) {
			if (!shader.uniforms.jointPalette) {
                ...
                var functionObject_jointPalette = ShaderBuilder.animation.jointPalette;
            }
			...
			for (var index = 0; index < skMesh.paletteMap.length; index++) {
				...
				if (refMat) {
					for (var i = 0; i < 12; i++) {
                        ...
                        var functionObject_i = refMat.data[ShaderBuilder.animation.order[i]];
                    }
				}
				/*
