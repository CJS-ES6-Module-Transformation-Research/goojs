...
var ShaderBuilder_animation;
var ShaderBuilder_light;
var ShaderBuilder_uniforms;
var ShaderBuilder_uber;
var ShaderBuilder_FOG_COLOR;
var ShaderBuilder_FOG_SETTINGS;
var ShaderBuilder_USE_FOG;
var ShaderBuilder_CLEAR_COLOR;
var ShaderBuilder_GLOBAL_AMBIENT;
var ShaderBuilder_ENVIRONMENT_TYPE;
var ShaderBuilder_SKYSPHERE;
var ShaderBuilder_SKYBOX;
var ShaderBuilder_defaultLight;
...
var defaultLight = new rendererlightDirectionalLight_DirectionalLightjs();
defaultLight.translation.setDirect(10, 10, 10);
defaultLight.direction.setDirect(1, 1, 1).normalize();
ShaderBuilder_defaultLight = defaultLight;;

ShaderBuilder_SKYBOX = null;;
ShaderBuilder_SKYSPHERE = null;;
ShaderBuilder_ENVIRONMENT_TYPE = 0;;
ShaderBuilder_GLOBAL_AMBIENT = [0, 0, 0];;
ShaderBuilder_CLEAR_COLOR = [0.3, 0.3, 0.3, 1];;
ShaderBuilder_USE_FOG = false;;
ShaderBuilder_FOG_SETTINGS = [0, 10000];;
ShaderBuilder_FOG_COLOR = [1, 1, 1];;
...
ShaderBuilder_uber = {
    ...
    reflectivity: function(shader, material) {
        ...
        shader.setDefine(
            "REFLECTION_TYPE",
            material.uniforms.reflectionType !== undefined ? material.uniforms.reflectionType : 0
        );
    },
    ...
    sky: function(shader, material) {
        if (material.getTexture("LOCAL_ENVIRONMENT")) {
            ...
            if (ShaderBuilder_SKYBOX && (material.uniforms.reflectivity || material.uniforms.refractivity)) {
                material.setTexture("ENVIRONMENT_CUBE", ShaderBuilder_SKYBOX);
            } else if (material.getTexture("ENVIRONMENT_CUBE")) {
                material.removeTexture("ENVIRONMENT_CUBE");
            }
            if (ShaderBuilder_SKYSPHERE && (material.uniforms.reflectivity || material.uniforms.refractivity)) {
                material.setTexture("ENVIRONMENT_SPHERE", ShaderBuilder_SKYSPHERE);
                shader.setDefine("ENVIRONMENT_TYPE", ShaderBuilder_ENVIRONMENT_TYPE);
    ...
    fog: function(shader) {
        // fog
        if (ShaderBuilder_USE_FOG) {
            shader.setDefine("FOG", true);
            ShaderBuilder_uniforms = ShaderBuilder_FOG_SETTINGS;;
            shader.uniforms.fogColor = ShaderBuilder_FOG_COLOR;
        } else {
    ...
    normalTangents: function(shader, shaderInfo) {
        //TODO: Hacky?
        if (shader.hasDefine("NORMAL") && shader.hasDefine("NORMAL_MAP") && !shaderInfo.meshData.getAttributeBuffer(rendererMeshData_MeshDatajs.TANGENT)) {
    ...
    processor: function(shader, shaderInfo) {
        ...
        shader.uniforms.clearColor = ShaderBuilder_CLEAR_COLOR;

        ShaderBuilder_uber.reflectivity(shader, material);
        ShaderBuilder_uber.sky(shader, material);


        ShaderBuilder_uber.defines(shader, attributeMap);
        ShaderBuilder_uber.txMaps(shader, textureMaps);


        ShaderBuilder_uber.uniforms(shader, textureMaps);
        ShaderBuilder_uber.attributes(shader, attributeMap, textureMaps);

        ShaderBuilder_uber.discard(shader, material);
        ShaderBuilder_uber.opacity(shader, material);

        ShaderBuilder_uber.fog(shader);

        shader.setDefine("SKIP_SPECULAR", true);
        ShaderBuilder_uber.normalTangents(shader, shaderInfo);
    }
};;
...
ShaderBuilder_light = {
    ...
    spotLight: function(light, uniforms, spotIndex) {
        ...
        uniform[ind + 12] = Math.cos(light.angle * MathUtilsjs_DEG_TO_RAD / 2);
        uniform[ind + 13] = light.penumbra !== undefined ? Math.sin(light.penumbra * MathUtilsjs_DEG_TO_RAD / 4) : 0;
    ...
    shadows: function(light, uniforms, i, shader, shaderInfo, shadowIndex) {
        var useLightCookie = light.lightCookie instanceof rendererTexture_Texturejs;
        ...
        if ((useLightCookie || light.shadowCaster) && light.shadowSettings.shadowData) {
            ...
            if (light.shadowCaster) {
                ...
                lightDefines.push(
                    "H",
                    light.shadowSettings.shadowType === "PCF" ? 1 : light.shadowSettings.shadowType === "VSM" ? 2 : 0
                );
            }
    ...
    processor: function(shader, shaderInfo) {
        ...
        if (shaderInfo.material.multiplyAmbient) {
            totalAmbient[0] = materialAmbient[0] * ShaderBuilder_GLOBAL_AMBIENT[0];
            totalAmbient[1] = materialAmbient[1] * ShaderBuilder_GLOBAL_AMBIENT[1];
            totalAmbient[2] = materialAmbient[2] * ShaderBuilder_GLOBAL_AMBIENT[2];
        } else {
            totalAmbient[0] = materialAmbient[0] + ShaderBuilder_GLOBAL_AMBIENT[0];
            totalAmbient[1] = materialAmbient[1] + ShaderBuilder_GLOBAL_AMBIENT[1];
            totalAmbient[2] = materialAmbient[2] + ShaderBuilder_GLOBAL_AMBIENT[2];
        ...
        if (lights.length > 0) {
            for (var i = 0; i < lights.length; i++) {
                ...
                if (light instanceof rendererlightPointLight_PointLightjs) {
                    ShaderBuilder_light.pointLight(light, uniforms, pointIndex);
                    pointIndex++;
                } else if (light instanceof rendererlightDirectionalLight_DirectionalLightjs) {
                    ShaderBuilder_light.directionalLight(light, uniforms, directionalIndex);
                    directionalIndex++;
                } else if (light instanceof rendererlightSpotLight_SpotLightjs) {
                    ShaderBuilder_light.spotLight(light, uniforms, spotIndex);
                    spotIndex++;
                }

                shadowIndex = ShaderBuilder_light.shadows(light, uniforms, i, shader, shaderInfo, shadowIndex);
            }
    ...
    builder: function(shader, shaderInfo) {
        ...
        if (lights.length > 0) {
            ...
            for (var i = 0; i < lights.length; i++) {
                ...
                if (light.lightCookie instanceof rendererTexture_Texturejs || light.shadowCaster) {
                    shadowIndex++;
                }
            }
            if (pointIndex > 0) {
        ...
        ShaderBuilder_light.prevertex = prevertex.join("\n");
        ShaderBuilder_light.vertex = vertex.join("\n");
        ShaderBuilder_light.prefragment = prefragment.join("\n");
        ShaderBuilder_light.fragment = fragment.join("\n");
    }
};;

ShaderBuilder_animation = {
    processor: function(shader, shaderInfo) {
        var pose = shaderInfo.currentPose;
        if (pose) {
            if (!shader.uniforms.jointPalette) {
                shader.uniforms.jointPalette = ShaderBuilder_animation.jointPalette;
            }
    ...
    jointPalette: function(shaderInfo) {
        ...
        if (pose) {
            ...
            for (var index = 0; index < skMesh.paletteMap.length; index++) {
                ...
                if (refMat) {
                    for (var i = 0; i < 12; i++) {
                        store[index * 12 + i] = refMat.data[ShaderBuilder_animation.order[i]];
                    }
                }
                /*
...
export { ShaderBuilder_SKYBOX as SKYBOX, ShaderBuilder_SKYSPHERE as SKYSPHERE, ShaderBuilder_GLOBAL_AMBIENT as GLOBAL_AMBIENT, ShaderBuilder_CLEAR_COLOR as CLEAR_COLOR, ShaderBuilder_USE_FOG as USE_FOG, ShaderBuilder_FOG_SETTINGS as FOG_SETTINGS, ShaderBuilder_FOG_COLOR as FOG_COLOR, ShaderBuilder_uber as uber, ShaderBuilder_light as light, ShaderBuilder_animation as animation, ShaderBuilder };