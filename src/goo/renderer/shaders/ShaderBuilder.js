...
var defaultLight = new _DirectionalLight2.default();
...
ShaderBuilder.uber = {
	...
	normalTangents: function normalTangents(shader, shaderInfo) {
		//TODO: Hacky?
		if (shader.hasDefine('NORMAL') && shader.hasDefine('NORMAL_MAP') && !shaderInfo.meshData.getAttributeBuffer(_MeshData2.default.TANGENT)) {
			_TangentGenerator2.default.addTangentBuffer(shaderInfo.meshData);
		}
	},
...
ShaderBuilder.light = {
	...
	spotLight: function spotLight(light, uniforms, spotIndex) {
		...
		uniform[ind + 12] = Math.cos(light.angle * _MathUtils2.default.DEG_TO_RAD / 2);
		uniform[ind + 13] = light.penumbra !== undefined ? Math.sin(light.penumbra * _MathUtils2.default.DEG_TO_RAD / 4) : 0;
	...
	shadows: function shadows(light, uniforms, i, shader, shaderInfo, shadowIndex) {
		var useLightCookie = light.lightCookie instanceof _Texture2.default;
	...
	builder: function builder(shader, shaderInfo) {
		...
		if (lights.length > 0) {
			...
			for (var i = 0; i < lights.length; i++) {
				...
				if (light.lightCookie instanceof _Texture2.default || light.shadowCaster) {
					shadowIndex++;
				}
			}
			if (pointIndex > 0) {
			...
			if (shadowIndex > 0) {
				prefragment.push(_ShaderFragment2.default.methods.unpackDepth, 'uniform vec4 shadowData[' + shadowIndex * 2 + '];', 'float texture2DCompare(sampler2D depths, vec2 uv, float compare) {', 'return step(unpackDepth(texture2D(depths, uv)), compare);', '}');
			}
