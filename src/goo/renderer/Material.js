...
Material.prototype.clone = function (options) {
	...
	if (options.shareUniforms) {
		...
		clone.uniforms = utilObjectUtils_ObjectUtilsjs.deepClone(this.uniforms);
	}
...
Material.createShader = function (shaderDefinition, name) {
	...
	var shader = new rendererShader_Shaderjs(name || null, shaderDefinition);
	if (shader.name === null) {
 ...
 */
export { Material_Material as Material };
