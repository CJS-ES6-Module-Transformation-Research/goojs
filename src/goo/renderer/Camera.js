...
import { BoundingVolume as BoundingVolume_BoundingVolumejs } from "../renderer/bounds/BoundingVolume";
...
function Camera(fov, aspect, near, far) {
	...
	this.translation = new Vector3_Vector3js(0, 0, 0);
	this._left = new Vector3_Vector3js(-1, 0, 0);
	this._up = new Vector3_Vector3js(0, 1, 0);
	this._direction = new Vector3_Vector3js(0, 0, -1);
	...
	this._coeffLeft = new Vector2_Vector2js();
	this._coeffRight = new Vector2_Vector2js();
	this._coeffBottom = new Vector2_Vector2js();
	this._coeffTop = new Vector2_Vector2js();
	...
	for (var i = 0; i < Camera.FRUSTUM_PLANES; i++) {
		this._worldPlane[i] = new Plane_Planejs();
	}
	...
	this.modelView = new Matrix4_Matrix4js();
	this.modelViewInverse = new Matrix4_Matrix4js();
	this.projection = new Matrix4_Matrix4js();
	this.modelViewProjection = new Matrix4_Matrix4js();
	this.modelViewProjectionInverse = new Matrix4_Matrix4js();

	//! AT: unused?
	this._planeState = 0;
	this._clipPlane = new Vector4_Vector4js();
	this._qCalc = new Vector4_Vector4js();

	this._corners = [];
	for (var i = 0; i < 8; i++) {
		this._corners.push(new Vector3_Vector3js());
	}
	this._extents = new Vector3_Vector3js();

	// Temp decl
	this.vNearPlaneCenter = new Vector3_Vector3js();
	this.vFarPlaneCenter = new Vector3_Vector3js();

	this.calcLeft = new Vector3_Vector3js();
	this.calcUp = new Vector3_Vector3js();
...
var newDirection = new Vector3_Vector3js(); // tmp
...
Camera.prototype.setFrustumPerspective = function (fov, aspect, near, far) {
	...
	if (this.fov !== undefined) {
		var h = Math.tan(this.fov * MathUtils_MathUtilsjs.DEG_TO_RAD * 0.5) * this.near;
		var w = h * this.aspect;
		this._frustumLeft = -w;
		...
		if (this._frustumFar - this._frustumNear < MathUtils_MathUtilsjs.EPSILON) {
			this._frustumFar = this._frustumNear + MathUtils_MathUtilsjs.EPSILON;
		}
...
Camera.prototype.lookAt = function (pos, worldUpVector) {
	...
	if (this._up.equals(Vector3_Vector3js.ZERO)) {
		this._up.set(Vector3_Vector3js.UNIT_Y);
	}
	this._left.set(this._up).cross(this._direction).normalize();
	if (this._left.equals(Vector3_Vector3js.ZERO)) {
...
Camera.prototype.contains = function (bound) {
	...
	for (var planeCounter = Camera.FRUSTUM_PLANES - 1; planeCounter >= 0; planeCounter--) {
		switch (bound.whichSide(this._worldPlane[planeCounter])) {
			case BoundingVolume_BoundingVolumejs.Inside:
				return Camera.Outside;
			case BoundingVolume_BoundingVolumejs.Outside:
				break;
			case BoundingVolume_BoundingVolumejs.Intersects:
...
Camera.prototype.getWorldPosition = function (screenX, screenY, screenWidth, screenHeight, zDepth, store) {
	if (this.projectionMode === Camera.Parallel) {
		...
		zDepth = MathUtils_MathUtilsjs.clamp(zDepth, this.near, this.far);
...
Camera.prototype.getWorldCoordinates = function (screenX, screenY, screenWidth, screenHeight, zDepth, store) {
	if (!store) {
		store = new Vector3_Vector3js();
	}
	this.checkInverseModelViewProjection();
	var position = new Vector4_Vector4js();
...
Camera.prototype.pack = function (sceneBounds) {
	...
	if (sceneBounds instanceof BoundingBox_BoundingBoxjs) {
		extents.setDirect(sceneBounds.xExtent, sceneBounds.yExtent, sceneBounds.zExtent);
	} else if (sceneBounds instanceof BoundingSphere_BoundingSpherejs) {
...
Camera.prototype.setToObliqueMatrix = function (clipPlane) {
	...
	this._qCalc.setDirect(
		(MathUtils_MathUtilsjs.sign(transformedClipPlane.x) + projection[8]) / projection[0],
		(MathUtils_MathUtilsjs.sign(transformedClipPlane.y) + projection[9]) / projection[5],
		-1,
