...
Camera.prototype.setFrustumPerspective = function (fov, aspect, near, far) {
	...
	if (this.fov !== undefined) {
		var h = Math.tan(this.fov * MathUtils_MathUtils.DEG_TO_RAD * 0.5) * this.near;
		var w = h * this.aspect;
		this._frustumLeft = -w;
		...
		if (this._frustumFar - this._frustumNear < MathUtils_MathUtils.EPSILON) {
			this._frustumFar = this._frustumNear + MathUtils_MathUtils.EPSILON;
		}
...
Camera.prototype.contains = function (bound) {
	...
	for (var planeCounter = Camera.FRUSTUM_PLANES - 1; planeCounter >= 0; planeCounter--) {
		switch (bound.whichSide(this._worldPlane[planeCounter])) {
			case BoundingVolume_BoundingVolume.Inside:
				return Camera.Outside;
			case BoundingVolume_BoundingVolume.Outside:
				break;
			case BoundingVolume_BoundingVolume.Intersects:
...
Camera.prototype.getWorldPosition = function (screenX, screenY, screenWidth, screenHeight, zDepth, store) {
	if (this.projectionMode === Camera.Parallel) {
		...
		zDepth = MathUtils_MathUtils.clamp(zDepth, this.near, this.far);
...
Camera.prototype.setToObliqueMatrix = function (clipPlane) {
	...
	this._qCalc.setDirect(
		(MathUtils_MathUtils.sign(transformedClipPlane.x) + projection[8]) / projection[0],
		(MathUtils_MathUtils.sign(transformedClipPlane.y) + projection[9]) / projection[5],
		-1,
