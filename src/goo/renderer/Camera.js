...
Camera.prototype.setFrustumPerspective = function (fov, aspect, near, far) {
	...
	if (this.fov !== undefined) {
		var h = Math.tan(this.fov * _MathUtils.MathUtils.DEG_TO_RAD * 0.5) * this.near;
		var w = h * this.aspect;
		this._frustumLeft = -w;
		...
		if (this._frustumFar - this._frustumNear < _MathUtils.MathUtils.EPSILON) {
			this._frustumFar = this._frustumNear + _MathUtils.MathUtils.EPSILON;
		}
...
Camera.prototype.contains = function (bound) {
	...
	for (var planeCounter = Camera.FRUSTUM_PLANES - 1; planeCounter >= 0; planeCounter--) {
		switch (bound.whichSide(this._worldPlane[planeCounter])) {
			case _BoundingVolume.BoundingVolume.Inside:
				return Camera.Outside;
			case _BoundingVolume.BoundingVolume.Outside:
				break;
			case _BoundingVolume.BoundingVolume.Intersects:
...
Camera.prototype.getWorldPosition = function (screenX, screenY, screenWidth, screenHeight, zDepth, store) {
	if (this.projectionMode === Camera.Parallel) {
		...
		zDepth = _MathUtils.MathUtils.clamp(zDepth, this.near, this.far);
...
Camera.prototype.setToObliqueMatrix = function (clipPlane) {
	...
	this._qCalc.setDirect((_MathUtils.MathUtils.sign(transformedClipPlane.x) + projection[8]) / projection[0], (_MathUtils.MathUtils.sign(transformedClipPlane.y) + projection[9]) / projection[5], -1, (1.0 + projection[10]) / projection[14]);
