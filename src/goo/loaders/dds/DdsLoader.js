...
DdsHeader.read = function (data) {
	...
	if (_DdsUtils2.default.isSet(header.dwCaps, DdsHeader.DDSCAPS_MIPMAP)) {
		if (!_DdsUtils2.default.isSet(header.dwFlags, DdsHeader.DDSD_MIPMAPCOUNT)) {
...
DdsLoader.updateDepth = function (image, info) {
	if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP)) {
		var depth = 0;
		if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_POSITIVEX)) {
			depth++;
		}
		if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_NEGATIVEX)) {
			depth++;
		}
		if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_POSITIVEY)) {
			depth++;
		}
		if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_NEGATIVEY)) {
			depth++;
		}
		if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_POSITIVEZ)) {
			depth++;
		}
		if (_DdsUtils2.default.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_NEGATIVEZ)) {
			depth++;
		}
...
DdsLoader.readDXT = function (imgData, totalSize, info, texture) {
	...
	for (var mip = 0; mip < info.header.dwMipMapCount; mip++) {
		...
		var flipped = _DdsUtils2.default.flipDXT(data, mipWidth, mipHeight, texture.format);
...
DdsLoader.readUncompressed = function (imgData, totalSize, useRgb, useLum, useAlpha, useAlphaPixels, info, texture) {
	var redLumShift = _DdsUtils2.default.shiftCount(info.header.ddpf.dwRBitMask);
	var greenShift = _DdsUtils2.default.shiftCount(info.header.ddpf.dwGBitMask);
	var blueShift = _DdsUtils2.default.shiftCount(info.header.ddpf.dwBBitMask);
	var alphaShift = _DdsUtils2.default.shiftCount(info.header.ddpf.dwABitMask);

	var sourcebytesPP = ~~(info.header.ddpf.dwRGBBitCount / 8);
	var targetBytesPP = _DdsUtils2.default.getComponents(texture.format) * 1; // 1 byte per unsignedbyte store
	...
	for (var mip = 0; mip < info.header.dwMipMapCount; mip++) {
		for (var y = 0; y < mipHeight; y++) {
			for (var x = 0; x < mipWidth; x++) {
				...
				i = _DdsUtils2.default.getIntFromBytes(b);
...
DdsLoader.populate = function (texture, info, data) {
	...
	var compressedFormat = _DdsUtils2.default.isSet(flags, DdsPixelFormat.DDPF_FOURCC);
	var rgb = _DdsUtils2.default.isSet(flags, DdsPixelFormat.DDPF_RGB);
	var alphaPixels = _DdsUtils2.default.isSet(flags, DdsPixelFormat.DDPF_ALPHAPIXELS);
	var lum = _DdsUtils2.default.isSet(flags, DdsPixelFormat.DDPF_LUMINANCE);
	var alpha = _DdsUtils2.default.isSet(flags, DdsPixelFormat.DDPF_ALPHA);
	texture.type = 'UnsignedByte';

	if (compressedFormat) {
		var fourCC = info.header.ddpf.dwFourCC;
		// DXT1 format
		if (fourCC === _DdsUtils2.default.getIntFromString('DXT1')) {
			info.bpp = 4;
		...
		// DXT3 format
		else if (fourCC === _DdsUtils2.default.getIntFromString('DXT3')) {
			...
			// DXT5 format
			else if (fourCC === _DdsUtils2.default.getIntFromString('DXT5')) {
				...
				// DXT10 info present...
				else if (fourCC === _DdsUtils2.default.getIntFromString('DX10')) {
					...
					else if (fourCC === _DdsUtils2.default.getIntFromString('DXT2')) {
							throw new Error('DXT2 is not supported.');
						}

						// DXT4 format - unsupported
						else if (fourCC === _DdsUtils2.default.getIntFromString('DXT4')) {
...
DdsLoader.prototype.load = function (buffer, tex, flipped, arrayByteOffset, arrayByteLength) {
	...
	if (dwMagic !== _DdsUtils2.default.getIntFromString('DDS ')) {
	...
	info.headerDX10 = info.header.ddpf.dwFourCC === _DdsUtils2.default.getIntFromString('DX10') ? DdsHeader.read(Int32Array.create(buffer, arrayByteOffset + 128, 5)) : null;
