import { DdsUtils as DdsUtils_DdsUtilsjs } from "../../loaders/dds/DdsUtils";
import { Capabilities as Capabilities_Capabilitiesjs } from "../../renderer/Capabilities";
var DdsLoader_populate;
var DdsLoader_readUncompressed;
var DdsLoader_readDXT;
var DdsLoader_updateDepth;
...
DdsHeader.read = function (data) { // Int32Array
	...
	if (DdsUtils_DdsUtilsjs.isSet(header.dwCaps, DdsHeader.DDSCAPS_MIPMAP)) {
		if (!DdsUtils_DdsUtilsjs.isSet(header.dwFlags, DdsHeader.DDSD_MIPMAPCOUNT)) {
...
DdsLoader_updateDepth = function(image, info) {
    if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP)) {
        var depth = 0;
        if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_POSITIVEX)) {
            depth++;
        }
        if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_NEGATIVEX)) {
            depth++;
        }
        if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_POSITIVEY)) {
            depth++;
        }
        if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_NEGATIVEY)) {
            depth++;
        }
        if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_POSITIVEZ)) {
            depth++;
        }
        if (DdsUtils_DdsUtilsjs.isSet(info.header.dwCaps2, DdsHeader.DDSCAPS2_CUBEMAP_NEGATIVEZ)) {
            depth++;
        }
...
DdsLoader_readDXT = function(imgData, totalSize, info, texture) {
    ...
    for (var mip = 0; mip < info.header.dwMipMapCount; mip++) {
        ...
        var flipped = DdsUtils_DdsUtilsjs.flipDXT(data, mipWidth, mipHeight, texture.format);
...
DdsLoader_readUncompressed = function(
    imgData,
    totalSize,
    useRgb,
    useLum,
    ...
    var redLumShift = DdsUtils_DdsUtilsjs.shiftCount(info.header.ddpf.dwRBitMask);
    var greenShift = DdsUtils_DdsUtilsjs.shiftCount(info.header.ddpf.dwGBitMask);
    var blueShift = DdsUtils_DdsUtilsjs.shiftCount(info.header.ddpf.dwBBitMask);
    var alphaShift = DdsUtils_DdsUtilsjs.shiftCount(info.header.ddpf.dwABitMask);

    var sourcebytesPP = ~~(info.header.ddpf.dwRGBBitCount / 8);
    var targetBytesPP = DdsUtils_DdsUtilsjs.getComponents(texture.format) * 1; // 1 byte per unsignedbyte store
    ...
    for (var mip = 0; mip < info.header.dwMipMapCount; mip++) {
        for (var y = 0; y < mipHeight; y++) {
            for (var x = 0; x < mipWidth; x++) {
                ...
                i = DdsUtils_DdsUtilsjs.getIntFromBytes(b);
...
DdsLoader_populate = function(texture, info, data) {
    var flags = info.header.ddpf.dwFlags;

    var compressedFormat = DdsUtils_DdsUtilsjs.isSet(flags, DdsPixelFormat.DDPF_FOURCC);
    var rgb = DdsUtils_DdsUtilsjs.isSet(flags, DdsPixelFormat.DDPF_RGB);
    var alphaPixels = DdsUtils_DdsUtilsjs.isSet(flags, DdsPixelFormat.DDPF_ALPHAPIXELS);
    var lum = DdsUtils_DdsUtilsjs.isSet(flags, DdsPixelFormat.DDPF_LUMINANCE);
    var alpha = DdsUtils_DdsUtilsjs.isSet(flags, DdsPixelFormat.DDPF_ALPHA);
    texture.type = "UnsignedByte";

    if (compressedFormat) {
        var fourCC = info.header.ddpf.dwFourCC;
        // DXT1 format
        if (fourCC === DdsUtils_DdsUtilsjs.getIntFromString("DXT1")) {
            info.bpp = 4;
        ...
        // DXT3 format
        else if (fourCC === DdsUtils_DdsUtilsjs.getIntFromString("DXT3")) {
        ...
        // DXT5 format
        else if (fourCC === DdsUtils_DdsUtilsjs.getIntFromString("DXT5")) {
        ...
        // DXT10 info present...
        else if (fourCC === DdsUtils_DdsUtilsjs.getIntFromString("DX10")) {
            ...
            // }
            throw new Error(
                "dxt10 LATC formats not supported currently: " + info.headerDX10.dxgiFormat
            );
        }

        // DXT2 format - unsupported
        else if (fourCC === DdsUtils_DdsUtilsjs.getIntFromString("DXT2")) {
            throw new Error("DXT2 is not supported.");
        }

        // DXT4 format - unsupported
        else if (fourCC === DdsUtils_DdsUtilsjs.getIntFromString("DXT4")) {
    ...
    for (var i = 0; i < texture.image.depth; i++) {
        ...
        if (compressedFormat) {
            imageData.push(DdsLoader_readDXT(data, totalSize, info, texture));
        }

        // read in uncompressed data
        else if (rgb || lum || alpha) {
            imageData.push(
                DdsLoader_readUncompressed(data, totalSize, rgb, lum, alpha, alphaPixels, info, texture)
            );
        }
    }
...
DdsLoader.prototype.load = function (buffer, tex, flipped, arrayByteOffset, arrayByteLength) {
	...
	if (dwMagic !== DdsUtils_DdsUtilsjs.getIntFromString('DDS ')) {
...
//		console.info('Reading DDS file.');
	...
	info.headerDX10 = info.header.ddpf.dwFourCC === DdsUtils_DdsUtilsjs.getIntFromString('DX10') ? DdsHeader.read(Int32Array.create(buffer,
	...
	DdsLoader_updateDepth(image, info);

	// add our format and image data.
	var contentOffset = 128 + (info.headerDX10 ? 20 : 0);
	DdsLoader_populate(tex, info, new Uint8Array(buffer, arrayByteOffset + contentOffset, arrayByteLength - contentOffset));
...
DdsLoader.prototype.isSupported = function () {
	return !!Capabilities_Capabilitiesjs.CompressedTextureS3TC;
};
