    ...
    for (var sourceRow = 0; sourceRow < blocksPerRow; sourceRow++) {
        ...
        for (var column = 0; column < blocksPerColumn; column++) {
            ...
            case "PrecompressedDXT5":
                ...
                DdsUtils_getBytesFromUInt24(
                    returnData,
                    target + 5,
                    DdsUtils_flipUInt24(DdsUtils_getUInt24(rawData, source + 2))
                );
                DdsUtils_getBytesFromUInt24(
                    returnData,
                    target + 2,
                    DdsUtils_flipUInt24(DdsUtils_getUInt24(rawData, source + 5))
                );
...
DdsUtils_flipUInt24 = function(uint24) {
    ...
    threeBits[0][0] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[0][1] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[0][2] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[0][3] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[1][0] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[1][1] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[1][2] = uint24 & DdsUtils_ThreeBitMask;
    uint24 >>= 3;
    threeBits[1][3] = uint24 & DdsUtils_ThreeBitMask;
