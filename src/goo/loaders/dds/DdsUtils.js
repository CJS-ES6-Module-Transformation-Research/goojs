...
var functionObject_isSupported = function() {
    return !!Capabilities.CompressedTextureS3TC;
};
...
var functionObject_shiftCount = function(mask) {
    if (mask === 0) {
        return 0;
    }
...
var functionObject_isSet = function(value, bitMask) {
    return (value & bitMask) === bitMask;
};
...
var functionObject_getIntFromString = function(string) {
    var bytes = [];
    for (var i = 0; i < string.length; i++) {
        bytes[i] = string.charCodeAt(i);
    }
...
var functionObject_getIntFromBytes = function(bytes) {
    var rVal = 0;
    rVal |= (bytes[0] & 255) << 0;
    if (bytes.length > 1) {
        rVal |= (bytes[1] & 255) << 8;
...
var functionObject_getComponents = function(format) {
    switch (format) {
    case "Alpha":
        return 1;
    case "RGB":
...
var functionObject_flipDXT = function(rawData, width, height, format) {
    var returnData = new Uint8Array(rawData.length);
...
var functionObject_getUInt24 = function(input, offset) {
    var result = 0;
...
var functionObject_getBytesFromUInt24 = function(input, offset, uint24) {
...
var functionObject_ThreeBitMask = 7;
...
var functionObject_flipUInt24 = function(uint24) {
    var threeBits = [];
    for (var i = 0; i < 2; i++) {
        threeBits.push([0, 0, 0, 0]);
    }
