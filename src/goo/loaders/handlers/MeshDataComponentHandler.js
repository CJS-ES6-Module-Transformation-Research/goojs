...
ComponentHandler_ComponentHandlerjs._registerClass('meshData', MeshDataComponentHandler);
...
MeshDataComponentHandler.prototype._create = function () {
	return new MeshDataComponent_MeshDataComponentjs();
};
...
MeshDataComponentHandler.prototype.update = function (entity, config, options) {
	...
	return ComponentHandler_ComponentHandlerjs.prototype.update.call(this, entity, config, options).then(function (component) {
		...
		if (config.shape) {
			var shapeCreator = ShapeCreatorMemoized_ShapeCreatorMemoizedjs['create' + StringUtils_StringUtilsjs.capitalize(config.shape)];
			if (shapeCreator) {
			...
			promises.push(that._load(config.meshRef, options).then(function (meshData) {
				...
				if (meshData.boundingBox) {
					...
					var center = new Vector3_Vector3js(max[0] + min[0], max[1] + min[1], max[2] + min[2]).scale(0.5);
					var bounding = new BoundingBox_BoundingBoxjs(center, size[0] / 2, size[1] / 2, size[2] / 2);
