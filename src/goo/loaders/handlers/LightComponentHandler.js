...
LightComponentHandler.prototype = Object.create(ComponentHandler_ComponentHandlerjs.prototype);
LightComponentHandler.prototype.constructor = LightComponentHandler;
ComponentHandler_ComponentHandlerjs._registerClass('light', LightComponentHandler);
...
LightComponentHandler.prototype._prepare = function (config) {
	...
	if (config.shadowCaster) {
		...
		ObjectUtils_ObjectUtilsjs.defaults(config.shadowSettings, {
			shadowType: 'Basic',
			near: 1,
			far: 1000,
			resolution: [512, 512],
...
LightComponentHandler.prototype._create = function () {
	return new LightComponent_LightComponentjs();
};
...
LightComponentHandler.prototype.update = function (entity, config, options) {
	var that = this;
	var Light = {
		SpotLight_SpotLightjs: SpotLight_SpotLightjs,
		DirectionalLight_DirectionalLightjs: DirectionalLight_DirectionalLightjs,
		PointLight_PointLightjs: PointLight_PointLightjs
	};
			...
			if (light.hasOwnProperty(key)) {
				if (key === 'shadowSettings') {
					for (var key in value) {
						...
						if (light.shadowSettings[key] instanceof Vector3_Vector3js) {
							light.shadowSettings[key].setDirect(shadowVal[0], shadowVal[1], shadowVal[2]);
						} else {
							light.shadowSettings[key] = ObjectUtils_ObjectUtilsjs.clone(shadowVal);
						}
					}
				} else if (light[key] instanceof Vector3_Vector3js) {
					light[key].setDirect(value[0], value[1], value[2]);
				} else {
					light[key] = ObjectUtils_ObjectUtilsjs.clone(value);
				}
			}
		}
