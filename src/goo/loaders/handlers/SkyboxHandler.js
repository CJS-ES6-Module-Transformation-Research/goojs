var SkyboxHandler_SkyboxHandler = SkyboxHandler;
...
import {     EnvironmentHandler as loadershandlersEnvironmentHandler_EnvironmentHandlerjs, } from "../../loaders/handlers/EnvironmentHandler";
import { Texture as rendererTexture_Texturejs } from "../../renderer/Texture";
import { SKYBOX as ShaderBuilderjs_SKYBOX, SKYSPHERE as ShaderBuilderjs_SKYSPHERE } from "../../renderer/shaders/ShaderBuilder";
...
var ShaderBuilderjs_SKYSPHERE_binding = ShaderBuilderjs_SKYSPHERE;
var ShaderBuilderjs_SKYBOX_binding = ShaderBuilderjs_SKYBOX;
...
function SkyboxHandler() {
	...
	// Skybox entity
	var skybox = new utilSkybox_Skyboxjs('box', [], null, 0);
	...
	// Skybox texture
	this._skyboxTexture = new rendererTexture_Texturejs(null, { flipY: false });
	...
	// Skysphere entity
	var skysphere = new utilSkybox_Skyboxjs('sphere', [], null, 0);
	...
	// Skysphere texture
	this._skysphereTexture = new rendererTexture_Texturejs(null, { flipY: false, wrapS: 'EdgeClamp', wrapT: 'EdgeClamp' });
	this._skysphere.meshRendererComponent.materials[0].setTexture('DIFFUSE_MAP', this._skysphereTexture);

	this._activeSkyshape = null;
}

SkyboxHandler.prototype = Object.create(loadershandlersConfigHandler_ConfigHandlerjs.prototype);
SkyboxHandler.prototype.constructor = SkyboxHandler;
ConfigHandlerjs__registerClass('skybox', SkyboxHandler);
...
SkyboxHandler.prototype._remove = function (ref) {
	...
	if (this._activeSkyboxRef === ref) {
		...
		this._activeSkyshape = null;
		ShaderBuilderjs_SKYBOX_binding = null;
		ShaderBuilderjs_SKYSPHERE_binding = null;
		this._activeSkyboxRef = null;
	}
};
...
SkyboxHandler.prototype._update = function (ref, config, options) {
	var that = this;
	return loadershandlersConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options).then(function (skybox) {
		if (!skybox) {
		...
		return utilrsvp_rsvpjsjs.all(promises).then(function (skyboxes) {
...
SkyboxHandler.prototype._updateSphere = function (ref, config, options, skybox) {
	...
	if (config.sphereRef) {
		return this._load(config.sphereRef, options).then(function (texture) {
			if (!texture || !texture.image) {
				entitiesSystemBus_SystemBusjsjs.emit('goo.error.skybox', {
					type: 'Sphere',
			...
			if (ref === loadershandlersEnvironmentHandler_EnvironmentHandlerjs.currentSkyboxRef && config.enabled) {
	...
	return utilPromiseUtils_PromiseUtilsjs.resolve(that._skysphere);
};
...
SkyboxHandler.prototype._updateBox = function (ref, config, options, skybox) {
	...
	var promises = sides.map(function (side) {
		return config[side] ? that._load(config[side], options) : utilPromiseUtils_PromiseUtilsjs.resolve();
	});
...
SkyboxHandler.prototype._show = function (skyshape) {
	...
	ShaderBuilderjs_SKYBOX_binding = skyshape === this._skybox ? this._skyboxTexture : null;
	ShaderBuilderjs_SKYSPHERE_binding = skyshape === this._skysphere ? this._skysphereTexture : null;
};

export { SkyboxHandler_SkyboxHandler as SkyboxHandler };