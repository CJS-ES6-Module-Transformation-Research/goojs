...
import { currentSkyboxRef as EnvironmentHandlerjs_currentSkyboxRef } from "../../loaders/handlers/EnvironmentHandler";
...
SkyboxHandler.prototype._updateSphere = function (ref, config, options, skybox) {
	...
	if (config.sphereRef) {
		return this._load(config.sphereRef, options).then(function (texture) {
			...
			if (ref === EnvironmentHandlerjs_currentSkyboxRef && config.enabled) {
