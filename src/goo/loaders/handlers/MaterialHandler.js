...
MaterialHandler.prototype = Object.create(ConfigHandler_ConfigHandlerjs.prototype);
MaterialHandler.prototype.constructor = MaterialHandler;
ConfigHandler_ConfigHandlerjs._registerClass('material', MaterialHandler);
...
MaterialHandler.prototype._prepare = function (config) {
	...
	ObjectUtils_ObjectUtilsjs.defaults(config.blendState, {
		blending: 'NoBlending',
	...
	ObjectUtils_ObjectUtilsjs.defaults(config.cullState, {
		enabled: true,
		cullFace: 'Back',
		frontFace: 'CCW'
	});

	ObjectUtils_ObjectUtilsjs.defaults(config.depthState, {
		enabled: true,
		write: true
	});
};
...
MaterialHandler.prototype._create = function () {
	return new Material_Materialjs();
};
...
MaterialHandler.prototype._update = function (ref, config, options) {
	var that = this;
	return ConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options).then(function (material) {
		if (!material) { return; }

		var promises = [];

		// Material settings
		ObjectUtils_ObjectUtilsjs.extend(material.blendState, config.blendState);
		ObjectUtils_ObjectUtilsjs.extend(material.cullState, config.cullState);
		ObjectUtils_ObjectUtilsjs.extend(material.depthState, config.depthState);
		...
		if (config.renderQueue === -1) {
			if (config.blendState.blending !== 'NoBlending') {
				material.renderQueue = RenderQueue_RenderQueuejs.TRANSPARENT;
			} else {
		...
		for (var name in config.uniforms) {
			if (config.uniforms[name].enabled === undefined) {
				material.uniforms[name] = ObjectUtils_ObjectUtilsjs.clone(config.uniforms[name]);
			} else if (config.uniforms[name].enabled) {
				material.uniforms[name] = ObjectUtils_ObjectUtilsjs.clone(config.uniforms[name].value);
			}
		}
		...
		if (!shaderRef) {
			material.shader = Material_Materialjs.createShader(ShaderLib_ShaderLibjs.texturedLit, 'DefaultShader');
		}
			...
			material.shader = Material_Materialjs.createShader(ShaderLib_ShaderLibjs[shaderName]);
		} else {
