...
EnvironmentHandler.prototype._remove = function (ref) {
	...
	ShaderBuilderjs_CLEAR_COLOR = defaults.backgroundColor;
	ShaderBuilderjs_GLOBAL_AMBIENT = defaults.globalAmbient.slice(0, 3);
	ShaderBuilderjs_USE_FOG = defaults.fog.enabled;
	ShaderBuilderjs_FOG_COLOR = defaults.fog.color.slice(0, 3);
	ShaderBuilderjs_FOG_SETTINGS = [defaults.fog.near, defaults.fog.far];
...
EnvironmentHandler.prototype._update = function (ref, config, options) {
	...
	return ConfigHandler_ConfigHandlerjs.prototype._update.call(this, ref, config, options).then(function (object) {
		...
		// Fog and ambient
		ShaderBuilderjs_CLEAR_COLOR = object.backgroundColor;
		ShaderBuilderjs_GLOBAL_AMBIENT = object.globalAmbient;
		ShaderBuilderjs_USE_FOG = object.fog.enabled;
		ShaderBuilderjs_FOG_COLOR = object.fog.color.slice(0, 3);
		ShaderBuilderjs_FOG_SETTINGS = [object.fog.near, config.fog.far];
