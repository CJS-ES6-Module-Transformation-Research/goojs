import { ConfigHandler as ConfigHandlerjs } from "./handlers/ConfigHandler";
...
import { ShapeCreatorMemoized as ShapeCreatorMemoizedjs } from "../util/ShapeCreatorMemoized";
...
function DynamicLoader(options) {
	...
	if (options.ajax) {
		...
		this._ajax = new Ajaxjs(options.rootPath);
	} else {
...
DynamicLoader.prototype.clear = function () {
	...
	if (this._world && this._world.gooRunner) {
		ShapeCreatorMemoizedjs.clearCache(this._world.gooRunner.renderer.context);
...
DynamicLoader.prototype._updateObject = function (ref, config, options) {
	...
	if (handler) {
		...
		return PromiseUtilsjs.resolve(config);
	} else {
...
DynamicLoader.prototype._loadBinariesFromRefs = function (references, options) {
	...
	function traverse(refs) {
		...
		return traverseFn({ collectionRefs: refs }).then(function () {
			return ArrayUtilsjs.fromValues(binaryRefs);
		});
	}
...
DynamicLoader.prototype._getHandler = function (type) {
	...
	var Handler = ConfigHandlerjs.getHandler(type);
	if (Handler) {
...
DynamicLoader._getRefsFromConfig = function (config) {
	...
	function traverse(key, value) {
		// Multiple refs
		if (StringUtilsjs.endsWith(key.toLowerCase(), 'refs') && value instanceof Object) {
			var foundRefs = 0;
		...
		// Single ref
		if (
			StringUtilsjs.endsWith(key.toLowerCase(), 'ref') &&
			key !== 'thumbnailRef' &&
			isValidId(value)
		) {
			refs.push(value);
...
DynamicLoader._isRefTypeInGroup = function (ref, group) {
	var type = DynamicLoader.getTypeForRef(ref);
	return type && Ajaxjs.types[group] && Ajaxjs.types[group][type];
};
