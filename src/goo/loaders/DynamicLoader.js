import { ConfigHandler as ConfigHandler_ConfigHandlerjs } from "./handlers/ConfigHandler";
...
import { ShapeCreatorMemoized as ShapeCreatorMemoized_ShapeCreatorMemoizedjs } from "../util/ShapeCreatorMemoized";
...
DynamicLoader.prototype.clear = function () {
	...
	if (this._world && this._world.gooRunner) {
		ShapeCreatorMemoized_ShapeCreatorMemoizedjs.clearCache(this._world.gooRunner.renderer.context);
...
DynamicLoader.prototype._updateObject = function (ref, config, options) {
	...
	if (handler) {
		...
		return PromiseUtils_PromiseUtilsjs.resolve(config);
	} else {
...
DynamicLoader.prototype._loadBinariesFromRefs = function (references, options) {
	...
	function traverse(refs) {
		...
		return traverseFn({ collectionRefs: refs }).then(function () {
			return ArrayUtils_ArrayUtilsjs.fromValues(binaryRefs);
		});
	}
...
DynamicLoader.prototype._getHandler = function (type) {
	...
	var Handler = ConfigHandler_ConfigHandlerjs.getHandler(type);
	if (Handler) {
...
DynamicLoader._getRefsFromConfig = function (config) {
	...
	function traverse(key, value) {
		// Multiple refs
		if (StringUtils_StringUtilsjs.endsWith(key.toLowerCase(), 'refs') && value instanceof Object) {
			var foundRefs = 0;
		...
		// Single ref
		if (
			StringUtils_StringUtilsjs.endsWith(key.toLowerCase(), 'ref') &&
			key !== 'thumbnailRef' &&
			isValidId(value)
		) {
			refs.push(value);
...
DynamicLoader._isRefTypeInGroup = function (ref, group) {
	var type = DynamicLoader.getTypeForRef(ref);
	return type && Ajax_Ajaxjs.types[group] && Ajax_Ajaxjs.types[group][type];
};
