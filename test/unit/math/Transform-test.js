...
function checkInversion(transform) {
	var vec1 = new srcgoomathVector3_Vector3js(100, 200, 300);
	var vec2 = new srcgoomathVector3_Vector3js();
	var vec3 = new srcgoomathVector3_Vector3js();
...
describe('Transform', function () {
	...
	beforeEach(function () {
		...
		t = new srcgoomathTransform_Transformjs();
		v1 = new srcgoomathVector3_Vector3js(10, 20, 30);
		v2 = new srcgoomathVector3_Vector3js(0, 0, 0);
	});
	...
	it('can be scaled', function () {
		...
		t.update();
		t.applyForward(v1, v2);
		expect(v2).toBeCloseToVector(new srcgoomathVector3_Vector3js(10 * 2, 20 * 3, 30 * 4));
	});
	...
	it('rotates around X axis', function () {
		...
		t.update();
		t.applyForward(v1, v2);
		expect(v2).toBeCloseToVector(new srcgoomathVector3_Vector3js(10, -30, 20));
	});
	...
	describe('lookAt', function () {
		it('centers the lookAt point in the view', function () {
			var lookAt = new srcgoomathVector3_Vector3js(5, 0, -10);
			var up = new srcgoomathVector3_Vector3js(0, 1, 0);
			var distance = lookAt.length();
			t.lookAt(lookAt, up);
			t.update();
			t.invert().applyForwardVector(lookAt, v2);
			expect(v2).toBeCloseToVector(new srcgoomathVector3_Vector3js(0, 0, -distance));
		});

		it('defaults up parameter of lookAt to UNIT_Y', function () {
			var transform1 = new srcgoomathTransform_Transformjs();
			var transform2 = new srcgoomathTransform_Transformjs();

			transform1.lookAt(new srcgoomathVector3_Vector3js(1, 2, 3));
			transform2.lookAt(new srcgoomathVector3_Vector3js(1, 2, 3), srcgoomathVector3_Vector3js.UNIT_Y);
			...
			transform1.lookAt(new srcgoomathVector3_Vector3js(1, 2, 3));
			transform2.lookAt(new srcgoomathVector3_Vector3js(1, 2, 3), srcgoomathVector3_Vector3js.UNIT_Z);
		...
		it('does nothing when trying to look at itself', function () {
			...
			transform.lookAt(new srcgoomathVector3_Vector3js(11, 22, 33));
			transform.update();

			var expected = new srcgoomathTransform_Transformjs();
	...
	describe('combine', function () {
		it('combines and updates the resulting transform', function () {
			...
			var result = srcgoomathTransform_Transformjs.combine(transform1, transform2);
			expect(result.translation.equals(new srcgoomathVector3_Vector3js(1, 2, 3).add(new srcgoomathVector3_Vector3js(11, 22, 33)))).toBeTruthy();
	...
	describe('multiply', function () {
		it('can multiply and keep scaling correct', function () {
			...
			expect(transform1.scale).toBeCloseToVector(new srcgoomathVector3_Vector3js(1 * 4, 2 * 5, 3 * 6));
		});
	});

	describe('clone', function () {
		it('clones a transform', function () {
			var original = new srcgoomathTransform_Transformjs();
