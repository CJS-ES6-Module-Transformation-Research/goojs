...
describe('Vector', function () {
	...
	describe('add', function () {
		it('can perform addition', function () {
			...
			expect(Vectorjs.add(b, b)).toBeCloseToVector(new Vectorjs(2).set(2, 4));
		});
	});
	...
	describe('sub', function () {
		it('can perform subtraction', function () {
			...
			expect(Vectorjs.sub(b, b)).toBeCloseToVector(new Vectorjs(2).set(0, 0));
		});
	});
	...
	describe('mul', function () {
		it('can perform multiplication', function () {
			...
			expect(Vectorjs.mul(b, b)).toBeCloseToVector(new Vectorjs(2).set(1, 4));
		});
	});
	...
	describe('div', function () {
		it('can perform division', function () {
			...
			expect(Vectorjs.div(b, b)).toBeCloseToVector(new Vectorjs(2).set(1, 1));
		});
	});
	...
	describe('copy', function () {
		it('can copy values', function () {
			...
			expect(Vectorjs.copy(source)).toBeCloseToVector(new Vectorjs(2).set(1, 2));
		});
	});
	...
	it('can calculate dot products', function () {
		...
		expect(Vectorjs.dot(a, b)).toEqual(5);
	});
	...
	describe('equals', function () {
		it('can be tested for approximate equaltiy', function () {
			...
			expect(Vectorjs.equals(a, b)).toEqual(true);
			expect(a.equals(c)).toEqual(false);
			expect(Vectorjs.equals(a, c)).toEqual(false);
		});
	...
	it('can calculate distances', function () {
		...
		expect(Vectorjs.distance(a, b)).toEqual(5);
		expect(a.distanceSquared(b)).toEqual(25);
		expect(Vectorjs.distanceSquared(a, b)).toEqual(25);
	});
