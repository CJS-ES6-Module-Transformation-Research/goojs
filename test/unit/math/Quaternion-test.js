...
describe('Quaternion', function () {
	...
	describe('constructor', function () {
		it('creates a zero quaternion when given no parameters', function () {
			...
			expect(quaternion.equals(new Quaternion_Quaternionjs(0, 0, 0, 1))).toBeTruthy();
		});

		it('creates a quaternion when given 4 parameters', function () {
			var quaternion = new Quaternion_Quaternionjs(11, 22, 33, 44);

			var expected = new Quaternion_Quaternionjs();
			expected.x = 11;
			expected.y = 22;
			expected.z = 33;
			expected.w = 44;
		...
		it('creates a vector when given an array', function () {
			var vector = new Quaternion_Quaternionjs([1, 2, 3, 4]);
			var expected = new Quaternion_Quaternionjs(1, 2, 3, 4);

			expect(vector).toBeCloseToVector(expected);
		});

		it('creates a vector when given a vector', function () {
			var original = new Quaternion_Quaternionjs(1, 2, 3, 4);
			var vector = new Quaternion_Quaternionjs(original);
			var expected = new Quaternion_Quaternionjs(1, 2, 3, 4);
	...
	describe('mul', function () {
		it('can multiply two quaternions', function () {
			var p = new Quaternion_Quaternionjs(1, 0, 0, 0);
			var q = new Quaternion_Quaternionjs(0, 1, 0, 0);
			p.mul(q);

			expect(p.equals(new Quaternion_Quaternionjs(0, 0, 1, 0))).toBeTruthy();
		});
	});
	...
	it('can slerp', function () {
		...
		var quat1 = new Quaternion_Quaternionjs(Math.sin(angle1), 0, 0, Math.cos(angle1));
		var quat2 = new Quaternion_Quaternionjs(Math.sin(angle2), 0, 0, Math.cos(angle2));

		var result = new Quaternion_Quaternionjs();
		var expectedResult = new Quaternion_Quaternionjs(Math.sin(half), 0, 0, Math.cos(half));

		Quaternion_Quaternionjs.slerp(quat1, quat2, 0.5, result);
		expect(result.equals(expectedResult)).toBeTruthy();
	});

	it('can slerp via prototype method', function () {
		var startQuat = new Quaternion_Quaternionjs();
		var endQuat = new Quaternion_Quaternionjs();
		var result = new Quaternion_Quaternionjs();
		startQuat.slerp(endQuat, 0.5, result);
		expect(result).toEqual(new Quaternion_Quaternionjs());
	});

	describe('negate', function () {
		it('can negate', function () {
			var q = new Quaternion_Quaternionjs(1, 1, 1, 1);
			q.negate();
			expect(q).toEqual(new Quaternion_Quaternionjs(-1, -1, -1, -1));
		});
	});
	...
	describe('conjugate', function () {
		it('conjugates a quaternion', function () {
			var original = new Quaternion_Quaternionjs(1, 2, 3, 4);
			var conjugate = new Quaternion_Quaternionjs().copy(original).conjugate();
			expect(conjugate.equals(new Quaternion_Quaternionjs(-1, -2, -3, 4))).toBeTruthy();
		});
	});

	describe('invert', function () {
		it('inverts a quaternion', function () {
			var original = new Quaternion_Quaternionjs(1, 2, 3, 4).normalize();
			var inverse = new Quaternion_Quaternionjs().copy(original).invert();
			expect(inverse.equals(new Quaternion_Quaternionjs(-1 / 30, -2 / 30, -3 / 30, 4 / 30).normalize())).toBeTruthy();
		});
	});
	...
	it('can be set from rotation matrix', function () {
		...
		expect(quaternion.equals(new Quaternion_Quaternionjs(0, 0, 1, 0))).toBeTruthy();
	});

	it('can convert to rotation matrix', function () {
		var matrix = new Matrix3_Matrix3js();

		var quaternion = new Quaternion_Quaternionjs(0, 0, 1, 0);
		quaternion.toRotationMatrix(matrix);

		expect(matrix).toBeCloseToMatrix(new Matrix3_Matrix3js(
			-1, 0, 0,
			0, -1, 0,
			0, 0, 1
		));
	...
	it('can be set from vector to vector', function () {
		var p = new Quaternion_Quaternionjs();
		var q = new Quaternion_Quaternionjs();
		q.fromVectorToVector(new Vector3_Vector3js(1, 0, 0), new Vector3_Vector3js(0, 1, 0));
		p.fromAngleAxis(Math.PI / 2, new Vector3_Vector3js(0, 0, 1));
		expect(p).toBeCloseToVector(q);
	});

	it('can be normalized', function () {
		var q = new Quaternion_Quaternionjs(0, 0, 0, 2);
		q.normalize();
	...
	it('can be set from axis angle', function () {
		var q = new Quaternion_Quaternionjs();
		var axis = new Vector3_Vector3js(1, 0, 0);
		var angle = 0;
		q.fromAngleAxis(angle, axis);
		expect(q).toEqual(new Quaternion_Quaternionjs());
	});
	...
	it('can be set from a zero axis and angle', function () {
		var q = new Quaternion_Quaternionjs();
		var axis = new Vector3_Vector3js(0, 0, 0);
		var angle = 0;
		q.fromAngleNormalAxis(angle, axis);
		expect(q).toEqual(new Quaternion_Quaternionjs());
	});
	...
	it('can generate axis and angle 2', function () {
		var q = new Quaternion_Quaternionjs();
		var axis = new Vector3_Vector3js(1, 0, 0);
		var axisResult = new Vector3_Vector3js();
		var angle = Math.PI / 2;
		q.fromAngleNormalAxis(angle, axis);
	...
	it('can set all components via other quaternion', function () {
		var q = new Quaternion_Quaternionjs();
		var p = new Quaternion_Quaternionjs(1, 2, 3, 4);
		q.set(p);
	...
	describe('clone', function () {
		it('clones a quaternion', function () {
			...
			expect(clone).toEqual(jasmine.any(Quaternion_Quaternionjs));
	...
	describe('NaN checks (only in dev)', function () {
		it('throws an exception when trying to set a quaternion component to NaN', function () {
			var quaternion1 = new Quaternion_Quaternionjs();
	...
	describe('deprecated shim added 2015-10-07 (v1.0)', function () {
		...
		it('can add two quaternions',function () {
			var p = new Quaternion_Quaternionjs(1,1,1,1);
			var q = new Quaternion_Quaternionjs(2,2,2,2);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.add(p,q,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(3,3,3,3));
		});
		...
		it('can subtract two quaternions',function () {
			...
			var q = new Quaternion_Quaternionjs(2,2,2,2);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.sub(p,q,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(-1,-1,-1,-1));
		});
		...
		it('can multiply two quaternions',function () {
			...
			var q = new Quaternion_Quaternionjs();
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.mul(p,q,result);
		...
		it('can divide component-wise',function () {
			var p = new Quaternion_Quaternionjs(2,2,2,2);
			var q = new Quaternion_Quaternionjs(2,2,2,2);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.div(p,q,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(1,1,1,1));
		});
		...
		it('can add a scalar to a quaternion',function () {
			var p = new Quaternion_Quaternionjs(1,1,1,1);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.scalarAdd(p,1,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(2,2,2,2));
		});

		it('can subtract a scalar from a quaternion',function () {
			var p = new Quaternion_Quaternionjs(1,1,1,1);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.scalarSub(p,1,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(0,0,0,0));
		});

		it('can multiply a scalar with a quaternion',function () {
			var p = new Quaternion_Quaternionjs(1,1,1,1);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.scalarMul(p,2,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(2,2,2,2));
		});
		...
		it('can divide a quaternion with a scalar',function () {
			var p = new Quaternion_Quaternionjs(2,2,2,2);
			var result = new Quaternion_Quaternionjs();
			Quaternion_Quaternionjs.scalarDiv(p,2,result);
			expect(result).toEqual(new Quaternion_Quaternionjs(1,1,1,1));
		});
		...
		describe('conjugate', function () {
			it('conjugates a quaternion', function () {
				var original = new Quaternion_Quaternionjs(1, 2, 3, 4);
				var conjugate = new Quaternion_Quaternionjs().copy(original).conjugate();
				expect(conjugate).toBeCloseToVector(new Quaternion_Quaternionjs(-1, -2, -3, 4));
			});
		});
		...
		describe('invert', function () {
			it('inverts a quaternion', function () {
				var original = new Quaternion_Quaternionjs(1, 2, 3, 4).normalize();
				var inverse = new Quaternion_Quaternionjs().copy(original).invert();
				expect(inverse).toBeCloseToVector(new Quaternion_Quaternionjs(-1/30, -2/30, -3/30, 4/30).normalize());
			});
		});
		...
		it('can be set from rotation matrix', function () {
			...
			expect(quaternion).toBeCloseToVector(new Quaternion_Quaternionjs(0, 0, 1, 0));
		});
