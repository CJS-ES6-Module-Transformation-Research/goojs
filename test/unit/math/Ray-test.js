...
describe('Ray', function () {
	...
	describe('constructor', function () {
		it('creates a ray given no parameters', function () {
			...
			expect(ray.origin).toBeCloseToVector(Vector3_Vector3js.ZERO);
			expect(ray.direction).toBeCloseToVector(Vector3_Vector3js.UNIT_Z);
		});

		it('creates a ray given only the origin', function () {
			var origin = new Vector3_Vector3js(1, 2, 3);
			var ray = new Ray_Rayjs(origin);
		...
		it('creates a ray given the origin and direction', function () {
			var origin = new Vector3_Vector3js(1, 2, 3);
			var direction = new Vector3_Vector3js(123, 234, 345);
			var ray = new Ray_Rayjs(origin, direction);
	...
	it('intersects triangle', function () {
		var ray = new Ray_Rayjs(new Vector3_Vector3js(0, 0, -1), new Vector3_Vector3js(0, 0, 1));
		var triangle = [
			new Vector3_Vector3js(-0.1, -0.1, 0),
			new Vector3_Vector3js(1.0, -0.1, 0),
			new Vector3_Vector3js(1.0, 1.0, 0)
		];
		var store = new Vector3_Vector3js(1, 1, 1);
		ray.intersects(triangle, false, store);
		expect(store).toEqual(new Vector3_Vector3js(0, 0, 0));
	});

	it('intersects quad', function () {
		var ray = new Ray_Rayjs(new Vector3_Vector3js(0, 0, -1), new Vector3_Vector3js(0, 0, 1));
		var quad = [
			new Vector3_Vector3js(-1, -1, 0),
			new Vector3_Vector3js(1, -1, 0),
			new Vector3_Vector3js(1, 1, 0),
			new Vector3_Vector3js(-1, 1, 0)
		];
	...
	describe('intersectsPlane', function () {
		it('intersects a plane', function () {
			//! AT: split in 2
			var plane = new Plane_Planejs(new Vector3_Vector3js(1, 0, 0), 4);
			var parallelRay = new Ray_Rayjs(new Vector3_Vector3js(0, 0, 0), new Vector3_Vector3js(0, 1, 0));
			var intersectingRay = new Ray_Rayjs(new Vector3_Vector3js(0, 0, 0), new Vector3_Vector3js(1, 0, 0));

			var intersectionPoint = new Vector3_Vector3js();
			...
			expect(intersectionPoint.equals(new Vector3_Vector3js(4, 0, 0))).toBeTruthy();
		});
	});
	...
	describe('distanceSquared', function () {
		it('computes the squared distance from a ray to a point', function () {
			//! AT: split in 2
			var ray = new Ray_Rayjs(new Vector3_Vector3js(4, 0, 0), new Vector3_Vector3js(0, 0, 1));
			var point = new Vector3_Vector3js(1, 0, 10);
			var collinearPoint = new Vector3_Vector3js(4, 0, 20);

			var closestPoint = new Vector3_Vector3js();

			expect(ray.distanceSquared(point, closestPoint)).toBeCloseTo(Math.pow(3, 2));
			expect(closestPoint.equals(new Vector3_Vector3js(4, 0, 10))).toBeTruthy();

			expect(ray.distanceSquared(collinearPoint, closestPoint)).toBeCloseTo(Math.pow(0, 2));
			expect(closestPoint.equals(new Vector3_Vector3js(4, 0, 20))).toBeTruthy();
		});
	});

	describe('copy', function () {
		it('can copy everything from another ray', function () {
			var original = new Ray_Rayjs(new Vector3_Vector3js(1, 2, 3), new Vector3_Vector3js(4, 5, 6));
			var copy = new Ray_Rayjs();
			copy.copy(original);
