import { Selection as srcgooentitiesSelection_Selectionjs } from "../../../src/goo/entities/Selection";
...
describe('Selection', function () {
	...
	describe('constructor', function () {
		it('constructs an empty selection if given no parameters', function () {
			var selection = new srcgooentitiesSelection_Selectionjs();

			expect(selection).toEqual(srcgooentitiesSelection_Selectionjs.EMPTY);
		});

		it('constructs a selection from an object', function () {
			var obj1 = someObject();
			var obj2 = someObject();
			var selection = new srcgooentitiesSelection_Selectionjs(obj1);
		...
		it('constructs a selection from a bunch of a objects', function () {
			...
			var obj2 = someObject();
			var obj3 = someObject();
			var selection = new srcgooentitiesSelection_Selectionjs(obj1, obj2);
		...
		it('constructs a selection from another selection', function () {
			...
			var obj2 = someObject();
			var obj3 = someObject();
			var selection1 = new srcgooentitiesSelection_Selectionjs([obj1, obj2]);
			var selection2 = new srcgooentitiesSelection_Selectionjs(selection1);
	...
	describe('each', function () {
		it('iterates over every element', function () {
			var array = [11, 22, 33];
			var selection = new srcgooentitiesSelection_Selectionjs(array);
			var sum = 0;
	...
	describe('and', function () {
		...
		it('concatenates two selection with common elements', function () {
			...
			var selection = new srcgooentitiesSelection_Selectionjs(array1);
	...
	describe('andSelf', function () {
		...
		it('returns itself when applied to an selection that has only one stack entry', function () {
			var selection = new srcgooentitiesSelection_Selectionjs(someObjects(5));
			selection.andSelf();
			expect(selection).toEqual(selection);
			expect(selection).toBe(selection);
		});
		...
		it('add the previous selection to the current one', function () {
			...
			array.forEach(function (element) {
				expect(selection.contains(element)).toBeTruthy();
				expect(srcgooentitiesSelection_Selectionjs.contains(element * 10)).toBeTruthy();
			});
