...
describe('MovementComponent', function () {
	describe('Test velocity deltas', function () {
		...
		beforeEach(function () {
			spatialMovementComponent = new MovementComponentjs();
		});

		it('adds velocity once', function () {
			var velocity = new Vector3js(0, 1, 0);
		...
		it('adds velocity some times', function () {
			...
			spatialMovementComponent.addVelocity(velocity);
			spatialMovementComponent.addVelocity(velocity);
			expect(spatialMovementComponent.getVelocity()).toEqual(new Vector3js(2, 2, 0));
			velocity.setDirect(-1, 0, 1);
			spatialMovementComponent.addVelocity(velocity);
			spatialMovementComponent.addVelocity(velocity);
			expect(spatialMovementComponent.getVelocity()).toEqual(new Vector3js(0, 2, 2));
		});
		...
		it('adds rotational velocity some times', function () {
			...
			spatialMovementComponent.addRotationVelocity(velocity);
			spatialMovementComponent.addRotationVelocity(velocity);
			expect(spatialMovementComponent.getRotationVelocity()).toEqual(new Vector3js(2, 2, 0));
			velocity.setDirect(-1, 0, 1);
			spatialMovementComponent.addRotationVelocity(velocity);
			spatialMovementComponent.addRotationVelocity(velocity);
			expect(spatialMovementComponent.getRotationVelocity()).toEqual(new Vector3js(0, 2, 2));
		});
