...
describe('MovementComponent', function () {
	describe('Test velocity deltas', function () {
		...
		beforeEach(function () {
			spatialMovementComponent = new _MovementComponent.MovementComponent();
		});

		it('adds velocity once', function () {
			var velocity = new _Vector.Vector3(0, 1, 0);
		...
		it('adds velocity some times', function () {
			...
			spatialMovementComponent.addVelocity(velocity);
			spatialMovementComponent.addVelocity(velocity);
			expect(spatialMovementComponent.getVelocity()).toEqual(new _Vector.Vector3(2, 2, 0));
			velocity.setDirect(-1, 0, 1);
			spatialMovementComponent.addVelocity(velocity);
			spatialMovementComponent.addVelocity(velocity);
			expect(spatialMovementComponent.getVelocity()).toEqual(new _Vector.Vector3(0, 2, 2));
		});
		...
		it('sets velocity', function () {
			...
			velocity = new _Vector.Vector3(3, 0, 0);
			spatialMovementComponent.setVelocity(velocity);
			expect(spatialMovementComponent.getVelocity()).toEqual(velocity);
		});
		...
		it('adds rotational velocity some times', function () {
			...
			spatialMovementComponent.addRotationVelocity(velocity);
			spatialMovementComponent.addRotationVelocity(velocity);
			expect(spatialMovementComponent.getRotationVelocity()).toEqual(new _Vector.Vector3(2, 2, 0));
			velocity.setDirect(-1, 0, 1);
			spatialMovementComponent.addRotationVelocity(velocity);
			spatialMovementComponent.addRotationVelocity(velocity);
			expect(spatialMovementComponent.getRotationVelocity()).toEqual(new _Vector.Vector3(0, 2, 2));
		});
