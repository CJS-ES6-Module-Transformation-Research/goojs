...
describe('TransformComponent', function () {
	...
	it('can set, add and get rotation', function () {
		...
		expect(transformComponent.getRotation()).toBeCloseToVector(new Vector3js(0.2, 0.4, 0.6 + 0.5));
	});
	...
	it('can set translation', function () {
		var tc = new TransformComponentjs();
		var translation;

		tc.setTranslation(1, 2, 3);
		translation = tc.getTranslation();
		expect(translation).toEqual(new Vector3js(1, 2, 3));

		tc.setTranslation([4, 5, 6]);
		translation = tc.getTranslation();
		expect(translation).toEqual(new Vector3js(4, 5, 6));

		tc.setTranslation(7, 8, 9);
		translation = tc.getTranslation();
		expect(translation).toEqual(new Vector3js(7, 8, 9));
	});

	it('can get world translation', function () {
		var parent = new TransformComponentjs();
		var child = new TransformComponentjs();
		...
		expect(translation).toEqual(new Vector3js(2, 2, 3));
	});
	...
	it('can add translation', function () {
		...
		translation = tc.getTranslation();
		expect(translation).toEqual(new Vector3js(1, 2, 4));
		...
		tc.setTranslation(new Vector3js(1, 2, 3));
		tc.addTranslation(new Vector3js(0, 0, 1));
		translation = tc.getTranslation();
		expect(translation).toEqual(new Vector3js(1, 2, 4));
	});
	...
	it('can set scale', function () {
		...
		tc.setScale(1, 2, 3);
		scale = tc.getScale();
		expect(scale).toEqual(new Vector3js(1, 2, 3));

		tc.setScale([4, 5, 6]);
		scale = tc.getScale();
		expect(scale).toEqual(new Vector3js(4, 5, 6));

		tc.setScale(7, 8, 9);
		scale = tc.getScale();
		expect(scale).toEqual(new Vector3js(7, 8, 9));
	});
	...
	it('can get world scale', function () {
		...
		expect(scale).toEqual(new Vector3js(2, 2, 3));
	});
	...
	it('can set rotation matrix', function () {
		...
		tc.setRotationMatrix(new Matrix3js([1, 2, 3, 4, 5, 6, 7, 8, 9]));
		matrix = tc.getRotationMatrix();
		expect(matrix).toEqual(new Matrix3js([1, 2, 3, 4, 5, 6, 7, 8, 9]));
	});
	...
	it('can move', function () {
		var tc = new TransformComponentjs();
		tc.lookAt(new Vector3js(1, 0, 0)); // look along the positive x axis
		...
		expect(translation).toBeCloseToVector(new Vector3js(10, 0, 0));
		tc.move(new Vector3js(0, 0, 1));
		expect(translation).toBeCloseToVector(new Vector3js(9, 0, 0));
	});
	...
	it('returns the host entity when calling setTranslation on it', function () {
		...
		expect(entity.setTranslation(new Vector3js(1, 2, 3))).toBe(entity);
	});
	...
	it('returns the host entity when calling any transform related method on it', function () {
		...
		expect(entity.setTranslation(new Vector3js(1, 2, 3))).toBe(entity);
		expect(entity.setScale(new Vector3js(1, 2, 3))).toBe(entity);
		expect(entity.setRotation(new Vector3js(1, 2, 3))).toBe(entity);
		expect(entity.lookAt(new Vector3js(1, 2, 3))).toBe(entity);
	});
	...
	describe('called from EntitySelection', function () {
		it('sets the translation of some entities', function () {
			var entity1 = new Entityjs(world).setComponent(new TransformComponentjs());
			var entity2 = new Entityjs(world).setComponent(new TransformComponentjs());

			new EntitySelectionjs(entity1, entity2).setTranslation(1, 2, 3);

			expect(entity1.transformComponent.transform.translation).toBeCloseToVector(new Vector3js(1, 2, 3));
			expect(entity2.transformComponent.transform.translation).toBeCloseToVector(new Vector3js(1, 2, 3));
		});
		...
		it('translates some entities', function () {
			...
			new EntitySelectionjs(entity1, entity2).addTranslation(1, 2, 3);

			expect(entity1.transformComponent.transform.translation)
			.toBeCloseToVector(new Vector3js(11 + 1, 22 + 2, 33 + 3));

			expect(entity2.transformComponent.transform.translation)
			.toBeCloseToVector(new Vector3js(44 + 1, 55 + 2, 66 + 3));
		});

		it('hides some entities', function () {
			var entity = new Entityjs(world).setComponent(new TransformComponentjs());
			new EntitySelectionjs(entity).hide();
	...
	describe('.applyOnEntity', function () {
		it('sets a TransformComponent when trying to add a 3 element array', function () {
			...
			expect(entity.transformComponent.transform.translation).toBeCloseToVector(new Vector3js(1, 2, 3));
		});

		it('modifies the TransformComponent if it already exists when trying to add a 3 element array', function () {
			var entity = new Entityjs(world);
			var transformComponent = new TransformComponentjs();
			var transformSystem = new TransformSystemjs();
		...
		it('applies all of the API functions correctly', function (){
			var entity = new Entityjs(world);
			var childEntity = new Entityjs(world);
			function traverseFunction(entity){
				expect(entity).toEqual(jasmine.any(Entityjs));
			}
			entity.set(new TransformComponentjs());
			childEntity.set(new TransformComponentjs());

			entity.setTranslation(1, 2, 3);
			expect(entity.getTranslation()).toEqual(new Vector3js(1, 2, 3));

			entity.setRotation(0, 0, 0);
			expect(entity.getRotation()).toEqual(new Vector3js(0, 0, 0));

			entity.setScale(1, 2, 3);
			expect(entity.getScale()).toEqual(new Vector3js(1, 2, 3));
			...
			expect(entity.getTranslation()).toEqual(new Vector3js(2, 4, 6));
	...
	(function () {
		function getEntity() {
			return world.createEntity().set(new MeshRendererComponentjs())
				.set(new LightComponentjs())
				.set(new HtmlComponentjs());
		}
