...
describe('EntityUtils', function () {
	...
	it('can get the root entity', function () {
		...
		expect(EntityUtils_EntityUtilsjs.getRoot(e1)).toBe(e1);
		expect(EntityUtils_EntityUtilsjs.getRoot(e2)).toBe(e1);
		expect(EntityUtils_EntityUtilsjs.getRoot(e3)).toBe(e1);
	});

	it('can get the total bounding box', function () {
		var e1 = world.createEntity(meshData, new MeshRendererComponent_MeshRendererComponentjs());
		var e2 = world.createEntity(meshData, new MeshRendererComponent_MeshRendererComponentjs(), [10, 10, 10]);
		e1.transformComponent.attachChild(e2.transformComponent);
		var e3 = world.createEntity(meshData, new MeshRendererComponent_MeshRendererComponentjs(), [10, 10, 10]);
		e2.transformComponent.attachChild(e3.transformComponent);
		world.process();
		...
		var bb = EntityUtils_EntityUtilsjs.getTotalBoundingBox(e1);
