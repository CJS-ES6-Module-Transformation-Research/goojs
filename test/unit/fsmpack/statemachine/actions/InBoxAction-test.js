import { InBoxAction as InBoxAction_InBoxActionjs } from "../../../../../src/goo/fsmpack/statemachine/actions/InBoxAction";
...
describe('InBoxAction', function () {
	describe('Check pos against boxes', function () {
		...
		it('box [0, 0, 0], [2, 2, 2] is inside at pos [1,1,1]', function () {
			...
			inBoxAction = new InBoxAction_InBoxActionjs('testId', settings);

			entity.transformComponent.setTranslation(new Vector3_Vector3js(1, 1, 1));
			spyOn(mockFsm, 'send');
		...
		it('box [0, 0, 0], [2, 2, 2] is outside at pos [3,3,3]', function () {
			...
			entity.transformComponent.setTranslation(new Vector3_Vector3js(3, 3, 3));
			spyOn(mockFsm, 'send');
			inBoxAction.update(mockFsm);
		...
		it('box [-90, -100, -100], [-110, 100, 100] is inside at pos [-100,0,0]', function () {
			...
			entity.transformComponent.setTranslation(new Vector3_Vector3js(-100, 0, 0));
			spyOn(mockFsm, 'send');
			inBoxAction.update(mockFsm);
			expect(mockFsm.send).toHaveBeenCalledWith(settings.transitions.inside);
		});
