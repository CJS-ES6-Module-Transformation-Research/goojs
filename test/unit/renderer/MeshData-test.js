...
describe('MeshData', function () {
	...
	it('can translate vertices', function () {
		...
		transform.update();
		box.applyTransform(MeshDatajs.POSITION, transform);
	...
	it('can apply a function on vertices', function () {
		...
		box.applyFunction(MeshDatajs.POSITION, function (vert) {
	...
	it('can get attribute buffer', function () {
		...
		expect(getAttributeBuffer(MeshDatajs.POSITION)).toBeDefined();
		expect(getAttributeBuffer(MeshDatajs.NORMAL)).toBeDefined();
		expect(getAttributeBuffer(MeshDatajs.COLOR)).toBeUndefined();
		expect(getAttributeBuffer(MeshDatajs.TANGENT)).toBeUndefined();
		expect(getAttributeBuffer(MeshDatajs.TEXCOORD0)).toBeDefined();
		expect(getAttributeBuffer(MeshDatajs.TEXCOORD1)).toBeUndefined();
		expect(getAttributeBuffer(MeshDatajs.TEXCOORD2)).toBeUndefined();
		expect(getAttributeBuffer(MeshDatajs.TEXCOORD3)).toBeUndefined();
		expect(getAttributeBuffer(MeshDatajs.WEIGHTS)).toBeUndefined();
		expect(getAttributeBuffer(MeshDatajs.JOINTIDS)).toBeUndefined();
	});
});
