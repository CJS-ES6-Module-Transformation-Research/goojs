...
describe('MeshData', function () {
	...
	it('can translate vertices', function () {
		var box = new Quad_Quadjs();

		var transform = new Transform_Transformjs();
		transform.translation.setDirect(1, 2, 3);
		transform.update();
		box.applyTransform(MeshData_MeshDatajs.POSITION, transform);
	...
	it('can apply a function on vertices', function () {
		...
		box.applyFunction(MeshData_MeshDatajs.POSITION, function (vert) {
	...
	it('can get attribute buffer', function () {
		...
		expect(getAttributeBuffer(MeshData_MeshDatajs.POSITION)).toBeDefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.NORMAL)).toBeDefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.COLOR)).toBeUndefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.TANGENT)).toBeUndefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD0)).toBeDefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD1)).toBeUndefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD2)).toBeUndefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.TEXCOORD3)).toBeUndefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.WEIGHTS)).toBeUndefined();
		expect(getAttributeBuffer(MeshData_MeshDatajs.JOINTIDS)).toBeUndefined();
	});
});
