...
describe('Texture', function () {
	...
	describe('create', function () {
		function testTypesAndFormats(data, defaultType, defaultFormat) {
			var settings = {};
			var texture = new srcgoorendererTexture_Texturejs(data, settings, 1, 1);
			...
			texture = new srcgoorendererTexture_Texturejs(data, settings, 1, 1);
	...
	describe('clone', function () {
		...
		it('can clone a texture holding no image', function () {
			var original = new srcgoorendererTexture_Texturejs();
		...
		it('can clone a texture holding a typed array', function () {
			var original = new srcgoorendererTexture_Texturejs(new Uint8Array([11, 22, 33, 44]), {}, 1, 1);
			var clone = original.clone();
		...
		it('can clone a texture holding an html element', function () {
			...
			var original = new srcgoorendererTexture_Texturejs(images[0]);
			var clone = original.clone();
		...
		it('can clone a texture holding an 6 html elements', function () {
			...
			var original = new srcgoorendererTexture_Texturejs(images);
			var clone = original.clone();
