...
describe('BoundingSphere', function () {
	...
	describe('containsPoint', function () {
		it('returns false for an outside point', function () {
			var boundingSphere = new BoundingSpherejs(new Vector3js(10, 20, 30), 2);
			expect(boundingSphere.containsPoint(new Vector3js(31, 19, 11))).toBeFalsy();
		});

		it('returns true for a point on the edge', function () {
			var boundingSphere = new BoundingSpherejs(new Vector3js(10, 20, 30), 2);
			expect(boundingSphere.containsPoint(new Vector3js(12, 20, 30))).toBeTruthy();
		});

		it('returns true for an inside point', function () {
			var boundingSphere = new BoundingSpherejs(new Vector3js(10, 20, 30), 2);
			expect(boundingSphere.containsPoint(new Vector3js(10, 20, 30))).toBeTruthy();
		});
	});

	describe('merge', function () {
		it('merges two identical overlapping spheres', function () {
			var boundingSphere1 = new BoundingSpherejs(new Vector3js(3, 2, 1), 5);
			var boundingSphere2 = new BoundingSpherejs(new Vector3js(3, 2, 1), 2);

			var mergedBoundingSphere = boundingSphere1.merge(boundingSphere2);
			expect(mergedBoundingSphere.center).toBeCloseToVector(new Vector3js(3, 2, 1));
			expect(mergedBoundingSphere.radius).toBeCloseTo(5);
		});

		it('merges two intersecting spheres', function () {
			var boundingSphere1 = new BoundingSpherejs(new Vector3js(-20, 0, 0), 4);
			var boundingSphere2 = new BoundingSpherejs(new Vector3js( 10, 0, 0), 8);

			var mergedBoundingSphere = boundingSphere1.merge(boundingSphere2);
			expect(mergedBoundingSphere.center).toBeCloseToVector(new Vector3js((-20 - 4 + 10 + 8) / 2, 0, 0));
	...
	describe('intersects', function () {
		...
		it('intersects a bounding sphere', function () {
			var boundingSphere1 = new BoundingSpherejs(new Vector3js(2 * 1, 3 * 1, 6 * 1), 7);
			var boundingSphere2 = new BoundingSpherejs(new Vector3js(2 * 3, 3 * 3, 6 * 3), 7);

			expect(boundingSphere1.intersects(boundingSphere2)).toBeTruthy();
		});

		it('does not intersect a bounding sphere', function () {
			var boundingSphere1 = new BoundingSpherejs(new Vector3js(2 * 1, 3 * 1, 6 * 1), 6);
			var boundingSphere2 = new BoundingSpherejs(new Vector3js(2 * 3, 3 * 3, 6 * 3), 7);
	...
	describe('copy', function () {
		it('can copy everything from another bounding sphere', function () {
			var original = new BoundingSpherejs(new Vector3js(1, 2, 3), 123);
			var copy = new BoundingSpherejs();
			copy.copy(original);
