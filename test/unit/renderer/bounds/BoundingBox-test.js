...
describe('BoundingBox', function () {
	...
	describe('containsPoint', function () {
		it('returns false for an outside point', function () {
			var boundingBox = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(10, 20, 30), 2, 2, 2);
			expect(boundingBox.containsPoint(new srcgoomathVector3_Vector3js(31, 19, 11))).toBeFalsy();
		});

		it('returns true for a point on the edge of the bounding volume (one of the corners)', function () {
			var boundingBox = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(10, 20, 30), 2, 2, 2);
			expect(boundingBox.containsPoint(new srcgoomathVector3_Vector3js(11, 19, 31))).toBeTruthy();
		});

		it('returns true for an inside point', function () {
			var boundingBox = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(10, 20, 30), 2, 2, 2);
			expect(boundingBox.containsPoint(new srcgoomathVector3_Vector3js(10, 20, 30))).toBeTruthy();
		});
	});
	...
	describe('computeFromPoints', function () {
		function buildCustomTriangle(verts) {
			...
			var meshData = new srcgoorendererMeshData_MeshDatajs(srcgoorendererMeshData_MeshDatajs.defaultMap([srcgoorendererMeshData_MeshDatajs.POSITION]), 3, indices.length);

			meshData.getAttributeBuffer(srcgoorendererMeshData_MeshDatajs.POSITION).set(verts);
		...
		it('computes the center of the bounding box from verts (of default box)', function () {
			var boundingBox1 = new srcgoorendererboundsBoundingBox_BoundingBoxjs();

			var boxMeshData = new srcgooshapesBox_Boxjs();
			boundingBox1.computeFromPoints(boxMeshData.dataViews.POSITION);
			expect(boundingBox1.center).toBeCloseToVector(new srcgoomathVector3_Vector3js(0, 0, 0));
		});
		...
		it('computes the center of the bounding box from verts (of custom triangle)', function () {
			...
			expect(boundingBox1.center).toBeCloseToVector(new srcgoomathVector3_Vector3js(1, 0, 15));
		});
		...
		it('computes max & min of the bounding box from verts (of default box)', function () {
			...
			var boxMeshData = new srcgooshapesBox_Boxjs();
			boundingBox1.computeFromPoints(boxMeshData.dataViews.POSITION);
			expect(boundingBox1.min).toBeCloseToVector(new srcgoomathVector3_Vector3js(-0.5, -0.5, -0.5));
			expect(boundingBox1.max).toBeCloseToVector(new srcgoomathVector3_Vector3js(0.5, 0.5, 0.5));
		});
		...
		it('computes max & min of the bounding box from verts (of custom triangle)', function () {
			...
			var triangleMeshData = buildCustomTriangle([0, -5, 10, 2, 5, 20, 0, 1, 11]);
			boundingBox1.computeFromPoints(triangleMeshData.dataViews.POSITION);
			expect(boundingBox1.min).toBeCloseToVector(new srcgoomathVector3_Vector3js(0, -5, 10));
			expect(boundingBox1.max).toBeCloseToVector(new srcgoomathVector3_Vector3js(2, 5, 20));
		});
	...
	describe('merge', function () {
		it('merges two identical overlapping boxes', function () {
			var boundingBox1 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(0, 0, 0), 2, 3, 4);
			var boundingBox2 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(0, 0, 0), 2, 3, 4);

			var mergedBoundingBox = boundingBox1.merge(boundingBox2);
			expect(mergedBoundingBox.center).toBeCloseToVector(new srcgoomathVector3_Vector3js(0, 0, 0));
		...
		it('merges two intersecting boxes', function () {
			var boundingBox1 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(-5, -5, -5), 10, 10, 10);
			var boundingBox2 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(10, 10, 10), 10, 10, 10);

			var mergedBoundingBox = boundingBox1.merge(boundingBox2);
			expect(mergedBoundingBox.center).toBeCloseToVector(new srcgoomathVector3_Vector3js((-15 + 20) / 2, (-15 + 20) / 2, (-15 + 20) / 2));
		...
		it('merges two nonintersecting boxes', function () {
			var boundingBox1 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(-10, -10, -10), 5, 5, 5);
			var boundingBox2 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(20, 20, 20), 10, 10, 10);

			var mergedBoundingBox = boundingBox1.merge(boundingBox2);
			expect(mergedBoundingBox.center).toBeCloseToVector(new srcgoomathVector3_Vector3js((-15 + 30) / 2, (-15 + 30) / 2, (-15 + 30) / 2));
	...
	describe('intersects', function () {
		it('intersects a bounding box', function () {
			var boundingBox1 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(0, 0, 0), 10, 10, 10);
			var boundingBox2 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(20, 20, 20), 11, 11, 11);
		...
		it('does not intersect a bounding box', function () {
			var boundingBox1 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(0, 0, 0), 10, 10, 10);
			var boundingBox2 = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(20, 20, 20), 9, 11, 11);
	...
	describe('copy', function () {
		it('can copy everything from another bounding box', function () {
			var original = new srcgoorendererboundsBoundingBox_BoundingBoxjs(new srcgoomathVector3_Vector3js(1, 2, 3), 123, 234, 345);
			var copy = new srcgoorendererboundsBoundingBox_BoundingBoxjs();
			copy.copy(original);
