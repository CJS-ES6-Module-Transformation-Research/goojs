...
import { BoundingBox as BoundingBox_BoundingBox } from "../../../../src/goo/renderer/bounds/BoundingBox";
...
describe('BoundingBox', function () {
	...
	describe('containsPoint', function () {
		it('returns false for an outside point', function () {
			var boundingBox = new BoundingBox_BoundingBox(new Vector3_Vector3(10, 20, 30), 2, 2, 2);
			expect(boundingBox.containsPoint(new Vector3_Vector3(31, 19, 11))).toBeFalsy();
		});

		it('returns true for a point on the edge of the bounding volume (one of the corners)', function () {
			var boundingBox = new BoundingBox_BoundingBox(new Vector3_Vector3(10, 20, 30), 2, 2, 2);
			expect(boundingBox.containsPoint(new Vector3_Vector3(11, 19, 31))).toBeTruthy();
		});

		it('returns true for an inside point', function () {
			var boundingBox = new BoundingBox_BoundingBox(new Vector3_Vector3(10, 20, 30), 2, 2, 2);
			expect(boundingBox.containsPoint(new Vector3_Vector3(10, 20, 30))).toBeTruthy();
		});
	});
	...
	describe('computeFromPoints', function () {
		function buildCustomTriangle(verts) {
			...
			var meshData = new MeshData_MeshData(MeshData_MeshData.defaultMap([MeshData_MeshData.POSITION]), 3, indices.length);
		...
		it('computes the center of the bounding box from verts (of default box)', function () {
			var boundingBox1 = new BoundingBox_BoundingBox();

			var boxMeshData = new Box_Box();
			boundingBox1.computeFromPoints(boxMeshData.dataViews.POSITION);
			expect(boundingBox1.center).toBeCloseToVector(new Vector3_Vector3(0, 0, 0));
		});
		...
		it('computes the center of the bounding box from verts (of custom triangle)', function () {
			...
			expect(boundingBox1.center).toBeCloseToVector(new Vector3_Vector3(1, 0, 15));
		});
		...
		it('computes max & min of the bounding box from verts (of default box)', function () {
			...
			var boxMeshData = new Box_Box();
			boundingBox1.computeFromPoints(boxMeshData.dataViews.POSITION);
			expect(boundingBox1.min).toBeCloseToVector(new Vector3_Vector3(-0.5, -0.5, -0.5));
			expect(boundingBox1.max).toBeCloseToVector(new Vector3_Vector3(0.5, 0.5, 0.5));
		});
		...
		it('computes max & min of the bounding box from verts (of custom triangle)', function () {
			...
			var triangleMeshData = buildCustomTriangle([0, -5, 10, 2, 5, 20, 0, 1, 11]);
			boundingBox1.computeFromPoints(triangleMeshData.dataViews.POSITION);
			expect(boundingBox1.min).toBeCloseToVector(new Vector3_Vector3(0, -5, 10));
			expect(boundingBox1.max).toBeCloseToVector(new Vector3_Vector3(2, 5, 20));
		});
	...
	describe('merge', function () {
		it('merges two identical overlapping boxes', function () {
			var boundingBox1 = new BoundingBox_BoundingBox(new Vector3_Vector3(0, 0, 0), 2, 3, 4);
			var boundingBox2 = new BoundingBox_BoundingBox(new Vector3_Vector3(0, 0, 0), 2, 3, 4);

			var mergedBoundingBox = boundingBox1.merge(boundingBox2);
			expect(mergedBoundingBox.center).toBeCloseToVector(new Vector3_Vector3(0, 0, 0));
		...
		it('merges two intersecting boxes', function () {
			var boundingBox1 = new BoundingBox_BoundingBox(new Vector3_Vector3(-5, -5, -5), 10, 10, 10);
			var boundingBox2 = new BoundingBox_BoundingBox(new Vector3_Vector3(10, 10, 10), 10, 10, 10);

			var mergedBoundingBox = boundingBox1.merge(boundingBox2);
			expect(mergedBoundingBox.center).toBeCloseToVector(new Vector3_Vector3((-15 + 20) / 2, (-15 + 20) / 2, (-15 + 20) / 2));
		...
		it('merges two nonintersecting boxes', function () {
			var boundingBox1 = new BoundingBox_BoundingBox(new Vector3_Vector3(-10, -10, -10), 5, 5, 5);
			var boundingBox2 = new BoundingBox_BoundingBox(new Vector3_Vector3(20, 20, 20), 10, 10, 10);

			var mergedBoundingBox = boundingBox1.merge(boundingBox2);
			expect(mergedBoundingBox.center).toBeCloseToVector(new Vector3_Vector3((-15 + 30) / 2, (-15 + 30) / 2, (-15 + 30) / 2));
	...
	describe('intersects', function () {
		it('intersects a bounding box', function () {
			var boundingBox1 = new BoundingBox_BoundingBox(new Vector3_Vector3(0, 0, 0), 10, 10, 10);
			var boundingBox2 = new BoundingBox_BoundingBox(new Vector3_Vector3(20, 20, 20), 11, 11, 11);
		...
		it('does not intersect a bounding box', function () {
			var boundingBox1 = new BoundingBox_BoundingBox(new Vector3_Vector3(0, 0, 0), 10, 10, 10);
			var boundingBox2 = new BoundingBox_BoundingBox(new Vector3_Vector3(20, 20, 20), 9, 11, 11);

			expect(boundingBox1.intersects(boundingBox2)).toBeFalsy();
		});

		it('intersects a bounding sphere', function () {
			var boundingBox = new BoundingBox_BoundingBox(new Vector3_Vector3(0, 0, 0), 10, 10, 10);
			var boundingSphere = new BoundingSphere_BoundingSphere(new Vector3_Vector3(20, 20, 0), 15);
		...
		it('does not intersect a bounding sphere', function () {
			var boundingBox = new BoundingBox_BoundingBox(new Vector3_Vector3(0, 0, 0), 10, 10, 10);
			var boundingSphere = new BoundingSphere_BoundingSphere(new Vector3_Vector3(20, 20, 0), 12);
	...
	describe('copy', function () {
		it('can copy everything from another bounding box', function () {
			var original = new BoundingBox_BoundingBox(new Vector3_Vector3(1, 2, 3), 123, 234, 345);
			var copy = new BoundingBox_BoundingBox();
			copy.copy(original);
