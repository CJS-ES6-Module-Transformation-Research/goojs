import {     WorldFittedTerrainScript as WorldFittedTerrainScript_WorldFittedTerrainScript, } from "../../../src/goo/scriptpack/WorldFittedTerrainScript";
...
xdescribe('WorldFittedTerrainScript', function () {
	describe('Uses default dimensions', function () {
		...
		beforeEach(function () {
			...
			terrainScript = new WorldFittedTerrainScript_WorldFittedTerrainScript();
		});

		var heightMatrix = [[0, 0, 0, 0], [0, 0.5, 0.5, 0], [0.5, 1, 1, 0.5], [1, 1, 1, 1]];

		it('adds bad heightmap and gets exception', function () {
			var terrainScript = new WorldFittedTerrainScript_WorldFittedTerrainScript();
		...
		it('finds the registered heightMatrix', function () {
			...
			heightData = terrainScript.getHeightDataForPosition(new Vector3_Vector3(1, 1, 1));
		...
		it('looks for positions on a default dimensions heightMatrix', function () {
			terrainScript.addHeightData(heightMatrix);
			heightAtPos = terrainScript.getTerrainHeightAt(new Vector3_Vector3(1, 1, 1));
			expect(heightAtPos).toEqual(0);
			heightAtPos = terrainScript.getTerrainHeightAt(new Vector3_Vector3(99.99, 49, 99.99));
			expect(heightAtPos).toBeCloseTo(49.9924);
		});

		it('looks outside default dimensions', function () {
			terrainScript.addHeightData(heightMatrix);
			heightAtPos = terrainScript.getTerrainHeightAt(new Vector3_Vector3(-1, 0, 0));
			expect(heightAtPos).toEqual(null);
			heightAtPos = terrainScript.getTerrainHeightAt(new Vector3_Vector3(100, 50, 101));
			expect(heightAtPos).toEqual(null);
		});
	});
