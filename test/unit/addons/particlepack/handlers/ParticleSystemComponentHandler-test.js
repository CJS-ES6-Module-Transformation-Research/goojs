import { PolyCurve as PolyCurve_PolyCurvejs } from "../../../../../src/goo/addons/particlepack/curves/PolyCurve";
...
describe('ParticleSystemComponentHandler', function () {
	...
	it('loads an entity with a ParticleSystemComponent', function (done) {
		...
		loader.load(config.id).then(function (entity) {
			expect(entity.particleSystemComponent).toEqual(jasmine.any(ParticleSystemComponent_ParticleSystemComponentjs));

			function newConstantPolyCurve(value){
				return new PolyCurve_PolyCurvejs({ segments: [new ConstantCurve_ConstantCurvejs({ value: value })] });
			}
			...
			expect(c.boxExtents).toEqual(new Vector3_Vector3js(1, 2, 3));
			...
			expect(c.sortMode).toEqual(ParticleSystemComponent_ParticleSystemComponentjs.SORT_CAMERA_DISTANCE);
