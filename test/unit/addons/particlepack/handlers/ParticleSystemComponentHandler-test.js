import { PolyCurve as PolyCurve_PolyCurve } from "../../../../../src/goo/addons/particlepack/curves/PolyCurve";
...
describe('ParticleSystemComponentHandler', function () {
	...
	it('loads an entity with a ParticleSystemComponent', function (done) {
		var config = Configs_Configs.entity(['transform', 'particleSystem']);
		...
		loader.load(config.id).then(function (entity) {
			expect(entity.particleSystemComponent).toEqual(jasmine.any(ParticleSystemComponent_ParticleSystemComponent));

			function newConstantPolyCurve(value){
				return new PolyCurve_PolyCurve({ segments: [new ConstantCurve_ConstantCurve({ value: value })] });
			}
			...
			expect(c.boxExtents).toEqual(new Vector3_Vector3(1, 2, 3));
			...
			expect(c.sortMode).toEqual(ParticleSystemComponent_ParticleSystemComponent.SORT_CAMERA_DISTANCE);
