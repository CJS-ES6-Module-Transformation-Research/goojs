...
describe('Vector4Curve', function () {
	it('.getVec4ValueAt', function () {
		var curve = new Vector4Curvejs({
			x: new ConstantCurvejs({ value: 1 }),
			y: new ConstantCurvejs({ value: 2 }),
			z: new ConstantCurvejs({ value: 3 }),
			w: new ConstantCurvejs({ value: 4 })
		});
		var store0 = new Vector4js();
		var store1 = new Vector4js();
		var store2 = new Vector4js();

		curve.getVec4ValueAt(0, 0, store0);
		curve.getVec4ValueAt(0.5, 0, store1);
		curve.getVec4ValueAt(1, 0, store2);

		expect(store0).toEqual(new Vector4js(1, 2, 3, 4));
		expect(store1).toEqual(new Vector4js(1, 2, 3, 4));
		expect(store2).toEqual(new Vector4js(1, 2, 3, 4));
	});
	...
	it('.getVec4IntegralValueAt', function () {
		...
		expect(store0).toEqual(new Vector4js(0, 0, 0, 0));
		expect(store1).toEqual(new Vector4js(0.5, 1, 1.5, 2));
		expect(store2).toEqual(new Vector4js(1, 2, 3, 4));
	});
