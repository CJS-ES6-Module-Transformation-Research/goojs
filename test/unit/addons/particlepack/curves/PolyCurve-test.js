...
describe('PolyCurve', function () {
	it('can add a segment', function () {
		var set = new PolyCurve_PolyCurvejs();
	...
	it('.getValueAt', function () {
		...
		it('can get a value from multiple curve types', function () {
			var set = new PolyCurve_PolyCurvejs();
			set.addSegment(new Curve_Curvejs({ timeOffset: 0 }));
			set.addSegment(new LinearCurve_LinearCurvejs({ timeOffset: 0.5, k: 1, m: 0 }));
	...
	it('.getIntegralValueAt', function () {
		var set = new PolyCurve_PolyCurvejs();
		var curve = new LinearCurve_LinearCurvejs({ timeOffset: 0, k: 1, m: 0 });
		set.addSegment(curve);
	...
	it('.toGLSL', function () {
		var set = new PolyCurve_PolyCurvejs();
		set.addSegment(new Curve_Curvejs({ timeOffset: 0 }));
		set.addSegment(new Curve_Curvejs({ timeOffset: 0.5 }));
		expect(set.toGLSL('t','lerp')).toBe('step(0.0,t)*step(-0.5,-t)*0.0+step(0.5,t)*step(-1.0,-t)*0.0');
	});

	it('.integralToGLSL', function () {
		var set = new PolyCurve_PolyCurvejs();
		set.addSegment(new LinearCurve_LinearCurvejs({ timeOffset: 0, k: 1, m: 0 }));
