...
import { RigidBodyComponent as RigidBodyComponent_RigidBodyComponentjs } from "../../../../../src/goo/addons/physicspack/components/RigidBodyComponent";
import { BallJoint as BallJoint_BallJointjs } from "../../../../../src/goo/addons/physicspack/joints/BallJoint";
/* global CANNON */
...
describe('RigidBodyComponent', function () {
	...
	var Quaternion = Quaternion_Quaternionjs;
	var World = World_Worldjs;
	var SystemBus = SystemBus_SystemBusjs;
	var PhysicsMaterial = PhysicsMaterial_PhysicsMaterialjs;
	var PhysicsSystem = PhysicsSystem_PhysicsSystemjs;
	var ColliderComponent = ColliderComponent_ColliderComponentjs;
	var RigidBodyComponent = RigidBodyComponent_RigidBodyComponentjs;
	var BallJoint = BallJoint_BallJointjs;
	...
	beforeEach(function () {
		...
		rigidBodyComponent = new RigidBodyComponent_RigidBodyComponentjs({ mass: 1 });
		colliderComponent = new ColliderComponent_ColliderComponentjs({
	...
	it('can set constraints', function () {
		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_NONE;
		expect(rigidBodyComponent.cannonBody.linearFactor).toEqual(new CANNON.Vec3(1, 1, 1));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION_X;
		expect(rigidBodyComponent.cannonBody.linearFactor).toEqual(new CANNON.Vec3(0, 1, 1));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION_Y;
		expect(rigidBodyComponent.cannonBody.linearFactor).toEqual(new CANNON.Vec3(1, 0, 1));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION_Z;
		expect(rigidBodyComponent.cannonBody.linearFactor).toEqual(new CANNON.Vec3(1, 1, 0));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION_X;
		expect(rigidBodyComponent.cannonBody.angularFactor).toEqual(new CANNON.Vec3(0, 1, 1));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION_Y;
		expect(rigidBodyComponent.cannonBody.angularFactor).toEqual(new CANNON.Vec3(1, 0, 1));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION_Z;
		expect(rigidBodyComponent.cannonBody.angularFactor).toEqual(new CANNON.Vec3(1, 1, 0));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_POSITION;
		expect(rigidBodyComponent.cannonBody.linearFactor).toEqual(new CANNON.Vec3(0, 0, 0));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_ROTATION;
		expect(rigidBodyComponent.cannonBody.angularFactor).toEqual(new CANNON.Vec3(0, 0, 0));

		rigidBodyComponent.constraints = RigidBodyComponent_RigidBodyComponentjs.FREEZE_ALL;
		expect(rigidBodyComponent.cannonBody.linearFactor).toEqual(new CANNON.Vec3(0, 0, 0));
		expect(rigidBodyComponent.cannonBody.angularFactor).toEqual(new CANNON.Vec3(0, 0, 0));
	...
	it('can applyForce', function () {
		...
		rigidBodyComponent.applyForce(new Vector3_Vector3js(1, 2, 3));
	...
	it('can applyForceWorld', function () {
		rigidBodyComponent.setPosition(new Vector3_Vector3js(1, 2, 3));
		rigidBodyComponent.setQuaternion(new Quaternion_Quaternionjs().fromAngleAxis(Math.PI / 4, new Vector3_Vector3js(1,0,0))); // Should not affect at all
		var worldForce = new Vector3_Vector3js(0, 1, 0);
		var worldPosition = new Vector3_Vector3js(2, 2, 3); // (1,0,0) relative to the body
	...
	it('can applyForceLocal', function () {
		rigidBodyComponent.setPosition(new Vector3_Vector3js(1, 2, 3));
		var localForce = new Vector3_Vector3js(0, 1, 0);
		var localPosition = new Vector3_Vector3js(1, 0, 0);
	...
	it('can set velocity', function () {
		rigidBodyComponent.setVelocity(new Vector3_Vector3js(1, 2, 3));
	...
	it('can get velocity', function () {
		...
		var velocity = new Vector3_Vector3js();
		rigidBodyComponent.getVelocity(velocity);
		expect(velocity).toEqual(new Vector3_Vector3js(1, 2, 3));
	});
	...
	it('can set quaternion', function () {
		rigidBodyComponent.setQuaternion(new Quaternion_Quaternionjs(1, 2, 3, 4));
	...
	it('can get quaternion', function () {
		...
		var quat = new Quaternion_Quaternionjs();
		rigidBodyComponent.getQuaternion(quat);
		expect(quat).toEqual(new Quaternion_Quaternionjs(1, 2, 3, 4));
	});
	...
	it('can add and remove a BallJoint', function () {
		var joint = new BallJoint_BallJointjs({
	...
	it('can clone', function () {
		...
		rigidBodyComponent.setAngularVelocity(new Vector3_Vector3js(4, 5, 6));
		rigidBodyComponent.setVelocity(new Vector3_Vector3js(1, 2, 3));
		...
		var angularVelocity = new Vector3_Vector3js();
		rigidBodyComponent2.getAngularVelocity(angularVelocity);
		expect(angularVelocity).toEqual(new Vector3_Vector3js(4, 5, 6));
	...
	it('can set materials per collider', function () {
		var rigidBodyComponent = new RigidBodyComponent_RigidBodyComponentjs({ mass: 1 });
		...
		var colliderComponent2 = new ColliderComponent_ColliderComponentjs({
			collider: new SphereCollider_SphereColliderjs({ radius: 1 }),
